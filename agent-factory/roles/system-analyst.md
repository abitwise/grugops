...
- The product ticket and BA/PM's filled handoff `plans/handoffs/<TICKET-ID>-product.md` — the behavior to analyze (cite the universal-header `## Scope` / `## Risks`).
...
4. Hand off behavior the architect can shape and the engineer can build without re-asking — name the unknowns, don't paper them. No framework choice, no code.

## Output (file + format)
Read the `system-handoff.md` template from `agent-factory/handoffs/` (KIT, read-only), fill it per ticket (actors, flows, state transitions, inputs/outputs, edge/error cases, open questions), and write the instance to `plans/handoffs/<TICKET-ID>-system.md` (STATE); cite the universal-header `## Scope` / `## Risks` as authoritative.
