...
## Agents involved
- Security/NFR — runs the deep ASVS audit and writes `plans/handoffs/<TICKET-ID>-security-nfr.md`.
...
## Handoffs produced
Under `plans/handoffs/` (filled from the templates in `agent-factory/handoffs/`): `<TICKET-ID>-security-nfr.md` (Security/NFR — the severity-tagged audit, filled from `agent-factory/handoffs/security-nfr-handoff.md`).
...
## Commit
Commit the artifacts this workflow wrote (the board context, the security-nfr audit handoff, and the updated traceability rows) per `agent-factory/_commit-convention.md` — branch guard first (never a protected branch; switch to `grugops/security-audit-<id>`), then `type(scope): summary`. Never merge, never deploy; humans hold both.
