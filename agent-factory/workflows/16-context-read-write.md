...
Every stop condition of step 3's upward search, quoted from `TRUSTED_ROOT_STOP_CONDITIONS` in `scripts/context-io.ts`. `scripts/context-io.test.ts` asserts this list equal to that export in both directions, so a stop the code gains and a stop this list names cannot come apart. The bound is the home-directory stop, which holds whatever markers a filesystem carries; the version-control markers say where a repository starts, which is a different question.

- The upward search never inspects the user's home directory, and never inspects any ancestor of it.
- When the home directory cannot be determined, the upward search does not run at all.
- The upward search ends at the first ancestor carrying a version-control marker, and that ancestor's own configuration wins over one nested below it.
