...
// SAFE-02). Nothing semantic changed: the same approval env-var NAME, the same DEPLOY
// pattern set, the same SELF_APPROVE detection, the same fail-closed stdin parse, and the
// same exit-0-+-JSON-deny block mechanism. Only TypeScript types were added — see D-10 and
// RESEARCH Pitfall 2 (a careless "cleanup" silently disables the deny path; translate, never
// redesign). The committed compiled output is hooks/guard.js, which the host hook runs.
//
