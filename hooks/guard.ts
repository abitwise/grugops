...
const CHECKPOINT_PATTERNS: readonly { readonly id: Checkpoint; readonly patterns: RegExp[] }[] = [
 ...
 */
function spellUnreadable(words: readonly string[]): string {
  const MAX_WORDS = 3;
  const MAX_CHARS = 48;
  const spell = (w: string): string => {
...
for (const group of CHECKPOINT_PATTERNS) {
  ...
  if (matrixUnread) {
    deny(
      `Blocked (fail-closed): this command matches the "${group.id}" checkpoint and the grugops ` +
  ...
  if (r.unauthorizedLowering) {
    deny(
      `Blocked: the checkpoint "${r.id}" is declared \`${r.declared}\` in factory.config.json, but ` +
  ...
  const escape =
    modelled.untokenizable && modelled.failClosed.has(group.id)
