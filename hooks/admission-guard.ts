...
function effectiveCommandWord(token: string): string {
...
function stripQuotedRuns(token: string): string {
  let out = "";
  let i = 0;
...
interface LiveToken {
  ...
  start: number; // byte offset in the original command string where this token began
}
...
function matchCommandSubstitution(cmd: string, open: number): number {
  let depth = 0;
  let i = open;
  const n = cmd.length;
  while (i < n) {
    ...
    if (c === "$" && cmd[i + 1] === "(") { depth += 1; i += 2; continue; }
...
function segmentHasAdmitShape(tokens: LiveToken[], from: number): boolean {
  let sawScript = false;
...
function isEnvIndirectedWord(value: string): boolean {
...
function rawTextHasAdmitShape(text: string): boolean {
...
function rawSegmentText(cmd: string, tokens: LiveToken[], from: number): string {
...
function noteFileInSegment(tokens: LiveToken[], from: number): string | null {
  let sawScript = false;
  for (let j = from; j < tokens.length && (j === from || !tokens[j].startsSegment); j++) {
    const v = tokens[j].value;
    if (v.includes(ADMIT_SCRIPT)) sawScript = true;
...
function tokenRawSlice(cmd: string, tokens: LiveToken[], j: number): string {
...
function tokenIsFinalLiteral(tokens: LiveToken[], j: number, cmd: string): boolean {
  const tok = tokens[j];
  if (tok.value === DYNAMIC_COMMAND_WORD) return false;
  if (!FINAL_LITERAL_VALUE.test(tok.value)) return false;
  const raw = tokenRawSlice(cmd, tokens, j);
  return !raw.includes("$") && !raw.includes("`");
}

// The structural disposition of one segment's admit shape: a resolvable admit (with its note file), an
// UNRESOLVABLE admit-or-script (fail CLOSED), or none. The detector reads de-quoted token values + the
// token start offsets liveTokens ALREADY emits — no second walk, no tokenizer change.
type AdmitDisposition =
  | { kind: "admit"; noteFile: string | null }
  | { kind: "unresolvable" }
  | { kind: "none" };
...
function launcherDisposition(
  tokens: LiveToken[],
  launcherIdx: number,
  cmd: string,
  behindFeeder: boolean,
): AdmitDisposition {
  ...
  for (let j = launcherIdx + 1; j < tokens.length && !tokens[j].startsSegment; j++) {
    if (tokenIsFinalLiteral(tokens, j, cmd) && tokens[j].value.includes(ADMIT_SCRIPT)) {
      const verbIdx = j + 1;
      if (verbIdx >= tokens.length || tokens[verbIdx].startsSegment) return { kind: "none" };
      if (!tokenIsFinalLiteral(tokens, verbIdx, cmd)) return { kind: "unresolvable" };
  ...
  if (word === "npx" || JS_RUNNERS.has(word)) {
    ...
    for (let j = launcherIdx + 1; j < tokens.length && !tokens[j].startsSegment; j++) {
      if (!tokenIsFinalLiteral(tokens, j, cmd)) return { kind: "unresolvable" };
    }
  ...
  if (behindFeeder && rawTextHasAdmitShape(cmd)) return { kind: "unresolvable" };
  return { kind: "none" };
}
...
// A single LIVE admit segment the unified authority resolved: its note file (null when unlocatable),
// and whether its command word was statically unresolvable (a dynamic-evaluation / indirection /
// command-substitution word the resolver cannot see through → fail CLOSED).
interface AdmitSegment {
...
function liveAdmitSegments(cmd: string): AdmitSegment[] {
  const tokens = liveTokens(cmd);
  const segments: AdmitSegment[] = [];
  for (let i = 0; i < tokens.length; i++) {
    ...
    while (cmdIdx < tokens.length) {
      if (cmdIdx > i && tokens[cmdIdx].startsSegment) break; // stay within this segment's leading run
      ...
      if (!isPrefix && !isModifierFlag) break;
      if (isPrefix && STDIN_ARGV_FEEDERS.has(effectiveCommandWord(v))) behindFeeder = true;
      cmdIdx += 1;
    }
...
function commandHasLiveAdmitShape(cmd: string): boolean {
...
function classifySegmentOrDeny(seg: AdmitSegment): void {
  ...
  if (seg.unresolvable) {
    if (!approved) {
      deny(
        `Admission blocked (fail-closed): an admission was launched behind a command word the hook ` +
  ...
  try {
    ...
    if (hasDuplicateBy || hasMalformedBy) {
      deny(
        `Admission blocked (fail-closed): the note "${noteFile}" has a structurally invalid authoring ` +
