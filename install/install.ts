...
// any model costs.
const TARGET_BANNER =
  "<!-- GENERATED — do not hand-edit. This file is written by the grugops installer. To refresh " +
...
const adapterDestHazard = (dest: string): string | null => {
  ...
  try {
    ...
    return (
      `${TARGET} could not be resolved to a real path, so this run cannot say whether ${dest} is ` +
  ...
  while (!existsSync(probe)) {
    ...
    if (up === probe) break; // filesystem root reached — resolve it and let the compare decide.
    probe = up;
  }
  ...
  try {
    ...
    return (
      `the directory ${probe} that would hold ${dest} could not be resolved to a real path, so ` +
  ...
  if (resolvedDir !== resolvedTarget && !resolvedDir.startsWith(resolvedTarget + sep)) {
    return (
      `${dest} resolves to ${join(resolvedDir, basename(dest))}, which is OUTSIDE the target ` +
    ...
    if (docNames.length > 0) {
      renderAdaptersInMirror((res) => {
        ...
        if (unlisted.length > 0) {
          docWarn(
            `adapter staleness: the fresh render produced ${unlisted.length} member(s) that the ` +
        ...
        if (unreadable.length > 0) {
          docWarn(
            `adapter staleness: ${unreadable.length} member(s) of the target's adapter set are ` +
...
function renderAdaptersInMirror(use: (result: MirrorResult) => void): void {
  ...
  try {
    ...
    use({
      ok: false,
      reason:
        `the adapter render could not start: no temporary directory could be created under ` +
  ...
  try {
    ...
    try {
      ...
      use({
        ok: false,
        reason:
          `the adapter render could not start: the render mirror could not be built from the kit ` +
    ...
    if (existsSync(targetConfig)) {
      try {
        ...
        use({
          ok: false,
          reason:
            `the adapter render could not start: the model configuration at ${targetConfig} exists ` +
...
// generator banner lines it recognised on the way.
...
const RETIRED_CONFIG_KEYS: ReadonlyArray<readonly [string, string]> = [
  [
    "autonomy",
...
function reportRetiredConfigKeys(): void {
  ...
  try {
    ...
    report(
      "skipped",
      `retired-key check (${cfgPath} did not parse — the check was NOT performed, so this run ` +
  ...
  if (parsed === null || typeof parsed !== "object" || Array.isArray(parsed)) {
    report(
      "skipped",
      `retired-key check (${cfgPath} is not a JSON object — the check was NOT performed)`,
    );
    return;
  }
