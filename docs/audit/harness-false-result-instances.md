...
| 13 | `31-30` (this plan), `scripts/check-platform-shapes.ts` | the platform shape corpus, GOV-02 ledger position | the position was staged with no governing configuration, so `appendNote` wrote the note and never reached the ledger append at all — the row would have reported "not refused" for a position that was never driven | the CONTROL and the non-regular-file shapes returned the SAME verdict at that position, which is impossible if the position is being driven. **CORRECTED 2026-09-11 by plan `31-37`, forced by `WR-34`:** this cell used to end *"the fixture now writes `context: { human_admission: "high-severity", audit_retention: "retained" }` before the shape is planted"*. That remedy was REVERTED. Reaching `appendAuditLedger` needs a governing configuration, so the fixture had to spell a config path and both dial names — which made `check-platform-shapes` a governance-dial site and turned three derived cases in `scripts/context-io.test.ts` red, `AUTO-06` among them. The guard was right, so **the position was DROPPED**, not re-fixtured, and the module's driven-row count fell from 17 to 13. What covers it instead: `scripts/context-io.test.ts` drives a FIFO and a directory at the GOV-02 ledger path on every CI leg. The restore criterion is carried in `deferred-items.md` (`status: open`, owner unassigned): publish the two governance dial key names from the one authority, then a probe composes the fixture without spelling either literal. The SIGNAL that caught this instance is unchanged and is re-producible again: `31-36` gave the control a reachable red path through `GRUGOPS_PLATFORM_SHAPES_STALE_CONTROL` | read off this list |
...
| 15 | `deferred-items.md` (raised by plan `31-38`, round 8) | the first `admitAndAppend` governance-dial probe | its dial was written at the WRONG NESTING LEVEL — the configuration key sat at the config root instead of inside the `context` object the reader looks in — so the probe never moved the dial at all, and every value it drove produced one indistinguishable outcome that reads exactly like a closure of `CR-22` position 3 and is not one | all three dial values were driven and their outcomes compared: each returned `admission REFUSED (W3)` with zero files on disk, which cannot happen if the dial is being moved. `Every dial value then produced the identical` outcome is the record's own wording for the signal, and the nesting the reader actually requires is `{ "context": { "human_admission": … } }` | the record claimed **15**; this table offers **15** — they AGREE, and both are printed because this column exists to show whether they do |
| 16 | `deferred-items.md` (raised by plan `31-38`, round 8) | the full-suite sweep for probe roots left inside the tree | the residue it read was the SUBJECT's, when it was the measuring session's own AST probe roots — so a RED that named the tree was a false result about what the tree held, rather than a defect in what the tree does | the failing step named `POINT 7: no probe root is left under '.temp'` while the subject was unchanged; the residue was located at `.temp/31-38-probe/`, removed, and the suite re-run clean — which is also why the record requires that sweep to be taken AFTER the suite, printing the mid-flight readings that prove it. `The suite was measuring the measurement.` is the record's own wording | (unnumbered — the record logged it as a second, self-inflicted one without reaching for a number); this table assigns **16** |
...
- **Why row 16 IS a row, written down rather than argued away (plan `31-40`, round 9).** Round 8's
  own record offered the other reading: a harness measuring its own residue is not a false premise
  about the SUBJECT, so it might belong nowhere. The judgement taken here is that it IS a row, and
  the reason is the premise rather than the verdict. Every instance on this list is a harness whose
  own premise was false; this harness's premise was *"the `.temp` tree I am reading belongs to the
  subject"*, and that premise was false. The RED it produced was a true statement about the disk and
  a false statement about the thing under test, which is exactly the distinction rows 3, 9 and 14
  turn on. Recording the classification is the point: the alternative reading is on the record, and
  the next reader can disagree with a written argument rather than with a silence.
- **Rows 15 and 16 were appended by plan `31-40`, ordinals read off this table, not typed.** The
  record that raised row 15 stated **15** and the table offered **15**; the agreement is printed in
  the row's own last column rather than assumed, because a column that only ever shows agreement is
  a column nobody would notice going wrong. No prior record was edited to produce either row.
