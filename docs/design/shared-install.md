...
> Status: **design / decision capture (historical)** — this was the input for the v1.1 install
> phases (7–9), which shipped the two-root model described below. Origin: surfaced by the live CC
> dogfood (DOG-02), 2026-06-06. Decision: **clean split, build as a phase.**
>
> **Tooling note (superseded by the Phase-15 TypeScript pivot, D-13, ratified 2026-06-13):** the
> installer this document describes shipped in v1.1 as a dual POSIX-shell + Node-stdlib pair held
> in byte parity. Phase 15 collapsed that dual installer into a single Node-required TypeScript
> source compiled to a committed `install.js`; the validator and harnesses are likewise TypeScript
> now. The live invocation form today is `node install/install.js` (Node 22+ required).

## Problem (from the dogfood)

Installing grugops into a *new* repo is broken in three ways, all from one root cause:

1. **Wrong target.** The v1.1 installer defaulted `TARGET=$(pwd)`. Run from inside the clone, it installs into the clone. No `--target` flag and no prompt — the only redirect is the obscure `TARGET=/path` env var. → user "had to change folder to run it."
2. **Symlinks back into the clone.** `INSTALL_MODE=symlink` was the default (D-30). Adapters are symlinked into the clone — fragile (break if the clone moves), confusing in the target's git.
3. **The kit never arrives.** The installer *explicitly* never touches `agent-factory/`. It lays down only thin adapters that reference `agent-factory/roles/…`, `agent-factory/config/…`, `agent-factory/workflows/…`, `plans/…` **repo-relative**. Those paths don't exist in the target → the adapters dangle → Claude hunts in the clone.
...
- `install --target <repo>` + interactive prompt ("Install into which repo? [.]").
...
- Preserve the contract: additive, idempotent, `DRY_RUN=1`, reversible; never overwrite/delete user content; never set the deploy-approval env var. The uninstaller is updated for the two-root layout. *(v1.1 shipped this as a dual POSIX + Node installer pair; Phase 15 consolidated it into a single Node TypeScript installer — see the tooling note at the top.)*

## Validator / test impact

- The structure validator (`scripts/validate-agent-factory.js`) assumes a single in-repo tree. It must become two-root aware (kit at `$GRUGOPS_HOME`/`VALIDATE_ROOT`, state in repo), or validate the kit and a target independently.
- The validator harness (the `scripts/validate.test.ts` Vitest suite) + fixtures: update for the split layout.
