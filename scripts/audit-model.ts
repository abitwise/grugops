...
// config cannot carry.
export const CLAIM_STATUSES = ["true", "overstated", "false", "dropped"] as const;
...
/** One residual the guarantees render publishes. */
export interface ResidualRow {
  /** The row's number, continuing the historical table's numbering. */
  readonly num: string;
  readonly item: string;
  /** One of DISPOSITIONS. The cell must be exactly `` `value` `` — nothing else on it. */
  readonly disposition: Disposition;
  /** The phase a `deferred` row names, or `—`. */
  readonly targetPhase: string;
  /** The register's own words. This is what the render quotes. Never blank. */
  readonly reason: string;
  /** 1-based line of the row in the register, for a refusal a reader can go to. */
  readonly line: number;
}
 ...
 */
export function readResidualAdditions(root: string = DEFAULT_ROOT): readonly ResidualRow[] {
  const abs = join(root, RESIDUAL_PATH);
  if (!existsSync(abs)) {
    throw new Error(
      `audit-model: refusing to parse ${RESIDUAL_PATH} — the residual register does not exist at ` +
  ...
  try {
    ...
    refuse(RESIDUAL_PATH, `it could not be read (${(e as Error).message})`);
  }
  ...
  if (table === null) {
    refuse(
      RESIDUAL_PATH,
      `it carries no \`${RESIDUAL_ADDITIONS_HEADING}\` heading. The additions table is located by ` +
  ...
  if (table.length === 0) {
    refuse(
      RESIDUAL_PATH,
      `\`${RESIDUAL_ADDITIONS_HEADING}\` carries no rows at all, not even a header row`,
    );
  }
  ...
  if (body.length === 0) {
    refuse(
      RESIDUAL_PATH,
      `the Phase 30 additions table carries zero residual rows. A vacuous residual list satisfies ` +
  ...
  const rows: ResidualRow[] = body.map((tl) => {
    const [num, item, disposition, targetPhase, reason] = tl.cells;
    const m = RESIDUAL_BACKTICKED_RE.exec(disposition);
    if (m === null) {
      refuse(
        RESIDUAL_PATH,
        `the additions row at line ${tl.line} carries \`Disposition\` ${JSON.stringify(disposition)}, ` +
    ...
    if (!DISPOSITIONS.includes(m[1] as Disposition)) {
      refuse(
        RESIDUAL_PATH,
        `the additions row at line ${tl.line} carries disposition "${m[1]}", which is outside the ` +
    ...
    if (isBlank(reason)) {
      refuse(
        RESIDUAL_PATH,
        `the additions row at line ${tl.line} carries no \`Reason / owner\`. The reason IS what ` +
    ...
    if (m[1] === "deferred" && isBlank(targetPhase)) {
      refuse(
        RESIDUAL_PATH,
        `the additions row at line ${tl.line} is \`deferred\` with no \`Target phase\`. A deferral ` +
    ...
    return {
      num,
      item,
      disposition: m[1] as Disposition,
      targetPhase,
      reason,
      line: tl.line,
    };
  ...
  if (dupNums.length > 0) {
    refuse(
      RESIDUAL_PATH,
      `the Phase 30 additions table carries duplicate row number(s): ${dupNums.join(", ")}. The ` +
