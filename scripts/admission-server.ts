...
export const PROPOSE_NOTE_TOOL = {
  name: "propose_note",
  description:
    "Admit a verified context note through the grugops governance gate and persist it via the single " +
  ...
  inputSchema: {
    ...
    properties: {
      ...
      by: {
        type: "string",
        description:
          "The authoring role. Severity is derived from this (security-nfr / architect-design / " +
      ...
      verified_by: {
        type: "string",
        description:
          "Empty, a §14-gate#<id> stamp (gate-verified), or a human:NAME stamp (a human disposition " +
...
// writer). The server forks no writer, parses no shell string, and reads no approval env. Returns an
// MCP tool result: on success the new note id; on refusal an isError result naming the fault.
...
export function handleProposeNote(args: Record<string, unknown>): ToolResult {
  ...
  const note: NoteInput = {
    ...
    refs: Array.isArray(args.refs) ? (args.refs as unknown[]).map((r) => String(r)) : [],
    supersedes:
      args.supersedes === undefined || args.supersedes === null ? null : String(args.supersedes),
  };
  ...
  try {
    ...
    return { content: [{ type: "text", text: `admission FAIL: ${(e as Error).message}` }], isError: true };
  }
}

// ── JSON-RPC dispatch — the minimal MCP stdio handshake + tools/call routing. ────────────────────────
...
function reply(id: JsonRpcId, result: unknown): string {
  return JSON.stringify({ jsonrpc: "2.0", id, result });
}
function replyError(id: JsonRpcId, code: number, message: string): string {
...
export function dispatch(msg: JsonRpcMessage): string | null {
  ...
  switch (method) {
    case "initialize": {
      const clientProto = (params?.protocolVersion as string | undefined) ?? DEFAULT_PROTOCOL_VERSION;
