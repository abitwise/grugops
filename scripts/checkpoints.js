...
function runIsLiteral(r) {
    if (r.quote === "single")
        return true;
    if (r.quote === "double")
        return !DOUBLE_QUOTE_ACTIVE_RE.test(r.text); // C1: a double-quoted run is literal only when nothing in it is active
 ...
 */
export function canonicalWordValue(runs) {
    if (runs.length === 1) {
        const only = runs[0];
