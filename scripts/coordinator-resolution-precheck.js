...
const EVIDENCE_FILE = ".planning/phases/27-spawn-correctness-kit-set-authority/27-SPAWN-03-RUNTIME-EVIDENCE.md";
...
function parseArgs(argv) {
    const opts = { keep: false, inspectTarget: null };
    for (let i = 0; i < argv.length; i++) {
        const a = argv[i];
        if (a === "--keep-scratch-target")
            opts.keep = true;
        else if (a === "--inspect-target")
            opts.inspectTarget = argv[++i] ?? "";
...
function observePlatformVersion() {
    ...
    const floor = parseTriple(PLATFORM_FLOOR);
    const low = parseTriple(KNOWN_BAD_LOW);
    const high = parseTriple(KNOWN_BAD_HIGH);
    if (cmp(got, low) >= 0 && cmp(got, high) <= 0) {
        fail(`the installed platform is ${reported}, inside the known-bad window ${KNOWN_BAD_LOW}-${KNOWN_BAD_HIGH} whose default spawn depth is 1. An observation taken here would describe that window, not the advertised floor. Upgrade to ${PLATFORM_FLOOR} or later before observing.`);
    }
    if (cmp(got, floor) < 0) {
        fail(`the installed platform is ${reported}, below the advertised floor ${PLATFORM_FLOOR}. Upgrade before observing — this kit's depth and scheduling claims are written against ${PLATFORM_FLOOR} and later.`);
    }
...
function runScratchInstall() {
    ...
    if (detail.includes("install INCOMPLETE")) {
        fail(`the scratch install printed the INCOMPLETE banner while exiting ${String(r.status)} — the banner and the exit status disagree, so the install refused a whole class. Installer output follows:\n${detail}`);
    }
    return {
        target,
        home,
...
function readMaterializedKit(adapterPath) {
...
function observeTarget(targetRoot) {
    let rels;
    try {
        rels = listAgentAdapters(targetRoot);
    }
    catch (e) {
        return fail(`the installed adapter set could not be read from ${join(targetRoot, ADAPTER_DIR)} — ${e instanceof Error ? e.message : String(e)}`);
    }
    const out = [
    ...
    if (coordinators.length > 1) {
        fail(`${coordinators.length} installed adapters carry the \`coordinator: true\` marker (${coordinators.map((c) => c.rel).join(", ")}) — exactly one is expected, and an observation taken against an ambiguous set would not say which one resolved.`);
    }
    ...
    // name-to-filename.
    const installedNames = new Set(adapters.map((a) => a.name).filter((n) => n !== null && n !== ""));
    const unresolved = granted.filter((n) => !installedNames.has(n));
    if (unresolved.length > 0) {
        fail(`${unresolved.length} of the coordinator's ${granted.length} granted name(s) resolve to no installed adapter file: ${unresolved.join(", ")}. A routed subtask naming one of those would find nothing to run.`);
    }
    out.push(`granted names resolving to an installed adapter file: ${granted.length} of ${granted.length}.`);
    const kit = readMaterializedKit(join(targetRoot, ADAPTER_DIR, coordinator.rel));
    if (kit === "") {
        fail(`the coordinator adapter ${coordinator.rel} carries no materialized \`KIT=\` line inside its \`grugops:materialized-kit\` sentinel block — the installed adapter cannot resolve the kit root without falling back to the self-heal path.`);
    }
    out.push(`materialized kit path: ${kit}`);
    if (!existsSync(kit)) {
        fail(`the coordinator adapter's materialized kit path ${kit} does not exist — the adapter points at a kit root that is not on disk.`);
    }
    const roleFile = join(kit, COORDINATOR_ROLE_FILE);
    if (!existsSync(roleFile)) {
        fail(`the coordinator's role file is missing at ${roleFile} — the kit path resolves but does not carry the role the coordinator adapter reads.`);
    }
    out.push(`materialized kit path resolves: yes — the directory exists and carries ${COORDINATOR_ROLE_FILE}.`);
    return out;
}
// ---------------------------------------------------------------------------
...
const QUOTED_TEST = "From an installed target repository, run `claude --agent grugops-orchestrator`; observe the session startup header, then ask for work that routes to a specialist (e.g., \"map this repo\") and observe whether a distinct role agent resolves and runs.";
const QUOTED_EXPECTED = "Startup header names `@grugops-orchestrator`; a role agent (not the coordinator itself) executes the routed subtask.";
const QUOTED_WHY = "The Claude Code runtime is the system under test; the startup header is an interactive TUI element with no in-repo command able to observe it.";
function printHumanRemainder(installed, keep) {
    const cdLine = keep
        ? `cd ${installed.target}`
        : "cd <an installed target repository>   # re-run with --keep-scratch-target to have one named here";
    console.log("");
    console.log("-- HUMAN-ONLY REMAINDER — NOT PERFORMED BY THIS COMMAND ------------------------");
    console.log("");
    console.log("The two steps below are the runtime half of this check. No command in this repository");
    console.log("can perform them, and this one does not attempt to. Quoted from 27-VERIFICATION.md:");
    console.log("");
    ...
    console.log("  Then confirm the session startup header names @grugops-orchestrator.");
    console.log("");
    console.log("Step 2 — NOT PERFORMED by this command:");
    console.log('    In that session, ask for work that routes to one specialist (for example: "map this repo").');
    console.log("  Then confirm a distinct role agent resolves and runs, rather than the coordinator");
    console.log("  working the task inline.");
    console.log("");
    console.log(`Record what you saw in ${EVIDENCE_FILE}.`);
    console.log("Paste this block and fill each slot from the observation. An empty slot means the");
    ...
    for (const slot of BACKSTOP_SLOTS)
        console.log(`    - ${slot}:`);
    console.log("");
}
// ---------------------------------------------------------------------------
...
function main(argv) {
    ...
    console.log("Discharges the OBSERVABLE preconditions only. It starts no model session and spends no tokens.");
    console.log("");
    console.log(observePlatformVersion());
    let installed;
    if (opts.inspectTarget !== null) {
        if (!existsSync(opts.inspectTarget)) {
            fail(`the target supplied to --inspect-target does not exist: ${opts.inspectTarget}`);
        }
        installed = {
    ...
    for (const line of observeTarget(installed.target))
        console.log(line);
    ...
    if (opts.keep && opts.inspectTarget === null) {
        ...
        console.log("Remove both when you are done; nothing else on this machine was written.");
        console.log("");
    }
    console.log(`${PRECONDITIONS_HOLD} every observable precondition of the coordinator-resolution check is satisfied on this tree. The two runtime steps above are NOT PERFORMED by this command, and SPAWN-03's runtime half stays unverified until a human observes it and records the observation in ${EVIDENCE_FILE}.`);
    return 0;
}
let code = 1;
try {
    ...
    if (e instanceof PreconditionFailure) {
        ...
        console.log(`${PRECONDITION_FAILED} the precheck stopped on an unexpected error — ${e instanceof Error ? e.message : String(e)}`);
    }
    console.log("No observation was taken. Fix the precondition named above and re-run before starting the interactive step.");
    code = 1;
}
finally {
