...
export const imperativeLexiconFails = () => FAILS;
...
// ---------------------------------------------------------------------------
export const GOVERNED_CORPUS_EXCLUDED_LOCATIONS = [
    {
        location: "agent-factory/packaging/",
        label: "packaging documents",
        reason: "adapter, subagent-frontmatter and slash-command templates describe how the kit is PACKAGED " +
    ...
    },
    {
        ...
        reason: "reference documentation for every dial in factory.config.json. It is documentation about " +
    ...
    },
    {
        ...
        reason: "the roles are governed by guard_voice, guard_caveman_voice and guard_role_clause_uniqueness " +
...
function flatMarkdown(dir) {
    const a = abs(dir);
    if (!existsSync(a)) {
        DERIVATION_REFUSALS.push(`the governed-corpus directory ${dir} does not exist at ${a} — refusing to report a verdict ` +
...
function workflowMembers() {
    try {
        ...
        DERIVATION_REFUSALS.push(`the workflow part could not be derived through kit-model's listWorkflows() — ` +
            `${e.message}`);
        return [];
    }
}
...
export const GENERATED_EXEMPT = CANDIDATE_PARTS.flatMap((p) => [...p.members].filter(isGenerated));
/**
 ...
 */
export const GOVERNED_CORPUS_PARTS = CANDIDATE_PARTS.map((p) => ({
    name: p.name,
    members: p.members.filter((m) => !GENERATED_EXEMPT.includes(m)),
}));
/** The concatenation, in part order. */
export function governedCorpus() {
...
// adopts.
// ---------------------------------------------------------------------------
export const APPROVED_STEP_VERBS = [
    "Append",
    "Apply",
    "Assemble",
    "Assess",
...
const APPROVED_STEP_VERBS_LOWER = new Set(APPROVED_STEP_VERBS.map((v) => v.toLowerCase()));
...
const unwrapCodeSpans = (s) => s.replace(/`([^`]*)`/g, "$1");
...
function derivedNamePart(name, derive) {
    try {
        return { name, members: derive() };
    }
    catch (e) {
        DERIVATION_REFUSALS.push(`the "${name}" part of the Technical Names set could not be derived — ${e.message}`);
        return { name, members: [] };
    }
}
function configKeys() {
    ...
    if (!existsSync(a))
        throw new Error(`${CONFIG_JSON} does not exist at ${a}`);
...
function tableFirstCellsUnderHeading(rel, heading) {
    const a = abs(rel);
    if (!existsSync(a))
        throw new Error(`${rel} does not exist at ${a}`);
...
function boardColumns() {
    const a = abs(BOARD);
    if (!existsSync(a))
        throw new Error(`${BOARD} does not exist at ${a}`);
    const text = readFileSync(a, "utf8");
    const flags = fencedLineFlags(text);
    const lines = text.split("\n");
...
export const TECHNICAL_NAME_PARTS = [
    ...
    derivedNamePart("noteKinds", () => tableFirstCellsUnderHeading(NOTE_CONTRACT, NOTE_KINDS_HEADING)),
...
export function technicalNames() {
    const seen = new Set();
...
export const TECHNICAL_NAMES = technicalNames();
/**
...
export function countWords(sentence) {
    ...
    for (const name of TECHNICAL_NAMES) {
        if (name.indexOf(" ") === -1)
            continue; // a one-word name already counts as one word
        while (t.includes(name))
            t = t.replace(name, "TECHNICALNAME");
    }
 ...
 */
function tallyStepsHeadings(file, text, flags) {
    let any = 0;
    let decided = 0;
    let undecided = 0;
    ...
    for (let i = 0; i < lines.length; i++) {
        if (flags[i])
            continue;
        const raw = lines[i];
        if (STEPS_HEADING_ANY_LEVEL.test(raw))
            any += 1;
        if (STEPS_HEADING.test(raw))
            decided += 1;
...
function deriveElements(corpus) {
    const bullets = [];
    const sentences = [];
    const unreadable = [];
    const stepsFiles = [];
    ...
    for (const file of corpus) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (trimmed.startsWith("<!--"))
                continue; // an HTML comment is not prose a reader reads
 ...
 */
function setRefusal(expectedSide, visitedSide) {
    ...
    const parts = [];
    if (missing.length > 0) {
        parts.push(`${expectedSide.label} but NOT ${visitedSide.label} (${missing.length}): ` +
            `${missing.join(", ")} — ${expectedSide.missingReason}`);
    }
    if (extra.length > 0) {
        parts.push(`${visitedSide.label} but NOT ${expectedSide.label} (${extra.length}): ` +
...
function classifyStep(b) {
    const firstToken = b.text.split(/\s+/)[0] ?? "";
    if (APPROVED_STEP_VERBS.includes(firstToken))
        return null;
    let kind;
...
const STEP_REMEDY = {
    "bold-label": "delete the bold label and start with the verb it was decorating — `**Run the gate** in order: …` becomes `Run the gate in order: …`",
    "actor-subject": "the first token is a project Technical Name used as a SUBJECT. Re-narrate the step as an instruction to the agent that performs it; name the actor later in the sentence if it is load-bearing",
    "determiner-subject": "the bullet describes what happens rather than instructing. Re-narrate it as an instruction beginning with an approved verb",
    "conditional-clause": "move the condition after the instruction, or state it as a `## Stop conditions` bullet, which this predicate deliberately does not govern",
    "not-an-approved-verb": "begin the bullet with a verb from APPROVED_STEP_VERBS. Do NOT add this token to that list unless it is a verb a grugops procedural step already uses in bare imperative position",
};
function renderStepFinding(f) {
    return (`        ${f.file}:${f.line} — ${f.rule} [${f.kind}] first token "${f.token}" is not an ` +
...
const STEPS_SECTION_REMEDY = "Write the section's procedure as list items, or move the explanatory paragraphs under a heading that is not `## Steps`.";
const STEPS_ANCHOR_RULE_ID = "WP-04";
const STEPS_ANCHOR_RULE = "A bullet under a `## Steps` heading is procedural.";
const FORM_REMEDY = {
    ...
    "procedural-sentence-too-long": `split the step into two steps. A procedural sentence is bounded at 20 words (WP-02), and ` +
        `${STEPS_ANCHOR_RULE_ID} is what makes that bound the one applied here: ${STEPS_ANCHOR_RULE}`,
    "descriptive-sentence-too-long": "split the sentence. A descriptive sentence is bounded at 25 words (WP-03)",
    "modal-in-procedural-step": "delete the modal. The step is the obligation (WP-05); if the action is conditional, state the condition rather than hedging the verb",
    "bare-demonstrative-subject": "name the antecedent. A sentence opening with a bare demonstrative followed by a copula, an auxiliary or a modal has no subject a reader can resolve without the previous sentence (WP-06)",
    "and-slash-or": "write the one meaning intended (WP-07). `and/or` asks the reader to pick",
    "more-than-one-instruction": "split the sentence at the conjunction. One instruction per sentence (WP-08)",
};
function renderFormFinding(f) {
    return (`        ${f.file}:${f.line} — ${f.rule} [${f.kind}] ${f.detail} — ` +
...
function classifySentence(s) {
    ...
    if (s.procedural && APPROVED_STEP_VERBS_LOWER.has(words[0])) {
        for (let i = 1; i < words.length - 1; i++) {
            if (INSTRUCTION_CHAINS.has(words[i]) &&
 ...
 */
export const LEXICON_MEASURED_LABEL = "imperative lexicon — governed file(s) carrying a `## Steps` section";
...
function runAll() {
    ...
    for (const part of GOVERNED_CORPUS_PARTS) {
        if (part.members.length === 0) {
            fail(`the "${part.name}" part of the governed corpus derived ZERO members — refusing to report ` +
    ...
    if (corpus.length !== GOVERNED_CORPUS_COUNT) {
        fail(`the governed corpus derived ${corpus.length} document(s), expected exactly ` +
    ...
    if (GENERATED_EXEMPT.length !== GENERATED_EXEMPT_COUNT) {
        fail(`${GENERATED_EXEMPT.length} candidate document(s) carry the \`${GENERATED_MARKER}\` marker, ` +
    ...
    for (const part of TECHNICAL_NAME_PARTS) {
        if (part.members.length === 0) {
            fail(`the "${part.name}" part of the Technical Names set derived ZERO members — the set is ` +
    ...
    if (TECHNICAL_NAMES.length !== TECHNICAL_NAMES_COUNT) {
        fail(`the Technical Names set derived ${TECHNICAL_NAMES.length} member(s), expected exactly ` +
    ...
    for (const u of elements.unreadable) {
        fail(`${u} is a member of the governed corpus and could not be read — refusing to report a ` +
    ...
    // that — could not fire.
    const bulletFilesVisited = [];
    for (const b of elements.bullets) {
        if (!bulletFilesVisited.includes(b.file))
            bulletFilesVisited.push(b.file);
        const f = classifyStep(b);
        if (f !== null)
            stepFindings.push(f);
    }
    const stepFiles = bulletFilesVisited.length;
    process.stdout.write(`        corpus: ${corpus.length} file(s) in 4 part(s) — ${corpusBreakdown()}; ` +
        ...
        `verbatim-copy claim)\n`);
    process.stdout.write(`        ${elements.bullets.length} \`## Steps\` bullet(s) across ${stepFiles} file(s); ` +
    ...
    process.stdout.write(`        steps headings by ATX level: ${heads.decided} at the decided \`## Steps\` level, ` +
    ...
    if (heads.any !== heads.decided + heads.undecided) {
        fail(`the steps-heading tallies do not reconcile: ${heads.any} at any level, but ` +
    ...
    if (heads.decided !== heads.anchors) {
        fail(`the steps-heading tally counted ${heads.decided} decided-level heading(s) while the ` +
    ...
    if (heads.any === 0) {
        fail(`the steps-heading tally saw ZERO steps headings across ${corpus.length} governed ` +
    ...
    if (heads.undecided > 0) {
        fail(`${heads.undecided} steps heading(s) in the governed corpus sit at an ATX level ` +
    ...
    const stepSetRefusal = setRefusal({
        ...
        missingReason: "each of these files opens a step section and contributed no bullet to the loop, so no " +
            "WP-01 verdict was reported over any step it declares",
    }, {
        label: "contributed a bullet",
        members: bulletFilesVisited,
        extraReason: "each of these files was attributed a step bullet without the heading scan ever seeing a " +
    ...
    if (stepSetRefusal !== null) {
        ...
        fail(`the step-heading file set and the bullet-bearing file set are not equal, so the element ` +
    ...
    const sentenceFilesVisited = [];
    for (const s of elements.sentences) {
        if (!sentenceFilesVisited.includes(s.file))
            sentenceFilesVisited.push(s.file);
        for (const f of classifySentence(s))
            formFindings.push(f);
    }
    const byKind = new Map();
    for (const f of formFindings)
        byKind.set(f.kind, (byKind.get(f.kind) ?? 0) + 1);
    const proceduralSentences = elements.sentences.filter((s) => s.procedural).length;
    process.stdout.write(`        ${elements.sentences.length} sentence(s) — ${proceduralSentences} procedural, ` +
        `${elements.sentences.length - proceduralSentences} descriptive; by finding kind: ` +
        `${[...byKind].map(([k, n]) => `${k} ${n}`).join(", ") || "none"}\n`);
    process.stdout.write(`        passive voice is deliberately NOT checked (D-15) — the kit's own correct prose is ` +
    ...
    const sentenceSetRefusal = setRefusal({
        ...
        members: corpus,
        missingReason: "each of these files is a member of the governed corpus and yielded not one sentence, so " +
            "no WP-02..WP-08 verdict was reported over any prose it carries",
    }, {
        label: "yielded a sentence",
        members: sentenceFilesVisited,
        extraReason: "each of these files produced a sentence without being a member of the derived corpus, so " +
            "the scan reached outside the set it publishes a verdict over",
    });
    if (sentenceSetRefusal !== null) {
        fail(`the governed-corpus file set and the sentence-bearing file set are not equal, so the ` +
    ...
    if (FAILS === 0) {
        ...
        pass(`LANG-02: ${corpus.length} governed document(s) in 4 derived part(s) — ${corpusBreakdown()}; ` +
            `${GENERATED_EXEMPT.length} excluded by the derived \`${GENERATED_MARKER}\` marker; ` +
        ...
        pass(`LANG-01: ${TECHNICAL_NAMES.length} Technical Name(s) DERIVED from the kit, never listed — ` +
