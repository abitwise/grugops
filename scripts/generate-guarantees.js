 ...
 */
export function pointerLine(doc) {
    const depth = doc.split("/").length - 1;
    const target = `${"../".repeat(depth)}${OUT}`;
    return (`Which safety claims still hold on this repository, joined to the live checkpoint matrix: ` +
        `[\`${OUT}\`](${target}).`);
}
    ...
    for (const m of rowText.matchAll(/`([^`\n]+)`/g)) {
        ...
        if (t.startsWith(EXTERNAL_PATH_MARKER)) {
            ...
            if (rest.trim() === "" || rest !== rest.trim())
                out.push(t); // malformed: refused downstream
            continue;
        }
        if (/\s/.test(t))
...
export function renderGuarantees(root = DEFAULT_ROOT) {
    ...
    `Every entry below is quoted from \`${RESIDUAL_PATH}\` § *${RESIDUAL_ADDITIONS_HEADING.replace(/^##\s*/, "")}*.`, "This page does not restate them in its own words: it publishes the register's, so the record and", "the public page cannot come to disagree about what is still open.", "", ...residuals.flatMap((r) => [
        `### ${r.num}. ${r.item}`,
        "",
