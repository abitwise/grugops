...
import { readRegistry, REGISTRY_PATH, SAFETY_FLOORS, } from "./audit-model.js";
 ...
 */
export function declaredSafetyRows(root = DEFAULT_ROOT) {
    let text;
    try {
        text = readFileSync(join(root, REGISTRY_PATH), "utf8");
    }
    catch (e) {
        throw new Error(`generate-guarantees: cannot read the claim registry at ${join(root, REGISTRY_PATH)} — ` +
            `refusing to report a safety-row count that was not read (${e.message})`);
    }
    let n = 0;
    for (const line of text.split("\n")) {
        if (line.replace(/\r$/, "") === SAFETY_KIND_LINE)
            n += 1;
    }
    return n;
}
 ...
 * not cardinality.
 */
export function declaredDroppedRows(root = DEFAULT_ROOT) {
    let text;
    try {
        text = readFileSync(join(root, REGISTRY_PATH), "utf8");
    }
        ...
        throw new Error(`generate-guarantees: cannot read the claim registry at ${join(root, REGISTRY_PATH)} — ` +
            `refusing to report a dropped-row set that was not read (${e.message})`);
    }
    const out = [];
    let current = null;
    for (const raw of text.split("\n")) {
        ...
        if (line === DROPPED_STATUS_LINE)
            out.push(current ?? DROPPED_WITHOUT_HEADING);
    }
    return out;
}
/**
 ...
 */
export function disclosureFor(row) {
    const lowered = row.floors
        .filter((f) => f.lowered)
        .slice()
        .sort((a, b) => (a.id < b.id ? -1 : a.id > b.id ? 1 : 0));
    if (lowered.length === 0) {
        throw new Error(`generate-guarantees: refusing to generate a disclosure for ${row.claimId} — none of the ` +
 ...
 */
export function dropConsistencyRefusals(rows) {
    const refusals = [];
    for (const row of rows) {
        const dropped = row.status === "dropped";
        if (dropped && !row.lowered) {
            refusals.push(`${row.claimId} is recorded \`status: dropped\` in ${REGISTRY_PATH}, but every floor it ` +
        ...
        if (!dropped && row.lowered) {
            refusals.push(`${row.claimId} rests on lowered floor(s) ` +
                `${row.floors
                    .filter((f) => f.lowered)
 ...
 */
export function renderGuarantees(root = DEFAULT_ROOT) {
    const rows = guaranteesJoin(root);
    const declared = declaredSafetyRows(root);
    if (declared === 0) {
        throw new Error(`generate-guarantees: ${REGISTRY_PATH} declares no \`kind: safety\` row — refusing to render ` +
    ...
    if (rows.length !== declared) {
        throw new Error(`generate-guarantees: the join produced ${rows.length} safety row(s) where ${REGISTRY_PATH} ` +
    ...
    if (onlyInBytes.length > 0 || onlyInJoin.length > 0) {
        throw new Error(`generate-guarantees: the ${droppedFromJoin.length} dropped row(s) the join produced ` +
    ...
    if (inconsistent.length > 0) {
        throw new Error(`generate-guarantees: the registry's dropped rows and the live checkpoint matrix disagree in ` +
    ...
    lines.push("## Which public sentences rest on which floor", "", "| claim | file | measured status | floors, and where each is held | standing |", "|---|---|---|---|---|", ...rows.map((r) => {
        ...
        const standing = r.status === "dropped"
    ...
    }), "", "## The residual this page does not close", "", "**Lowering a safety floor takes two keys, and the second one is a session environment variable**", "**a human sets by name.** The claim this page makes about that variable is the one that holds,", "stated at its true tier rather than as an absolute:", "", "- It is **un-forgeable from inside a tool call**. An agent's own inline `export VAR=...` lands", "  in the child environment it spawns, which the hook process never inherits; a human's session", "  export does reach the hook.", "- It is **reachable by an agent that can write the host's settings files**. A settings-file", "  `env` entry in `.claude/settings.json` or `.claude/settings.local.json` is reapplied to a live", "  session and is read by the next hook subprocess as if a human had exported it. The hook cannot", "  tell the two apart, because the value is the same string either way.", "- The narrowing measure is a `permissions.deny` recommendation over those settings files plus a", "  companion write-matcher guard. It **narrows** the vector; it does not close it, and this page", "  does not say that it does. An agent that can write arbitrary project files can reach the grant", "  — the same irreducible class the residual register records as the same-uid, no-hook,", "  direct-filesystem forgery residual, disposition `accepted`.", "- The grant is **session-scoped under a name, not per-action consent**. One export authorizes", "  every subsequent action the lowered floor admits, for the life of that session. A reader who", "  takes it as an approval of one act is reading it more narrowly than it is written.", "", "`UNKNOWN - verify`: whether a host tier exists on which the settings-file vector is closed", "rather than narrowed. Nothing in this repository measures that today, and this page does not", "assert it.", "");
    return `${lines.join("\n")}`;
}
// ── Entry point ──────────────────────────────────────────────────────────────────────────────
