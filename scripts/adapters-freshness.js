...
import { inheritForEveryStem, mirroredResolvedPresetLine, resolvedAssignmentsIn, resolvedPresetsIn, } from "./model-tiers.js";
...
if (announcedPreset !== ZERO_CONFIG_PRESET) {
    die(`Adapter freshness check FAILED: the mirrored regeneration resolved the model preset as "${announcedPreset}", and this gate requires "${ZERO_CONFIG_PRESET}".\n` +
        `${WHY_ZERO_CONFIG}\n` +
...
if (ZERO_CONFIG_ALIASES.length !== 1) {
    die(`Adapter freshness check FAILED: the zero-config alias set derived from the degrading-policy authority holds ${ZERO_CONFIG_ALIASES.length} member(s) rather than exactly one, so this gate has no single alias to require. This is a defect in scripts/model-tiers.ts, not in the adapters.`);
}
const assignments = resolvedAssignmentsIn(r.stdout ?? "");
if (assignments.length === 0) {
    die(`Adapter freshness check FAILED: the mirrored regeneration printed NO resolved-assignment line, so this gate can see which PRESET the run was handed but not what the run RESOLVED. Announcing the preset alone is the half-blind pin finding CR-01 named.\n` +
...
if (assignments.length > 1) {
    die(`Adapter freshness check FAILED: the mirrored regeneration printed ${assignments.length} resolved-assignment lines — an ambiguous answer, refused rather than resolved by taking one of them. This gate requires exactly one.\n${WHY_ZERO_CONFIG}`);
}
const assignment = assignments[0];
if (!assignment.ok) {
    die(`Adapter freshness check FAILED: the mirrored regeneration's resolved-assignment line could not be read.\n${assignment.reason}\n${WHY_ZERO_CONFIG}`);
}
if (assignment.value.overrides !== 0) {
    die(`Adapter freshness check FAILED: the mirrored regeneration applied ${assignment.value.overrides} per-role model override(s), and this gate requires 0.\n` +
        `${WHY_ZERO_CONFIG}\n` +
...
if (announcedAliases.length !== ZERO_CONFIG_ALIASES.length ||
    announcedAliases.some((a, i) => a !== ZERO_CONFIG_ALIASES[i])) {
    die(`Adapter freshness check FAILED: the mirrored regeneration emitted the model alias(es) ${announcedAliases.map((a) => `"${a}"`).join(", ")}, and this gate requires exactly ${ZERO_CONFIG_ALIASES.map((a) => `"${a}"`).join(", ")}.\n` +
        `${WHY_ZERO_CONFIG}\n` +
...
const listAdapters = (root, what) => {
    try {
        return listAgentAdapters(root);
    }
    catch (e) {
        die(`Adapter freshness check FAILED: cannot read the ${what} adapter directory ${join(root, ADAPTER_DIR)} — ${e instanceof Error ? e.message : String(e)}\nRun \`${REGEN_CMD}\` and commit the result.`);
    }
};
