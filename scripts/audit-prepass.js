...
const REFERENCE_RE = new RegExp("(?:[A-Za-z0-9._-]+/)+[A-Za-z0-9._-]+\\.(?:md|ts|js|json|ya?ml)" +
    "|(?<![A-Za-z0-9._/-])[A-Za-z0-9._-]+\\.md", "g");
export const PREPASS_PREDICATES = [
    {
        id: "retired-vocabulary",
        label: "Retired grugops vocabulary",
        re: new RegExp(RETIRED_ALTERNATION, "gi"),
        what: "Sees the exact literals scripts/dead-vocabulary.ts declares retired, case-insensitively. " +
    ...
    },
    {
        ...
        re: REFERENCE_RE,
        what: "Sees slashed repo-relative paths and bare `*.md` filenames, and adjudicates each against " +
        ...
        isHit: (candidate, ctx) => {
            if (candidate.includes("/"))
                return !existsSync(join(ctx.root, candidate));
    ...
    },
    {
        ...
        what: "Sees a number adjacent to a kit noun, and anything shaped like a version. It CANNOT tell a " +
    ...
    },
    {
        ...
        what: "Sees the project's standing honesty marker. It CANNOT tell a marker that is still genuinely " +
...
export function buildPrepassContext(root = ROOT) {
    const legal = new Set();
 ...
 */
export function predicateHits(predicate, line, ctx) {
    const re = new RegExp(predicate.re.source, predicate.re.flags);
    const matched = line.match(re) ?? [];
    const decide = predicate.isHit ?? (() => true);
    return matched.filter((c) => decide(c, ctx));
}
export function runPrepass(root = ROOT) {
    const refusals = [];
    const rows = [];
    const perPredicate = new Map(PREPASS_PREDICATES.map((p) => [p.id, 0]));
    let files = [];
    try {
        ...
        refusals.push(`the audit set could not be derived — ${e.message}`);
    ...
    for (const f of files) {
        if (!existsSync(join(root, f))) {
            refusals.push(`${f} is in the audit set but does not exist on disk — refusing to run any predicate. A ` +
    ...
    if (files.length !== AUDIT_SET_COUNT) {
        refusals.push(`the derived audit set holds ${files.length} file(s), expected exactly ${AUDIT_SET_COUNT} ` +
...
function renderEvidence(result, today) {
    ...
    out.push(`- **Predicates:** ${PREPASS_PREDICATES.length} (pinned at ${PREPASS_PREDICATE_COUNT}) — ` +
    ...
    out.push("");
    out.push("Every row below is a mechanical hit a human still has to adjudicate. A row is not a finding " +
    ...
    out.push("");
    out.push("The D-07 rubric asks things no regular expression answers: whether a file describes the " +
    ...
    out.push("|---|---|---|---|");
    for (const p of PREPASS_PREDICATES) {
        out.push(`| \`${p.id}\` | ${p.label} | ${result.perPredicate.get(p.id) ?? 0} | ${p.what} |`);
    }
    out.push("");
    for (const p of PREPASS_PREDICATES) {
...
function main() {
    ...
    process.stdout.write(`audit-prepass: ${result.rows.length} evidence row(s) across ${result.files.length} file(s) ` +
