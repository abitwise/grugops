 ...
 * ---------------------------------------------------------------------------------------------
 */
export const ROLE_DIR_EXEMPT: readonly { readonly name: string; readonly why: string }[] = [
  {
    name: ".gitkeep",
  ...
  },
  {
    ...
    why:
      "the D-02 role-switch protocol. `listRoles` excludes `_`-prefixed files by long-standing " +
...
export const ROLE_DIR_EXEMPT_NAMES: readonly string[] = ROLE_DIR_EXEMPT.map((e) => e.name);

/** Every entry in the roles directory except the named exemptions. */
export function listRoleDirEntries(kitRoot: string = DEFAULT_KIT_ROOT): string[] {
  const dir = join(kitRoot, ROLES_SUBPATH);
 ...
 * ---------------------------------------------------------------------------------------------
 */
export const MARKDOWN_ALIAS_EXTENSIONS: readonly string[] = [
  ".markdown",
  ".mdown",
  ".mdwn",
  ".mkd",
...
export function isCanonicalMarkdownName(name: string): boolean {
  return name.endsWith(MARKDOWN_EXT);
}

/** Does this filename spell markdown WITHOUT being the canonical spelling? */
export function imitatesMarkdownName(name: string): boolean {
  if (isCanonicalMarkdownName(name)) return false;
 ...
 * ---------------------------------------------------------------------------------------------
 */
export const WORKFLOW_DIR_EXEMPT: readonly { readonly name: string; readonly why: string }[] = [
  {
    name: ".gitkeep",
    why:
      "git's own empty-directory placeholder. It is not a document, it carries no prose, and it is " +
...
export const WORKFLOW_DIR_EXEMPT_NAMES: readonly string[] = WORKFLOW_DIR_EXEMPT.map((e) => e.name);
