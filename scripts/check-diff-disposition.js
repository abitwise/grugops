...
export const FROZEN_SOURCES = {
    registryAnchors: {
        ...
        companion: "docs/audit/28-claim-registry.md must change in the SAME commit (Phase 28 / D-25)",
    },
    structuralSections: {
    ...
    positiveGuardLiterals: {
        ...
        companion: "the guard's own source must change in the SAME commit, so the guard and the kit move together",
    },
};
/**
    ...
    for (const c of changed.clauses) {
        ...
        if (source !== null) {
            ...
            if (source === "structuralSections") {
                ...
                const sources = new Set(POSITIVE_GUARD_LITERAL_SITES.map((s) => s.source));
