...
function main(): void {
  ...
  const anchorsPerDoc = new Map<string, { id: string; index: number }[]>();
  ...
  for (const doc of docs) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (ANCHOR_ATTEMPT_RE.test(lines[i])) {
        fail(
          `${doc}:${i + 1} carries a line that reads as a claim anchor but is outside the canonical ` +
  ...
  for (const doc of docs) {
    ...
    for (const claim of claims.filter((c) => c.file === doc).slice().sort((a, b) => (a.id < b.id ? -1 : 1))) {
      ...
      if (at === undefined) continue; // already reported by the bijection above
      ...
      if (at + want.length > lines.length - 1) {
        ...
        fail(
          `${doc}: the anchor for ${claim.id} sits at line ${at + 1} and its claim needs ` +
      ...
      if (!a.equals(b)) {
        fail(
          `${doc}: the text at ${claim.id}'s anchor (line ${at + 2}) is not byte-identical to the ` +
...
// vitest worker.
const isEntry =
  process.argv[1] !== undefined && process.argv[1].endsWith("check-claim-anchors.js");
if (isEntry) main();
