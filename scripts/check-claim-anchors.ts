...
function main(): void {
  ...
  let joinById: Map<string, GuaranteeRow> | null = null;
  try {
    ...
    for (const refusal of dropConsistencyRefusals(joined)) {
      fail(
        `the registry's dropped rows and the live checkpoint matrix disagree — ${refusal}`,
      );
    }
  } catch (e) {
    ...
    if (claims.some((c) => c.status === "dropped")) {
      fail(
        `the registry carries dropped row(s) and the guarantees join could not be built, so their ` +
    ...
    for (const claim of claims.filter((c) => c.file === doc).slice().sort((a, b) => (a.id < b.id ? -1 : 1))) {
      ...
      if (dropped) {
        ...
        if (joined === undefined) {
          if (joinById !== null) {
            fail(
              `${doc}: ${claim.id} is \`status: dropped\` but is not a \`kind: safety\` row, so it ` +
        ...
        try {
          ...
          fail(`${doc}: ${claim.id}'s disclosure text could not be generated — ${(e as Error).message}`);
          continue;
        }
      }
      ...
      if (!block.matches) {
        ...
        const deleted =
          !isBlank(block.verbatimFirstLine) &&
          (isBlank(block.documentFirstLine) || CLAIM_ANCHOR_RE.test(block.documentFirstLine));
        if (deleted) {
          fail(
            `${doc}: the anchored region for ${claim.id} at line ${block.start + 1} has been DELETED, ` +
          ...
          fail(
            `${doc}: the text at ${claim.id}'s anchor (line ${block.start + 1}) is not byte-identical to ` +
  ...
  for (const claim of claims) {
    ...
    if (claim.status === "dropped") continue;
