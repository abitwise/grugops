...
export function deriveFrozenSet(root: string = ROOT): FrozenSet {
  ...
  for (const anchor of FROZEN_SECTION_ANCHORS) {
    ...
    for (const base of files) {
      ...
      if (crs.length > 0) {
        refusals.push(
          `${rel}: carries a carriage return on ${crs.length} line(s) (lines ` +
      ...
      if (occurrences.length > 1) {
        refusals.push(
          `${rel}: carries ${occurrences.length} \`${anchor.heading}\` sections (lines ` +
      ...
      if (imitations.length > 0) {
        refusals.push(
          `${rel}: carries ${imitations.length} heading(s) that RENDER as \`${anchor.heading}\` ` +
  ...
  for (const name of readdirSync(dir).sort()) {
    ...
    for (let i = at + 1; i < end; i++) {
      ...
      if (cells.length !== DISPOSITION_COLUMNS) {
        malformed.push(
          `${DISPOSITION_DIR}/${name}:${i + 1} splits to ${cells.length} cell(s) — ${line.trim()}`,
        );
        continue;
      }
    ...
    if (malformed.length > 0) {
      refusals.push(
        `${malformed.length} line(s) under \`${DISPOSITION_HEADING}\` in ` +
...
function main(): void {
  ...
  try {
    ...
    fail(
      `the derived kit could not be listed, so the watched corpus has nothing to be pinned ` +
        `against and NO verdict is reported over it — ${(e as Error).message}`,
    );
    verdict();
    return;
  }
  if (derivedKit.length !== WATCHED_CORPUS_MIN) {
    fail(
      `the derived kit resolved ${derivedKit.length} file(s), expected exactly ` +
  ...
  if (unwatched.length > 0) {
    corpusRefused = true;
    fail(
      `${unwatched.length} of the ${WATCHED_CORPUS_MIN} derived kit file(s) are NOT in the watched ` +
  ...
  let registryArm: string[] | null = null;
  try {
    registryArm = [
      ...new Set(
        readRegistry(ROOT)
    ...
    corpusRefused = true;
    fail(
      `${REGISTRY_PATH} could not be parsed, so the REGISTRY arm of the safety-surface union could ` +
        `not be derived and NO verdict is reported over the union's residue — ${(e as Error).message}`,
    );
  }
  if (registryArm !== null) {
    ...
    if (registryArm.length === 0) {
      corpusRefused = true;
      fail(
        `the registry arm is EMPTY — ${REGISTRY_PATH} carries ZERO \`kind: safety\` claim(s), so ` +
    ...
    if (unvouched.length > 0) {
      corpusRefused = true;
      fail(
        `${unvouched.length} of the union's markdown residue is UNVOUCHED — ${unvouched.join(", ")}. ` +
    ...
    if (!residue.includes(PROTOCOL_FILE)) {
      corpusRefused = true;
      fail(
        `${PROTOCOL_FILE} is ABSENT from the union's markdown residue. Its register row is the one ` +
    ...
    if (registryResidue.length !== RESIDUE_FROM_REGISTRY_COUNT) {
      corpusRefused = true;
      fail(
        `the registry arm's contribution to the watched corpus is ` +
  ...
  if (changedWithNoClause.length > 0 || clauseWithoutChange.length > 0) {
    fail(
      `the changed-file set and the clause-bearing-file set are not equal, so the element count ` +
