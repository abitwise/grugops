 ...
 */
export const UBUNTU_BLOCK_STEP_PREFIX = UBUNTU_BLOCK_STEP_NAME.slice(0, UBUNTU_BLOCK_STEP_NAME.indexOf(" ("));
 ...
 */
export function ubuntuBlock(ci) {
    const at = ci.indexOf(UBUNTU_BLOCK_STEP_NAME);
    if (at === -1) {
        throw new Error("CI wiring oracle: .github/workflows/ci.yml does not carry the step name " +
 ...
 * equality.
 */
export function ubuntuBlockCommands(ci) {
 ...
 */
export function withAppendedStep(ci, stepName, command) {
