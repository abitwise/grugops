...
export const FLIP_MANIFEST_DEFAULT =
  ".planning/phases/33-live-capture-windows-portability/33-FLIP-MANIFEST.md";

export const MANIFEST_STATUS_VALUES = ["pre-capture", "discharged"] as const;
...
export type LiveSurfacePartName = (typeof LIVE_SURFACE_PART_NAMES)[number];

export const FLIP_KINDS = ["status", "anchor", "marker", "parity-row", "ledger-row"] as const;
export type FlipKind = (typeof FLIP_KINDS)[number];
...
export const flipManifestFails = (): number => FAILS;
...
function readText(rel: string): string {
  try {
    ...
    throw new Error(
      `cannot read ${rel} at ${abs(rel)} — ${(e as Error).message}. The gate cannot derive the ` +
...
function git(args: readonly string[]): string {
  try {
    ...
    throw new Error(
      `\`git ${args.join(" ")}\` failed at ${ROOT} — ${(e as Error).message}. The actual side ` +
...
function gitLsFiles(pathspec: readonly string[]): string[] {
...
// ─────────────────────────────────────────────────────────────────────────────────────────────
// The manifest — parsed from its tables, each identified by its header row.
// ─────────────────────────────────────────────────────────────────────────────────────────────

export interface MarkdownTable {
  header: string[];
  rows: string[][];
  /** 0-based line index of the header row, for refusal messages. */
  at: number;
}

/** Strip one code-span wrapper: `` `x` `` or `` `` x `` `` (the latter for values that carry a backtick). */
export function unwrapCell(cell: string): string {
  const t = cell.trim();
...
export function parseTables(text: string): MarkdownTable[] {
...
export interface Manifest {
  ...
  /** The literal status line as written, so the flip commit can be derived from it with `git log -S`. */
  statusLine: string;
  settings: Map<string, string>;
  parts: { name: LiveSurfacePartName; rule: string; declaredCount: number }[];
  /** Listed members per part, as the manifest lists them. */
  listed: Map<LiveSurfacePartName, string[]>;
  exclusions: { file: string; reason: string }[];
  markers: string[];
  exemptions: { file: string; anchor: string; reason: string }[];
  flip: FlipRow[];
...
function statusLineFor(status: ManifestStatus): string {
...
function findTable(
  ...
  if (hits.length !== 1) {
    throw new Error(
      `the manifest carries ${hits.length} "${name}" table(s) (identified by header row), expected ` +
...
function requireCells(table: MarkdownTable, name: string, width: number): string[][] {
  for (const row of table.rows) {
    if (row.length !== width) {
      throw new Error(
        `a "${name}" row has ${row.length} cell(s), expected ${width}: ${JSON.stringify(row)} — ` +
...
export function parseManifest(text: string): Manifest {
  ...
  if (statusLines.length !== 1) {
    throw new Error(
      `the manifest carries ${statusLines.length} status line(s) beginning \`${STATUS_LABEL}\`, ` +
  ...
  if (status === undefined) {
    throw new Error(
      `the manifest status line reads ${JSON.stringify(statusLine)}; the only declared values are ` +
  ...
  for (const [k, v] of requireCells(settingsT, "settings", 2)) settings.set(k, unwrapCell(v));

  const parts: Manifest["parts"] = [];
  for (const [name, rule, count] of requireCells(partsT, "parts", 3)) {
    const n = unwrapCell(name) as LiveSurfacePartName;
    if (!(LIVE_SURFACE_PART_NAMES as readonly string[]).includes(n)) {
      throw new Error(
        `the manifest declares a part "${n}" this gate has no derivation rule for; the known parts ` +
  ...
  for (const known of LIVE_SURFACE_PART_NAMES) {
    if (!parts.some((p) => p.name === known)) {
      throw new Error(
        `the manifest does not declare the part "${known}" this gate derives; every derived part must be ` +
          `declared with its count so the two sides can be compared — NO verdict is reported`,
      );
    }
  }

  const listed = new Map<LiveSurfacePartName, string[]>();
  for (const p of parts) listed.set(p.name, []);
  for (const [part, member] of requireCells(membersT, "members", 2)) {
    const n = unwrapCell(part) as LiveSurfacePartName;
  ...
  for (const [id, file, kind, locator, current, shape] of requireCells(flipT, "flip class", 6)) {
    const k = unwrapCell(kind) as FlipKind;
    if (!(FLIP_KINDS as readonly string[]).includes(k)) {
  ...
  const corrections: CorrectionRow[] = requireCells(corrT, "correction class", 6).map(
    ([id, file, line, fragment, measured, corrected]) => ({
      id,
      file: unwrapCell(file),
      line,
...
export function declaredSet(m: Manifest): string[] {
...
// ─────────────────────────────────────────────────────────────────────────────────────────────
// The live-surface parts — derived here, listed in the manifest, compared.
// ─────────────────────────────────────────────────────────────────────────────────────────────

export interface DerivedPart {
  name: LiveSurfacePartName;
  members: string[];
}

export interface DerivedParts {
  parts: DerivedPart[];
  /** Every member once, in part order. */
  union: string[];
  overlap: number;
  refusals: string[];
}
...
export function deriveParts(m: Manifest): DerivedParts {
  const refusals: string[] = [...publicDocsDerivationRefusals()];
  ...
  for (const ex of m.exclusions) {
    if (!topLevel.includes(ex.file)) {
      refusals.push(
        `the manifest excludes "${ex.file}" from planningLedgers, but that path is not a tracked ` +
...
function parityTable(m: Manifest): MarkdownTable {
  const file = m.settings.get("parity table file") as string;
  const headerCell = m.settings.get("parity table header cell") as string;
  const hits = parseTables(readText(file)).filter((t) => t.header[0] === headerCell);
  if (hits.length !== 1) {
    throw new Error(
      `${file} carries ${hits.length} table(s) whose first header cell is "${headerCell}", expected ` +
...
function ledgerEntries(file: string): LedgerEntry[] {
  const lines = readText(file).split("\n");
  const open = lines.findIndex((l) => l.trimEnd() === "````json");
  const close = open === -1 ? -1 : lines.findIndex((l, i) => i > open && l.trimEnd() === "````");
  if (open === -1 || close === -1) {
    throw new Error(
      `${file} carries no \`\`\`\`json appendix between four-backtick fence lines — the ledger's ` +
  ...
  try {
    ...
    throw new Error(`${file}'s JSON appendix does not parse: ${(e as Error).message} — NO verdict is reported over its rows`);
  }
  if (!Array.isArray(parsed)) {
    throw new Error(`${file}'s JSON appendix is not an array — NO verdict is reported over its rows`);
  }
  return parsed.map((e) => {
    const o = e as { id?: unknown; status?: unknown };
...
function statusAt(commit: string, manifestRel: string): ManifestStatus | null {
  let text: string;
  try {
 ...
 */
export function deriveFlipCommit(m: Manifest, manifestRel: string): string {
  ...
  for (const c of candidates) {
    if (statusAt(c, manifestRel) !== "discharged") continue;
    ...
    if (!parentDischarged) return c;
  }
  throw new Error(
    `the manifest reads ${JSON.stringify(m.statusLine)} but no commit in the history of ${manifestRel} ` +
...
function parseArgs(argv: readonly string[]): Cli {
  const cli: Cli = { manifest: FLIP_MANIFEST_DEFAULT, range: null };
...
function checkParts(m: Manifest): string[] {
  const d = deriveParts(m);
  for (const r of d.refusals) fail(`live-surface derivation refused: ${r}`);
  ...
  for (const p of d.parts) note(`  ${p.name}: ${p.members.join(", ")}`);
  ...
  for (const p of d.parts) {
    if (p.members.length === 0) {
      refused = true;
      fail(
        `the "${p.name}" part of the live-surface set derived ZERO members — refusing to report a ` +
    ...
    const declared = m.parts.find((q) => q.name === p.name) as Manifest["parts"][number];
    if (declared.declaredCount !== p.members.length) {
      refused = true;
      fail(
        `the "${p.name}" part derived ${p.members.length} member(s) but the manifest declares ` +
    ...
    if (!sameSet(listed, p.members)) {
      ...
      fail(
        `the "${p.name}" part's derived members differ from the manifest's listing — derived but not ` +
  ...
  if (!Number.isInteger(pinned) || d.union.length !== pinned) {
    refused = true;
    fail(
      `the live-surface set derived ${d.union.length} document(s), expected exactly ${pinned} ` +
...
function checkLocators(m: Manifest, manifestRel: string): void {
    ...
    switch (row.kind) {
      ...
      case "anchor": {
        ...
        if (!discharged && n === 0) {
          refusals += 1;
          fail(
            `flip row ${row.id}: the pre-flip anchor ${JSON.stringify(row.locator)} does not occur in ` +
        ...
        if (discharged && n > 0) {
          refusals += 1;
          fail(
            `flip row ${row.id}: the pre-flip anchor ${JSON.stringify(row.locator)} still occurs ` +
      ...
      case "marker": {
        ...
        if (!discharged && n > 0) {
          refusals += 1;
          fail(
            `flip row ${row.id}: the post-flip marker ${JSON.stringify(row.locator)} already occurs ` +
        ...
        if (discharged && n === 0) {
          refusals += 1;
          fail(
            `flip row ${row.id}: the post-flip marker ${JSON.stringify(row.locator)} does not occur ` +
      ...
      case "parity-row": {
        ...
        if (!discharged && unwrapCell(cells[0]) !== label) {
          refusals += 1;
          fail(
            `flip row ${row.id}: parity data row ${idx + 1} is labelled ${JSON.stringify(cells[0])}, the ` +
      ...
      case "ledger-row": {
        ...
        if (ledger === null) ledger = ledgerEntries(row.file);
  ...
  for (const c of m.corrections) {
    ...
    if (!discharged && n === 0) {
      refusals += 1;
      fail(
        `correction row ${c.id}: the stale fragment ${JSON.stringify(c.fragment)} does not occur in ` +
  ...
  for (const e of m.exemptions) {
    if (countOccurrences(textOf(e.file), e.anchor) === 0) {
      refusals += 1;
      fail(
        `the residual-rule exemption anchored at ${JSON.stringify(e.anchor)} does not occur in ` +
  ...
  if (refusals === 0) {
    pass(
      `every declared locator resolves in the ${m.status} state: ${m.flip.length} flip row(s), ` +
...
function main(): void {
  ...
  if (!existsSync(abs(manifestRel))) {
    throw new Error(
      `the flip manifest does not exist at ${abs(manifestRel)} — the declared side cannot be derived, ` +
