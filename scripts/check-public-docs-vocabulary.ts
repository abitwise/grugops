 ...
 * ═════════════════════════════════════════════════════════════════════════════════════════════
 */
function isCanonicalMarkdown(name: string): boolean {
  return name.endsWith(MARKDOWN_EXT);
}

function imitatesMarkdown(name: string): boolean {
...
function refuseMarkdownImitations(part: string, entries: readonly string[]): readonly string[] {
  for (const name of entries) {
    if (!imitatesMarkdown(name)) continue;
    DERIVATION_REFUSALS.push(
      `${part}: "${name}" spells the markdown extension in a case this corpus does not admit, so it ` +
