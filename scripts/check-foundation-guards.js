...
import { readModelsConfig, resolveModels, inheritForEveryStem, TIERED, MODEL_TIERS_COUNT, } from "./model-tiers.js";
...
function guardModelAssignment() {
    ...
    if (!config.ok) {
        warn(`model assignment: the \`models\` configuration could not be read, so the expectation DEGRADES to \`inherit\` for every role stem (D-11) rather than to any pinned tier, and the comparison below runs against that fallback — ${config.reason}`);
        ...
        if (!resolution.ok) {
            warn(`model assignment: the \`models\` configuration read cleanly but could not be RESOLVED against this tree's role stems, so the expectation DEGRADES to \`inherit\` for every stem (D-11) rather than to any pinned tier — ${resolution.reason}`);
            resolved = inheritForEveryStem(stems);
    ...
    if (modelFail === "") {
        ...
        pass(`model assignment: ${adapterRels.length} committed adapter(s) under ${ADAPTER_DIR} compared against a resolution recomputed for ${stems.length} derived role stem(s); preset "${presetLabel}" from ${sourceLabel}; distinct aliases resolved: ${aliases.join(", ")}`);
    }
    else {
