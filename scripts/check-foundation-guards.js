...
function guardModelAssignment() {
    ...
    for (const rel of nonAgentSurfaces) {
        ...
        if (!parsed.value.has("model"))
            continue;
        strayPins.push(`${rel}: ${admittedValuesFor(parsed.value, "model")
            .map((v) => `\`${v}\``)
            .join(", ")}`);
    }
...
// else.
const PIN_SCAN_ROOT_DIRS = ["agent-factory", "docs", "install"];
const PIN_SCAN_ROOT_FILES = ["AGENTS.md", "README.md"];
...
const PIN_SCAN_SKIPPED_DIRS = ["node_modules", ".git"];
...
function pinOccurrencesIn(rel) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        for (const m of lines[i].matchAll(new RegExp(PIN_OCCURRENCE_SOURCE, "g"))) {
    ...
    const walk = (rel) => {
        for (const entry of readdirSync(abs(rel), { withFileTypes: true })) {
            if (entry.isDirectory()) {
                if (PIN_SCAN_SKIPPED_DIRS.includes(entry.name))
                    continue;
                walk(`${rel}/${entry.name}`);
                continue;
            }
...
function guardPlaywrightMcpPin() {
    ...
    const authorityRemedy = `the version has ONE home and this guard READS it from there; it must never fall back to an ` +
    ...
    if (!fileExists(PLAYWRIGHT_MCP_PIN_SOURCE)) {
        fail(`playwright MCP pin: the pin source ${PLAYWRIGHT_MCP_PIN_SOURCE} does not exist — ${authorityRemedy}`);
        FAILS += 1;
        return;
    }
    let authority;
    try {
        ...
        fail(`playwright MCP pin: the pin source ${PLAYWRIGHT_MCP_PIN_SOURCE} could not be read — ${e.message}. ${authorityRemedy}`);
        FAILS += 1;
        return;
    }
    const first = authority[0];
    if (first === undefined) {
        fail(`playwright MCP pin: the pin source ${PLAYWRIGHT_MCP_PIN_SOURCE} carries no pinned mention of the package, so there is no literal to read — ${authorityRemedy}`);
        FAILS += 1;
        return;
    }
    if (first.version === "") {
    ...
    try {
        ...
        fail(`playwright MCP pin: a member of the derived scan set could not be read — ${e.message}. A scan that could not open one of its own members has not been performed`);
        FAILS += 1;
        return;
    }
    ...
    process.stdout.write(`        pin \`${pin}\` read from ${PLAYWRIGHT_MCP_PIN_SOURCE} (line ${first.line}); ` +
    ...
    FAILS += reportMeasured(measured, { pass, fail }, (x) => `  ${x.file}:${x.line}: found \`${x.version === "" ? "(no version)" : x.version}\`, expected the pin \`${pin}\` — bump the literal in ${PLAYWRIGHT_MCP_PIN_SOURCE} and re-pin every mention, or correct this one`);
}
// ---------------------------------------------------------------------------
// Run all guards.
// ---------------------------------------------------------------------------
