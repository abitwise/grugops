...
publicDocsDerivationRefusals, } from "./check-public-docs-vocabulary.js";
...
import { readRegistry, scanAnchoredDocument, anchoredBlockAt, } from "./audit-model.js";
...
export const BANNED_CLAIM_EXCLUDED = [
    ...
    },
    {
        ...
        hits: 1,
        reason: "hit ONE line of correct operational text in agent-factory/roles/incident-responder.md, " +
...
function guaranteesMembers() {
    if (!existsSync(abs(GUARANTEES_DOC))) {
        DERIVATION_REFUSALS.push(`${GUARANTEES_DOC} is a NAMED member of the banned-claim scan set and does not exist at ` +
 ...
 */
export function bannedClaimScan(parts = BANNED_CLAIM_SCAN_PARTS) {
    const seen = new Set();
 ...
 */
export function bannedClaimScanOverlap(parts = BANNED_CLAIM_SCAN_PARTS) {
    const seen = new Set();
    let duplicates = 0;
    for (const part of parts) {
        for (const m of part.members) {
 ...
 */
export function deriveExemptBlocks(root, exemptText, callerLines, region) {
    const refusals = [];
    const empty = () => ({
        lines: new Set(),
        ids: [],
    ...
    try {
        ...
        refusals.push(`the claim registry could not be parsed, so NO line inside the one named exemption region is ` +
            ...
            `content bound. The parse said: ${e.message}`);
        return empty();
    }
    ...
    if (scan.lines.length !== callerLines.length &&
        scan.lines.length !== callerLines.length - 1) {
        refusals.push(`the exemption document assembles into ${callerLines.length} line(s) for this gate and ` +
    ...
    for (let i = 0; i < scan.lines.length; i++) {
        if (scan.lines[i] !== callerLines[i]) {
            refusals.push(`the exemption document's two assemblies first disagree at index ${i}: the authority reads ` +
    ...
    for (const c of claims) {
        if (c.file === BANNED_CLAIM_EXEMPT_REGION.file)
            rows.set(c.id, c);
    }
    const ids = [];
    const diverged = [];
    ...
    for (const anchor of scan.anchors) {
        ...
        try {
            ...
            refusals.push(`${anchor.id}'s anchored block could not be measured (${e.message}), so its ` +
    ...
    if (ids.length === 0) {
        refusals.push(`the one named exemption region \`${BANNED_CLAIM_EXEMPT_REGION.file}\` § ` +
        ...
        refusals.push(`the one named exemption region contains ${ids.length} registry-anchored block(s) ` +
    ...
    for (const id of diverged) {
        refusals.push(`${id}'s anchored block inside the one named exemption region no longer matches its registry ` +
 ...
 */
export const BANNED_CLAIM_EXEMPT_COMPOSITION = [
 ...
 */
const UNFROZEN_IN_REGION_REMEDY = `this line is INSIDE the one named exemption region and OUTSIDE every registry-anchored block, ` +
...
function runAll() {
    ...
    for (const refusal of publicDocsDerivationRefusals()) {
        fail(`public-document corpus derivation refused (raised in ` +
    ...
    if (!existsSync(exemptAbs)) {
        ...
        try {
            ...
            fail(`the document carrying the one named exemption region, ` +
                ...
                `(${e.message}). The exemption region is therefore NOT located and the ` +
    ...
    let exemptBlocks = null;
    if (exemptReadOk && exemptRegion !== null) {
        exemptBlocks = deriveExemptBlocks(ROOT, exemptText, exemptText.split("\n"), exemptRegion);
        for (const r of exemptBlocks.refusals)
            fail(r);
    }
    ...
    // carve-out.
    const exemptLineSet = exemptBlocks?.lines ?? new Set();
        ...
        if (isExemptRegionMember(file) && exemptReadOk) {
            ...
            try {
                ...
                fail(`${file} is a member of the banned-claim scan set and could not be read ` +
                    `(${e.message}) — refusing to report a verdict over a document that was ` +
    ...
    if (exemptRegion !== null) {
        ...
        for (const c of BANNED_CLAIM_EXEMPT_COMPOSITION) {
            ...
            if (live !== c.count) {
                fail(`the one named exemption region suppressed ${live} \`${c.group}\` occurrence(s), and ` +
        ...
        if (declaredSum !== BANNED_CLAIM_EXEMPT_SUPPRESSED) {
            fail(`BANNED_CLAIM_EXEMPT_COMPOSITION sums to ${declaredSum} and ` +
    ...
    // it found nothing.
    if (FAILS === 0)
        process.stdout.write(publishedSentence);
