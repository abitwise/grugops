 ...
 */
function locateExemptRegion(lines) {
    const headings = [];
    for (let i = 0; i < lines.length; i++) {
        if (lines[i] === BANNED_CLAIM_EXEMPT_REGION.heading)
            headings.push(i);
    }
...
function runAll() {
    ...
    for (const file of scan) {
        ...
        const region = file === BANNED_CLAIM_EXEMPT_REGION.file ? locateExemptRegion(lines) : null;
        for (let i = 0; i < lines.length; i++) {
            if (region !== null &&
                i >= region.headingAt &&
                i < region.endBefore) {
