...
import { FENCE_DELIMITER_LINE, fencedLineFlags, sectionEndIndex, unfencedHeadingIndex, unfencedMatchIndices, } from "./frontmatter.js";
// ---------------------------------------------------------------------------
 ...
 */
export function scanAnchoredDocument(text) {
    const lines = text.split("\n");
 ...
 */
export function anchoredBlockAt(scan, anchor, verbatim) {
    ...
    if (isBlank(verbatim)) {
        throw new Error(`audit-model: refusing to compare ${anchor.id}'s anchored block — its registry verbatim is ` +
