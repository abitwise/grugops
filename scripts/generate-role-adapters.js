...
import { readModelsConfig, resolveModels, resolvedPresetLine, } from "./model-tiers.js";
...
const HARD_LIMIT = "Never merge to a protected branch. Never deploy to prod. Humans always hold merge and deploy.";
const PROVENANCE = "<!-- GENERATED — do not hand-edit. Re-run: node scripts/generate-role-adapters.js -->";
...
const stemOf = (file) => file.replace(/\.md$/, "");
...
for (const file of roleFiles) {
    ...
    if (/[^\x00-\x7F]/.test(file)) {
        fail(`${file}: non-ASCII byte in a role filename — adapter names are compared by exact string equality and written as bytes; rename the role file to ASCII`);
    }
    ...
    if (caps.length === 0) {
        fail(`${file}: no \`capabilities:\` key in the role frontmatter — an adapter whose \`tools\` resolve to nothing is refused by the platform at launch; declare at least one of: ${VOCABULARY.join(", ")}`);
    }
    if (caps.length > 1) {
        ...
        fail(`${file}: ${caps.length} \`capabilities:\` keys in one role frontmatter, expected exactly 1 — every occurrence is retained rather than last-wins, because silently discarding a declaration is a bypass; delete the extra key`);
    }
    ...
    if (rawCaps === "") {
        ...
        fail(`${file}: \`capabilities:\` is absent or empty — an adapter whose \`tools\` resolve to nothing is refused by the platform at launch; declare at least one of: ${VOCABULARY.join(", ")}`);
    }
    const tokens = rawCaps.split(/\s+/).filter((t) => t !== "");
    for (const t of tokens) {
        if (!VOCABULARY.includes(t)) {
            fail(`${file}: capability token "${t}" is outside the closed vocabulary (${VOCABULARY.join(", ")}) — see agent-factory/packaging/subagent.frontmatter.md`);
        }
    }
    ...
    if (job === null || job.trim() === "") {
        fail(`${file}: no \`## One job\` section — refusing to emit an adapter with an empty description (\`description\` drives auto-routing)`);
    }
    const act = sectionBody(text, "Activates when");
    if (act === null || act.trim() === "") {
        fail(`${file}: no \`## Activates when\` section — refusing to emit an adapter with no use-when clause (\`description\` drives auto-routing)`);
    }
...
if (coordinators.length !== 1) {
    fail(`expected exactly one role whose basename is "${COORDINATOR_ROLE}.md" in ${ROLES_DIR}, found ${coordinators.length}${coordinators.length > 0 ? `: ${coordinators.map((c) => c.file).join(", ")}` : " — a zero-coordinator kit has no adapter that may hold the spawn grant"}`);
}
...
console.log(`generate-role-adapters: wrote ${rendered.length} adapters to ${OUT_DIR} (coordinator ${coordinator.name} grants ${grant.length} names)`);
