...
function guardReferentialIntegrity(): void {
  ...
  const grantedResult = keysGrantedAgentNames(
    parsedAdapters.get(coordinators[0])!,
  );
  if (!grantedResult.ok) {
    ...
    fail(
      `KIT-03: the coordinator ${ADAPTER_DIR}/${coordinators[0]} has an UNREADABLE grant enumeration — ${grantedResult.reason}. An unreadable grant cannot be compared against the adapter set, so it is NEVER read as "grants these names"`,
    );
    return;
  }
