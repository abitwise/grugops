...
function guardAdapterSize(): void {
  ...
  if (AGENT_ADAPTERS.length === 0 || SKILL_ADAPTERS.length === 0) {
    fail(
      `adapter derivation returned an empty set — ${ADAPTER_DIR}: ${AGENT_ADAPTERS.length} adapter(s), ${SKILL_DIR}: ${SKILL_ADAPTERS.length} adapter(s). An empty adapter directory is never "nothing to compare, therefore fine".`,
    );
  }
