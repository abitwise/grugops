...
function flattenBlock(
  ...
  const flush = (): Parsed<FrontmatterKeys> | null => {
    ...
    if (seen === undefined) keys.set(cur.key, [resolved.value]);
    else seen.push(resolved.value);
    cur = null;
    return null;
  };

  for (const raw of block) {
    if (raw.trim() === "") continue; // a blank line is a paragraph break, never a key boundary
    ...
    if (indent > baseIndent) {
      ...
      if (startsWithReference(t)) return refuseRef(t);
...
const allDeclaredWs = (residue: string): boolean =>
  [...residue].every((c) => DELIMITER_WS_CHAR.test(c));
...
function isLegalDelimiter(line: string, payload: string): boolean {
