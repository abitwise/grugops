...
export function blockContextFlags(text: string): boolean[] {
  const lines = text.split("\n");
  const flags: boolean[] = [];
  let inside = false;
  let openChar = "";
  let openLen = 0;
  let html: "none" | "comment" | "tag" = "none";
  for (const line of lines) {
    ...
    if (!inside) {
      ...
      if (HTML_BLOCK_OPEN.test(line)) {
        ...
        if (html === "comment" && line.includes("-->")) html = "none";
        flags.push(true);
        continue;
      }
    } else if (
...
function strictFenceRun(line: string): { char: string; len: number; info: string } | null {
  ...
  if (char !== "`" && char !== "~") return null;
  let len = 0;
  while (len < rest.length && rest[len] === char) len += 1;
  if (len < 3) return null;
  const info = rest.slice(len);
  if (char === "`" && info.includes("`")) return null; // a backtick fence's info string may not carry one
 ...
 * ---------------------------------------------------------------------------------------------
 */
export function carriageReturnLines(text: string): number[] {
  const lines = text.split("\n");
  const at: number[] = [];
  for (let i = 0; i < lines.length; i++) {
    // A trailing CR is CRLF, which git normalises away; a CR anywhere else is a lone one.
    if (lines[i].replace(/\r$/, "").includes("\r")) at.push(i);
  }
  return at;
}
 ...
 * ═════════════════════════════════════════════════════════════════════════════════════════════
 */
export function unfencedHeadingNearMisses(text: string, heading: string): number[] {
...
interface BlockHeader {
  ...
  // (D-62) THE EXPLICIT INDENTATION INDICATOR THIS HEADER DECLARES, or `null` where it declares none
  // and the content indentation is AUTO-DETECTED. Read by `blockExplicitIndent` from the same text
  // `blockLineBreak` reads; see that function for why the digit stopped being safe to discard.
...
const blockExplicitIndent = (indicator: string): number | null => {
  const digit = indicator.match(BLOCK_EXPLICIT_INDENT);
  return digit === null ? null : Number(digit[1]);
};

// (D-61 — 27-REVIEW.md § CR-02, round 11) THE INTRODUCTION SET, DECLARED ONCE AS DATA — SO EVERY
// QUESTION THIS MODULE ASKS "AT AN INTRODUCTION" IS ASKED AT ALL OF THEM BY CONSTRUCTION, NOT BY A
// FUTURE AUTHOR REMEMBERING.
//
// D-57 enumerated the four introductions from YAML's grammar and wrote them out as four straight-line
// branches. That was enough while exactly one question was asked at each (does `BLOCK_INDICATOR`
// match?). It stopped being enough the moment a SECOND question arrived — "is a node PROPERTY standing
// in front of this node start?" — because four branches are four places to forget, and the CR-02
// review reported the failure at only TWO of them. Closing the two reported spellings is the
// enumerate-the-bad shape this module has declined eleven times; so the set became data, and both
// questions now iterate it.
//
// Each member answers exactly one thing: WHERE, on this line, does the node this introduction
// introduces START. Nothing here decides what a header LOOKS like (`BLOCK_INDICATOR` still owns that)
// and nothing here decides what a node PROPERTY looks like (`NODE_PROPERTY_AT_NODE_START` still owns
// that). The set is the POSITION vocabulary and nothing else.
interface HeaderIntroduction {
  // Named so an assertion message and a ledger entry can cite the member rather than an index.
  readonly name: string;
  // The split this introduction performs, or null when the line is not of its shape. `leading` is the
  // text BEFORE the node — kept, because the loader keeps it (see `BlockHeader.leading`). `node` is
  // the node's own text, whose offset 0 IS a node start.
  readonly split: (text: string) => { readonly leading: string; readonly node: string } | null;
  // Does this introduction carry a mapping-VALUE indicator? See `BlockHeader.mappingValueIndicator`
  // for the eight measured loader rows this answer is derived from. It is a CONSTANT per member
  // rather than a re-derivation at the call site, because the answer is a property of the
  // introduction and not of the line.
  readonly mappingValueIndicator: boolean;
  // Does a MAPPING SEPARATOR — `key:`, `?` or `:` — stand between offset 0 of the line and this
  // introduction's node start? True for the three block-mapping introductions; false for the bare one,
  // whose node start IS offset 0 and is therefore already reached by the reference refusal's existing
  // application points. This is the field the fourth application point iterates: see
  // `mappingSeparatorNodeStarts`.
  readonly postSeparator: boolean;
}

const HEADER_INTRODUCTIONS: readonly HeaderIntroduction[] = [
  {
...
const explicitSplit = (
  text: string,
  indicator: "?" | ":",
): { readonly leading: string; readonly node: string } | null => {
  const explicit = text.match(BLOCK_MAP_EXPLICIT);
  if (explicit === null || explicit[1] !== indicator) return null;
...
const stripNodeProperties = (text: string): string => {
  let out = text;
  ...
  for (let n = 0; n < 2; n++) {
    ...
    if (isTag ? strippedTag : strippedAnchor) break; // two of a kind: YAML forbids it, so leave it
    if (isTag) strippedTag = true;
    else strippedAnchor = true;
...
function blockHeaderAt(text: string): BlockHeader | null {
  ...
  for (const introduction of HEADER_INTRODUCTIONS) {
    ...
    if (!BLOCK_INDICATOR.test(node)) continue;
    return {
...
function mappingSeparatorNodeStarts(
  text: string,
): readonly { readonly node: string; readonly mappingValueIndicator: boolean }[] {
  const out: { node: string; mappingValueIndicator: boolean }[] = [];
  for (const introduction of HEADER_INTRODUCTIONS) {
    if (!introduction.postSeparator) continue;
    const split = introduction.split(text);
    if (split === null) continue;
    out.push({
      node: split.node,
...
export const NODE_PROPERTY_AT_NODE_START =
  /^(?:!(?:<[^>]*>|[^\s[\]{},]*)|&[^\s[\]{},]+)(?=[\s[\]{},]|$)/;
...
interface Accumulator {
  ...
  //
  // (D-62 — 27-REVIEW.md § WR-01, round 11) THE PARAGRAPH ABOVE STATED THE END CONDITION AS THE
  // HEADER LINE'S INDENT, AND THAT IS NOT YAML'S RULE. YAML 1.2 § 8.1.1.1 AUTO-DETECTS the scalar's
  // content indentation from its FIRST NON-EMPTY CONTENT LINE and ends the scalar at the first line
  // LESS INDENTED THAN THAT. The two agree only where the first content line sits at the minimum,
  // which is the overwhelming majority of real documents — and diverge exactly where it does not:
  //
  //   tools:                         module (pre-D-62): GRANT, names ["grugops-orchestrator"]
  //     nested: >-                   /usr/bin/ruby -ryaml: {"nested"=>"Read,"}  -> NO grant
  //           Read,
  //       # x, Agent(grugops-orchestrator)
  //
  // A module GRANT the loader does not have is the direction this module's own doc block (search
  // "never exemptible") names as never exemptible, and it shipped in the change that stated it.
  //
  // `blockIndent` KEEPS ITS JOB AS THE FLOOR. The detected indentation must EXCEED the header line's
  // indent — that is what makes a line a content line at all, and it is the window in which the
  // detection happens — so the two facts are related rather than one replacing the other. See
  // `blockContentIndent`.
  //
  ...
  blockIndent: number;
  // (D-62) THE SCALAR'S OWN CONTENT INDENTATION, SET ONCE AT THE POINT IT BECOMES KNOWN — the first
  // non-empty content line — and `null` in the window before that. Never re-derived per line and
  // never held anywhere else, in the same discipline the three fields around it already follow.
  //
  // WHAT THE END CONDITION DOES IN THE UNKNOWN WINDOW, AND WHY IT IS THE LOADER'S ANSWER. Until the
  // first content line arrives there IS no detected indentation, and YAML's own rule for that window
  // is the floor: a line more indented than the header line's own indent is the scalar's first
  // content line and DEFINES the indentation; a line that is not is a sibling and the scalar is
  // empty. That is exactly `blockIndent`'s test, so the unknown window keeps the pre-D-62 condition
  // and the fix is scoped to the lines AFTER detection.
  //
  // WHERE THE HEADER DECLARES AN EXPLICIT INDENTATION DIGIT this is seeded at `openBlock` from
  // `blockIndent + digit` and no detection happens, because that is what the loader does — see
  // `blockExplicitIndent` for the eight measured rows and for the silent no-grant that discarding the
  // digit would have created.
  //
  // THE UNIT IS `indentOf`'S UNIT, WHICH IS A COUNT OF LEADING `[ \t]` CHARACTERS — one column per
  // character, a tab counted as ONE. Both sides of the comparison are taken by that one helper from
  // the raw line, so no rounding, truncation or unit mismatch is possible between the header line's
  // measurement and a content line's; they are literally the same function of the same shape of
  // input. A tab inside an indentation run is not a case this equivalence has to survive: YAML
  // forbids it, and `/usr/bin/ruby -ryaml` REJECTS `tools: >-` / `<TAB>Read, Agent(…)` outright with
  // "found a tab character where an indentation space is expected while scanning a block scalar", so
    ...
    if (indent > baseIndent) {
      ...
      if (cur.block) {
        ...
        if (inside) {
          ...
          if (cur.blockContentIndent === null) cur.blockContentIndent = indent;
      ...
      if (!inScalar && cur.state.flowDepth === 0) {
        ...
        for (const start of mappingSeparatorNodeStarts(t)) {
          if (!(start.mappingValueIndicator || startsNode)) continue;
