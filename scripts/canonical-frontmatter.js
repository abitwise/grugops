...
export const GRANT_KEYS = ["tools", "allowed-tools"];
...
export const DOUBLE_QUOTED_KEYS = [
    "description",
    "argument-hint",
];
...
export const PLAIN_SCALAR_ALPHABET = new Set([
...
const codePointLabel = (c) => `U+${(c.codePointAt(0) ?? 0).toString(16).toUpperCase().padStart(4, "0")}`;
...
const SIGIL_CONSTRUCT_NAME = new Map([
...
const admitValue = (key, raw, lineNo) => {
    ...
    if (raw.startsWith('"')) {
        if (!DOUBLE_QUOTED_KEYS.includes(key)) {
            return refuse("quoted-on-plain-only-key", `line ${lineNo}: \`${key}\` carries a double-quoted value, and the only keys admitted to carry one are ${DOUBLE_QUOTED_KEYS.join(" and ")}; the grant-bearing keys ${GRANT_KEYS.join(" and ")} admit plain scalars only, which is what keeps the double-quoted escape alphabet permanently outside the path that renders a spawn verdict`);
        }
        if (raw.length < 2 || !raw.endsWith('"')) {
            return refuse("unterminated-double-quote", `line ${lineNo}: the double-quoted value of \`${key}\` opens with \`"\` and the line does not close it`);
        }
        ...
        for (let i = 0; i < body.length; i += 1) {
            const c = body[i];
            if (isControl(c)) {
                return refuse("control-character", `line ${lineNo}: the double-quoted value of \`${key}\` carries the control character ${codePointLabel(c)}`);
            }
            if (c === '"') {
                return refuse("embedded-double-quote", `line ${lineNo}: the double-quoted value of \`${key}\` carries an unescaped \`"\` at offset ${i + 1} of its body, so the scalar this document expresses is not the text between the first and last quote on the line`);
            }
            if (c !== "\\") {
                resolved += c;
                continue;
            ...
            if (i + 1 >= body.length) {
                return refuse("unterminated-double-quote", `line ${lineNo}: the double-quoted value of \`${key}\` ends with a backslash, so the \`"\` that appeared to close it was escaped and the scalar is not closed on this line`);
            }
            const next = body[i + 1];
            const sub = DQ_ESCAPE_ALLOWLIST.get(next);
            if (sub === undefined) {
                return refuse("disallowed-escape", `line ${lineNo}: the double-quoted value of \`${key}\` carries the escape \`\\${next}\`, which is not one of the escapes this tree resolves (${[...DQ_ESCAPE_ALLOWLIST.keys()].map((k) => `\\${k}`).join(" ")}); resolving it would be this module decoding a document it deliberately does not decode`);
            }
            resolved += sub;
            i += 1;
        }
    ...
    for (const c of raw) {
        ...
        if (c === "(") {
            depth += 1;
            if (depth > 1) {
                return refuse("unbalanced-parentheses", `line ${lineNo}: the plain value of \`${key}\` nests \`(\` inside \`(\`; a nested parenthesis means the enumeration inside a scoped grant cannot be split on the comma the document wrote, so the names these bytes would be read as are not the names the document expresses`);
            }
        }
        else if (c === ")") {
            depth -= 1;
            if (depth < 0) {
                return refuse("unbalanced-parentheses", `line ${lineNo}: the plain value of \`${key}\` closes a \`)\` that was never opened`);
            }
        }
    }
    if (depth !== 0) {
...
export function admit(text, options) {
    ...
    if (lines[0] !== DELIMITER) {
        return refuse("no-opening-delimiter", `line 1 of the fence-stripped document is \`${excerpt(lines[0] ?? "")}\`, and the canonical form requires it to be exactly \`${DELIMITER}\`; a byte-order mark, leading whitespace or a directive before the delimiter is refused rather than skipped`);
    }
    let close = -1;
    ...
    if (close < 0) {
        return refuse("no-closing-delimiter", `the frontmatter region opened at line 1 is never closed by a \`${DELIMITER}\` line`);
    }
    const region = lines.slice(1, close);
    if (region.length === 0) {
        return refuse("empty-region", "the frontmatter region is empty; a region carrying no keys is refused rather than admitted as an empty map, because `readable and empty` and `unreadable` must never be confusable");
    }
    ...
    for (let r = 0; r < region.length; r += 1) {
        ...
        const lineNo = r + 2;
        if (line.includes("\t")) {
            return refuse("tab-in-region", `line ${lineNo}: the frontmatter region carries a TAB character; indentation decides structure here and a tab has no defined column width`);
        }
        for (const c of line) {
            if (isControl(c)) {
                return refuse("control-character", `line ${lineNo}: the frontmatter region carries the control character ${codePointLabel(c)}`);
            }
        }
    }
    ...
    for (let r = 0; r < region.length; r += 1) {
        ...
        if (indent !== KEY_INDENT && indent !== SEQUENCE_INDENT) {
            return refuse("bad-indentation", `line ${lineNo}: indented ${indent} column(s); the canonical form admits exactly two columns — ${KEY_INDENT} for a key line and ${SEQUENCE_INDENT} for a block-sequence item`);
        }
        if (indent === SEQUENCE_INDENT) {
            const item = SEQUENCE_ITEM.exec(line);
            if (item === null) {
                return refuse("unrecognized-line", `line ${lineNo}: \`${excerpt(line)}\` is indented as a block-sequence item and does not match the admitted production \`${LINE_PRODUCTIONS[2]}\``);
            }
            if (pendingKey === null) {
...
export function admittedHasSpawnGrant(doc) {
...
export function admittedGrantedNames(doc) {
    const names = new Set();
