...
interface TsTypeChecker {
  ...
  /**
   * D-36: what a CONTAINER holds. Declared OPTIONAL because it arrived in the public API later
   * than the rest of this surface, and its absence is fail-CLOSED: the walk reports
   * `container-unreadable` and the run exits 2, rather than quietly not reaching what a container
   * holds. It returns an EMPTY array for a type that is not a reference, which is what lets one
   * call site ask it of every library-declared type without a kind test of its own.
   */
  readonly getTypeArguments?: (type: TsType) => readonly TsType[];
...
interface TsProgram {
  ...
  /**
   * D-36: OPTIONAL on purpose, and its absence is fail-CLOSED rather than fail-quiet. It tells the
   * framework-surface walk the compiler's own library from the target's declarations. A Program
   * that does not publish it makes the walk descend into the library, reach its depth bound, and
   * exit 2 with the truncation cause — the loud outcome, never a narrower walk nobody chose.
   */
  readonly isSourceFileDefaultLibrary?: (file: TsSourceFile) => boolean;
}
...
export const SURFACE_TRUNCATION_REACHED = Object.freeze({
  ...
  "container-unreadable":
    "a standard-library container whose type arguments this checker does not publish, so what it holds could not be reached",
  ...
  "surface-unreadable":
    "a declaration this checker could not read, so the subtree hanging off it was never walked",
} as const);

/** Every way the walk can stop early, DERIVED from the record above rather than re-typed. */
export const SURFACE_TRUNCATION_ARMS: readonly string[] = Object.freeze(
  Object.keys(SURFACE_TRUNCATION_REACHED),
);

export type SurfaceTruncation = keyof typeof SURFACE_TRUNCATION_REACHED;
 ...
 */
export const SURFACE_SWALLOW_SITES: Readonly<
  Record<string, { readonly arm: SurfaceTruncation; readonly what: string } | { readonly residual: string; readonly what: string }>
> = Object.freeze({
  "addDeclarations#1": {
 ...
 */
export const SURFACE_TRUNCATED_CAUSE =
  `the ${PLAYWRIGHT_TEST_MODULE} declared surface was not fully walked (node bound ` +
...
export function surfaceTruncatedCause(reached: SurfaceTruncation): string {
 ...
 */
export function frameworkSurface(
  ts: TsApi,
  program: TsProgram,
  checker: TsTypeChecker,
  ...
  readonly typePaths: ReadonlyMap<TsSymbol, string>;
  readonly truncated: SurfaceTruncation | null;
} {
  const files = new Set<string>();
  const typePaths = new Map<TsSymbol, string>();
  let truncated: SurfaceTruncation | null = null;
  const addDeclarations = (symbol: TsSymbol): void => {
    for (const declaration of symbol.declarations ?? []) {
      if (isDefaultLibraryDeclaration(program, declaration)) continue;
      try {
  ...
  const isStandardLibraryOnly = (symbol: TsSymbol): boolean => {
  ...
  for (let head = 0; head < queue.length; head++) {
    ...
    if (symbol !== undefined) {
      ...
      if (isStandardLibraryOnly(symbol)) {
        ...
        if (held === null) truncated ??= "container-unreadable";
        else for (const arg of held) queue.push({ type: arg, path, depth: depth + 1 });
        continue;
      }
    ...
    if (depth >= SURFACE_DEPTH_BOUND) {
      if (hasExpandableMembers(ts, checker, type)) truncated ??= "depth-bound";
      continue;
    }
    try {
 ...
 */
function containerTypeArguments(checker: TsTypeChecker, type: TsType): readonly TsType[] | null {
 ...
 */
function hasExpandableMembers(ts: TsApi, checker: TsTypeChecker, type: TsType): boolean {
  try {
    if (checker.getPropertiesOfType(type).length > 0) return true;
  } catch {
    return true;
  }
  try {
 ...
 */
function isDefaultLibraryDeclaration(program: TsProgram, declaration: TsNode): boolean {
...
export interface SpecDerivation {
  ...
  /**
   * D-33 (5): how many directory entries the walk refused to descend into, per `SKIPPED_DIRECTORIES`
   * member. A member the walk never met is ABSENT rather than zero, so the record's own size is the
   * number of boundary names that actually narrowed this run.
   */
  readonly skippedDirectoryHits: Readonly<Record<string, number>>;
}
...
export function deriveSpecPaths(repoRoot: string): SpecDerivation {
  ...
  const skippedDirectoryHits: Record<string, number> = {};
...
function runMain(
  ...
  if (disclosure !== null) err(disclosure);
