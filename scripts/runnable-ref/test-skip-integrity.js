...
if (!registryPath) {
    process.stderr.write("Error: no registry file path was provided. Usage: node test-skip-integrity.js <registry-file> [--skip-count <N>] [--json]\n");
    process.exit(2);
}
let contents;
try {
...
for (const row of rows) {
    ...
    let invalid = false;
    if (row.testId === "") {
        findings.push("A skip-registry row has an empty Test ID; every justified skip must name the test it applies to.");
        invalid = true;
    }
    if (PLACEHOLDER_OWNERS.has(row.owner.toLowerCase())) {
        findings.push(`The skip justification for "${label}" has no real owner (found "${row.owner || "(blank)"}"); a placeholder owner is not an accountable owner.`);
        invalid = true;
    }
    if (row.ticket === "") {
        findings.push(`The skip justification for "${label}" has no Ticket/REQ reference; every justified skip must cite a ticket or requirement.`);
        invalid = true;
    }
    if (!VALID_CATEGORIES.has(row.category.toLowerCase())) {
        findings.push(`The skip justification for "${label}" has an unrecognized category "${row.category}"; it must be one of: ${[...VALID_CATEGORIES].join(", ")}.`);
        invalid = true;
    }
    const parsedExpiry = parseExpiry(row.expiry);
    if (parsedExpiry === null) {
        findings.push(`The skip justification for "${label}" has an unparseable Expiry "${row.expiry}"; it must be a real date in YYYY-MM-DD form.`);
        invalid = true;
    }
    else if (parsedExpiry < today) {
        findings.push(`The skip justification for "${label}" expired on ${parsedExpiry} (today is ${today}); an expired justification no longer counts and must be renewed or removed.`);
...
if (skipCountRaw === undefined || !/^\d+$/.test(skipCountRaw)) {
    findings.push("Skip count was not provided (UNKNOWN - verify); cannot confirm the suite is fully justified.");
}
else {
    const skipCount = parseInt(skipCountRaw, 10);
    if (skipCount > validJustifications) {
        findings.push(`Unjustified skips: the runner reported ${skipCount} skipped tests but only ${validJustifications} are justified.`);
    }
}
