 ...
 */
export const SKIPPED_DIRECTORY_DISCLOSURE_CLASS = Object.freeze({
 ...
 */
export const DISCLOSED_SKIPPED_DIRECTORIES = Object.freeze(SKIPPED_DIRECTORIES.filter((n) => SKIPPED_DIRECTORY_DISCLOSURE_CLASS[n] === "could-hide-evidence"));
/** The other half, derived the same way. Listed in the recipe so nothing becomes invisible. */
export const UNDISCLOSED_SKIPPED_DIRECTORIES = Object.freeze(SKIPPED_DIRECTORIES.filter((n) => SKIPPED_DIRECTORY_DISCLOSURE_CLASS[n] === "cannot-hide-evidence"));
...
export function isBannedModifierPath(dottedPath) {
    ...
    if (BANNED_EXACT_PATHS.includes(normalised))
        return true;
 ...
 * emission.
 */
export const SKIPPED_DIRECTORY_COUNT_MEANING = "Each number is a count of DIRECTORY ENTRIES the walk refused to descend into, never a count of uat specs hidden under them; the walk did not look inside, so it does not know that number.";
...
export function loadTypeScriptFromTarget(repoRoot) {
    try {
        ...
        if (candidate.NodeFlags === undefined ||
 ...
 */
export function createProgramForTarget(repoRoot, specAbsPaths, ts) {
    let configPath;
    try {
 ...
 */
export function faultKey(path, separator = sep) {
    if (separator === "") {
        throw new Error("uat-spec-integrity.faultKey: refusing an empty separator — it would split every character. " +
 ...
 */
function frameworkModuleSymbol(ts, program, checker) {
    const quoted = `"${PLAYWRIGHT_TEST_MODULE}"`;
    try {
        for (const ambient of checker.getAmbientModules()) {
            if (ambient.name === quoted)
                return ambient;
        }
    }
    catch {
...
export const MODIFIER_IDENTITY_ARMS = Object.freeze(Object.keys(IDENTITY_BAN_OPERAND));
/**
 ...
 */
export function resolveBannedModifier(ts, ctx, call) {
    ...
    for (const declaration of declarations) {
        try {
            if (ctx.frameworkFiles.has(declaration.getSourceFile().fileName))
                fromFramework = true;
        }
        catch {
    ...
    if (!fromFramework) {
        ...
        for (const declaration of declarations) {
            if (fromDeclarationSurface(ts, declaration))
                return { kind: "foreign-declared" };
        }
 ...
 */
function fromDeclarationSurface(ts, declaration) {
    ...
    try {
        if (declaration.getSourceFile().isDeclarationFile === true)
            return true;
    }
    catch {
...
function followAlias(ts, checker, symbol) {
 ...
 */
function throughBindingElement(ts, checker, symbol) {
    ...
    const wanted = propertyName !== undefined && ts.isIdentifier(propertyName) ? propertyName.text : symbol.name;
    try {
 ...
 */
function identityPath(ts, ctx, callee, symbol) {
    ...
    if (syntactic !== null) {
        ...
        if (segments.length >= 2 && !segments[0].endsWith(CALL_LINK_MARKER)) {
            ...
            if (rewritten === IDENTITY_HEAD_DROP)
...
function identityHeadSegment(ts, ctx, head) {
    const checker = ctx.checker;
    try {
...
export function deriveSpecPaths(repoRoot) {
    ...
    while (pending.length > 0) {
        const { absDir, relDir } = pending.pop();
        let entries;
        try {
...
export function renderSkippedDirectoryDisclosure(hits) {
    const disclosed = new Set(DISCLOSED_SKIPPED_DIRECTORIES);
...
function listHoists(ts, list) {
    ...
    const blockScoped = nodeFlags.Let | nodeFlags.Const | (nodeFlags.Using ?? 0) | (nodeFlags.AwaitUsing ?? 0);
 ...
 * comparison.
 */
export function canonicaliseHeadSegment(dottedPath, renames, scope = null) {
    if (dottedPath === null)
        return dottedPath;
    const segments = dottedPath.split(".");
 ...
 * into a spec file.
 */
const ASSERTION_HEADS = Object.freeze(["expect", "assert"]);
/**
 ...
 */
function canonicalAssertionHead(dottedPath) {
    if (dottedPath === null)
        return null;
...
export function findBannedConstructs(ts, sf, relPath, ctx) {
    const findings = [];
    ...
    const visit = (node) => {
        if (ts.isCallExpression(node)) {
            ...
            const spelled = canonicaliseHeadSegment(calleeDottedPath(ts, node.expression), renames, scope);
            ...
            const banPath = operand === "identity"
                ? identity.path
                : operand === "spelled"
                    ? spelled
                    : null;
            ...
            if (isBannedModifierCall(banPath, chainEnabledOptionKeys(ts, node))) {
                ...
                if (!reportedModifierChains.has(chainKey)) {
                    ...
                    findings.push({
                        pos,
                        text: `${relPath}:${lineOf(pos)}: banned modifier call — \`${banPath}\` decides which ` +
 ...
 */
export function analyzeSpecs(repoRoot, specRelPaths, ts, ctx, readFile = defaultReadFile) {
    ...
    for (const rel of specRelPaths) {
        ...
        try {
            ...
            errors.push(`The UAT spec ${rel} could not be PARSED (${describeCause(cause)}); the parser itself faulted on this file, so no verdict is reported for it.`);
            continue;
        }
        try {
            ...
            errors.push(`The UAT spec ${rel} could not have its parse INSPECTED (${describeCause(cause)}); the diagnostics this file produced could not be read, so no verdict is reported for it.`);
            continue;
        }
        try {
            ...
            if (analysed === undefined) {
                ...
                errors.push(`The UAT spec ${rel} could not be PARSED (${fault ?? "the program did not include it"}); the parser itself faulted on this file, so no verdict is reported for it.`);
                continue;
            }
            specFindings = findBannedConstructs(ts, analysed, rel, ctx);
        }
        catch (cause) {
            errors.push(`The UAT spec ${rel} could not be analysed (${describeCause(cause)}); the check was NOT performed for that file, so this run covers less than the derived set.`);
            continue;
        }
 ...
 */
export const PROCESS_BOUNDARY_MARKER = "UAT spec integrity: the runnable could not complete";
/**
...
export function main(argv, deps = {}) {
    const out = deps.out ?? ((s) => {
        process.stdout.write(s);
    });
    const err = deps.err ?? ((s) => {
        process.stderr.write(s);
    });
    try {
        ...
        err(`${PROCESS_BOUNDARY_MARKER} (${cause instanceof Error ? cause.message : String(cause)}); ` +
            `the check was NOT performed, so nothing is claimed about the specs.\n`);
        return 2;
    }
}
function runMain(argv, deps, out, err) {
