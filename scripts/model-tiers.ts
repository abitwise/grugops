 ...
 */
export function resolvedPresetLine(preset: PresetName): string {
 ...
 * to close.
 */
function anchoredValuesIn(output: string, prefix: string): string[] {
  const found: string[] = [];
 ...
 */
export function mirroredResolvedPresetLine(preset: PresetName): string {
...
export function mirroredResolvedPresetsIn(output: string): string[] {
  return anchoredValuesIn(output, MIRRORED_RESOLVED_PRESET_PREFIX);
}

// ── The RESOLVED-ASSIGNMENT LINE — the resolution's OUTPUT, not one of its inputs (CR-01) ─────────
//
// `resolveModels` takes TWO inputs — a preset AND a sparse override map — and the preset line
// describes only the first. A run carrying `{"models":{"roles":{...}}}` and no `preset` key
// therefore announced the zero-config answer while emitting adapters that were not the zero-config
// output, and every gate in the repository stayed green. This grammar closes that by announcing
...
export type ResolvedAssignmentResult =
  | { readonly ok: true; readonly value: ResolvedAssignment }
  | { readonly ok: false; readonly reason: string };
...
export interface ResolveModelsOptions {
  ...
  /**
   * The ALREADY-VALIDATED sparse per-role overrides, keyed by role stem. Applied ON TOP of the
...
export function resolveModels(
  ...
  if (preset === "tiered") {
    ...
    for (const [stem, alias] of inheritForEveryStem(sorted)) value.set(stem, alias);
  }
 ...
 */
export function inheritForEveryStem(stems: readonly string[]): ReadonlyMap<string, ModelAlias> {
  const value = new Map<string, ModelAlias>();
  ...
  if (rawPreset !== undefined) {
    if (!isPresetName(rawPreset)) {
      return {
        ok: false,
        reason:
          `model-tiers: ${path} sets \`models.preset\` to ${quoteValue(rawPreset)}, which is not a ` +
...
export function readModelsConfig(
  ...
  if (stems.length === 0) {
    return {
      ok: false,
      reason:
        "model-tiers: the derived stem set handed to readModelsConfig is EMPTY — refusing to " +
  ...
  for (const path of candidates) {
    ...
    try {
      ...
      return {
        ok: false,
        reason:
          `model-tiers: ${path} is present but could not be parsed as JSON — ` +
    ...
    if (parsed === null || typeof parsed !== "object" || Array.isArray(parsed)) {
      return {
        ok: false,
        reason:
          `model-tiers: ${path} parses, but the configuration file as a whole is ` +
    ...
    if (models === undefined) return zeroConfigModels(path);
    ...
    if (models === null || typeof models !== "object" || Array.isArray(models)) {
      return {
        ok: false,
        reason:
          `model-tiers: the \`models\` key in ${path} is ${describeShape(models)} rather than a ` +
    ...
    return readModelsBlock(models as Record<string, unknown>, path, stems);
  }
