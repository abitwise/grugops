...
import { mkdtempSync, mkdirSync, cpSync, rmSync, readFileSync, readdirSync, realpathSync, existsSync, } from "node:fs";
...
if (!existsSync(CONTEXT_ROOT)) {
    cleanup();
    console.log("Context fresh: no .grugops/context/ tree exists yet — nothing committed to drift (vacuous pass).");
    process.exit(0);
}
...
if (taskDirs.length === 0) {
    cleanup();
    console.log("Context fresh: .grugops/context/ exists but holds no per-task dirs — nothing to check (vacuous pass).");
    process.exit(0);
}
...
const derivedNames = ["index.md", "index.jsonl"];
for (const task of taskDirs) {
    ...
    const r = spawnSync("node", [mirroredRender, "render", task, mirroredContextRoot], { encoding: "utf8" });
    ...
    if (r.status !== 0) {
        ...
        console.log(`Context freshness check FAILED: the render did not run cleanly for task "${task}" — ` +
    ...
    for (const name of derivedNames) {
        ...
        try {
            ...
            cleanup();
            console.log(`Context freshness check FAILED: ${relPath} could not be read — ` +
        ...
        if (!committed.equals(rebuilt)) {
            cleanup();
            console.log(`STALE: ${relPath} — committed index differs from a fresh regeneration from notes/. ` +
...
cleanup();
console.log("Context fresh: every committed index.{md,jsonl} matches a fresh regeneration from notes/.");
process.exit(0);
