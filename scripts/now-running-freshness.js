...
// Wrapped in main() and gated behind the same import.meta-vs-argv check the siblings use (claim.ts
// `isMain`, check-uat-oracles.ts `isEntry`). Importing the module (e.g. a future aggregator folding
// it in, or a unit test of a helper) must NOT execute the gate or call process.exit — every
// side-effecting statement (mkdtempSync, cpSync, spawnSync, the exits) lives inside main().
function main() {
    // The queue root to check: the real .grugops/queue, or a CHECK_ROOT override (the oracle's hermetic
    // fixture root) pointing at a sibling .grugops/queue tree. Read at call time so an importer that sets
    ...
    if (!existsSync(claimedDir)) {
        cleanup();
        console.log("Now-running fresh: no .grugops/queue/claimed/ tree exists yet — nothing committed to drift (vacuous pass).");
        process.exit(0);
    }
    ...
    if (r.status !== 0) {
        ...
        console.log("Now-running freshness check FAILED: the render did not run cleanly — " +
    ...
    try {
        ...
        cleanup();
        console.log(`Now-running freshness check FAILED: ${relPath} could not be read — ` +
    ...
    if (!committed.equals(rebuilt)) {
        cleanup();
        console.log(`STALE: ${relPath} — the committed render differs from a fresh regeneration from the claim registry. ` +
            "The claimed/ registry is the source of truth; run `node scripts/claim.js now-running` and commit the result.");
        process.exit(1);
    }
    cleanup();
    console.log("Now-running fresh: the committed .grugops/queue/now-running.md matches a fresh regeneration from the claim registry.");
    process.exit(0);
}
