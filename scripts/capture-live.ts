 ...
 */
export function editAnchor(realPath: string, separator: string = sep): string {
  ...
  if (!posix.startsWith("/") || posix.startsWith("//")) {
    throw new Error(
      "capture-live.editAnchor: refusing a path that is neither POSIX-absolute nor win32 drive-absolute " +
...
/** The two scratch classes (hard rule 3): targets and kit homes, and the runner-owned transcript directories. */
export interface ScratchRegistry {
  targets: string[];
  transcripts: string[];
}

const SCRATCH: ScratchRegistry = { targets: [], transcripts: [] };

/** A target or kit-home scratch directory, registered in the TARGETS class. */
export function makeScratch(suffix: string, registry: ScratchRegistry = SCRATCH): string {
...
export function makeScratchTranscript(label: string, registry: ScratchRegistry = SCRATCH): string {
 ...
 */
export function cleanupPlan(code: number, keepTarget: boolean): CleanupPlan {
 ...
 */
export function cleanupScratch(plan: CleanupPlan, registry: ScratchRegistry = SCRATCH): string[] {
  if (plan.removeTargets) {
    for (const d of registry.targets) rmSync(d, { recursive: true, force: true });
    registry.targets.length = 0;
  }
  if (plan.removeTranscripts) {
    for (const d of registry.transcripts) rmSync(d, { recursive: true, force: true });
...
function outValue(v: string): string {
  if (v.startsWith("--")) fail(`--out was given \`${v}\`, which is a flag, not a directory path — write --out <dir> or --out=<dir>`);
  return v;
}
...
export function parseArgs(argv: readonly string[]): Options {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (a === "--out") opts.out = outValue(argv[++i] ?? "");
    else if (a.startsWith("--out=")) opts.out = outValue(a.slice("--out=".length));
    else {
      fail(
...
const GRANT_FIELDS = ["granted", "adapterNames", "coordinator", "prefix"] as const;
 ...
 */
export function grantDriftFields(before: GrantDerivation, after: GrantDerivation): string[] {
  const out: string[] = [];
  for (const f of GRANT_FIELDS) {
    if (JSON.stringify(before[f]) !== JSON.stringify(after[f])) out.push(f);
  }
  return out.sort();
}
...
export interface NoteRoute {
  ...
  /**
   * WRITE-SHAPED tool-use blocks that name the context root without being a direct path write: a
   * `Bash` COMMAND that names it and carries a write-shaped token (`WRITE_SHAPED_COMMAND_WORDS`, a
   * redirection, an in-place edit), or a file-writing tool that writes a SCRIPT whose content names it
   * (a script that will write there when run). An incidental read (`ls`, `cat`), a `description`
   * leaf and a prose mention in a non-script are not counted (33-REVIEW round 3 WR-02).
   */
  indirectContextWrites: number;
}
 ...
 */
function inputStrings(input: unknown): { paths: string[]; all: string[] } {
  const paths: string[] = [];
  const all: string[] = [];
  const walk = (value: unknown, key: string | null): void => {
    if (typeof value === "string") {
      all.push(value);
      if (key !== null && /path$/i.test(key)) paths.push(value);
...
export function noteRoute(frames: readonly StreamFrame[]): NoteRoute {
  ...
  const isScriptPath = (p: string): boolean => {
  ...
  for (const frame of frames) {
    for (const block of contentBlocks(frame)) {
      ...
      if (block.name === "Bash") {
        ...
        if (typeof command === "string" && namesRoot(command) && isWriteShapedCommand(command)) indirectContextWrites += 1;
        continue;
      }
      ...
      if (isScript && all.some(namesRoot)) indirectContextWrites += 1;
    }
  }
 ...
 */
export function runCommandBuffered(command: string, args: readonly string[], cwd: string, env: NodeJS.ProcessEnv, transcriptPath: string, boundMs: number): Promise<PlatformRunResult> {
...
/** The install decision, pure (33-REVIEW CR-05): `ok: false` carries the reason the caller throws with. */
...
function pluginInstall(target: string, pluginName: string, marketplaceName: string): string {
  ...
  if (!o.ok) fail(`plugin install ${pluginName}@${marketplaceName} did not complete (${o.reason})`);
  return `installed ${pluginName}@${marketplaceName} at local scope: ${o.line}`;
}

/**
 * The uninstall's outcome, RECORDED rather than swallowed (33-REVIEW WR-06): the platform's exit
 * status, the spawn error's message if the platform could not be started, and the trimmed output.
 * Best effort applies to the RESULT — a failed uninstall is named in the Run table, never thrown,
 * so a cleanup failure cannot hide a paid transcript — but never to the record of it.
 */
...
export function uninstallLine(u: UninstallOutcome): string {
  if (u.error !== null) return `error: ${u.error}${u.detail === "" ? "" : ` — ${u.detail}`}`;
  if (u.status === 0) return "exit 0";
...
export const LIVE_OPS: Readonly<LiveOps> = Object.freeze({ pluginInstall, runPlatform, pluginUninstall });

export interface RunSpec {
  request: string;
  /** The resolved `--allowedTools` list — `liveAllowedTools(target)` on the live path. */
  allowedTools: readonly string[];
  /**
   * The grant `capture()` derived from target A before the loop, or null when no cross-target
   * agreement is asserted. A target whose pre-spawn derivation differs on any derived field is a
   * refusal BEFORE the spawn — no token is spent against a grant that is not the one under test.
   */
  expectedGrant: GrantDerivation | null;
...
export interface TargetRun extends RunReport {
  ...
  transcriptText: string;
  /** The grant derived BEFORE the spawn — the value every CAP-03 side was scored against. */
  grant: GrantDerivation;
  installLine: string;
 ...
 */
export async function runTarget(build: TargetBuild, run: RunSpec, ops: Readonly<LiveOps> = LIVE_OPS): Promise<TargetRun> {
  ...
  try {
    ...
    if (grantDrift.length > 0) failed = true;
    scored = {
      label: build.label,
      transcriptName,
...
export function pluginCachePathAccepted(candidate: string, cacheRoot: string): string | null {
  ...
  if (isOutsideTargets(real, [realRoot])) return null;
  return real;
}
 ...
 */
export function contentDigest(root: string, relPaths: readonly string[]): string {
  ...
  for (const rel of sorted) {
    ...
    try {
      ...
      if (statSync(full).isFile()) bytes = readFileSync(full);
    } catch {
      bytes = null;
    }
...
export type ProvenanceState = PreconditionState;
...
export function provenanceVerdict(checkoutDigest: string | null, installedDigest: string | null): { state: ProvenanceState; detail: string } {
  if (checkoutDigest === null) return { state: "UNKNOWN - verify", detail: "the checkout digest could not be derived (the tracked-file list was unreadable)" };
  if (installedDigest === null) return { state: "UNKNOWN - verify", detail: "the installed copy's digest could not be derived" };
  if (checkoutDigest === installedDigest) return { state: "MET", detail: `the installed copy's content digest ${installedDigest} equals the checkout's` };
...
export interface ProvenanceReport {
  ...
  trackedCount: number;
  /** The `plugin under test per system/init` row's value. */
  pluginLine: string;
}
...
export interface RunReport {
  ...
  /** The `--allowedTools` list the subject held, stated in the Run table (CR-01 round 2). */
  toolGrant: readonly string[];
  /**
   * The derived-grant fields that changed between the pre-spawn and the post-run derivation, sorted.
   * Empty means none changed; non-empty means the run is failed and the row names the fields.
   */
  grantDrift: string[];
...
async function runAll(): Promise<void> {
  ...
  try {
    ...
    for (const p of cleanupScratch(cleanupPlan(code, keepTarget))) console.log(`transcript scratch preserved (exit ${code}): ${p}`);
  }
