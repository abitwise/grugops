 ...
 */
export function reportMeasured(m, emit, render) {
    // (1) VACUITY FLOOR — element level. A zero-element run can never print PASS.
    if (m.visited === 0) {
        emit.fail(`${m.label}: ZERO elements visited (expected ${m.expected}) — this check was NOT performed. ` +
    ...
    if (m.visited !== m.expected) {
        emit.fail(`${m.label}: visited ${m.visited} of ${m.expected} elements — the scan set is short, ` +
