...
export function parseTicketDocument(text) {
    ...
    for (let i = 1; i < close; i++) {
        ...
        if (m === null) {
            return ticketRefusal("unrecognized-line", `line ${i + 1} is \`${visible(line.slice(0, 60))}\`, which is neither \`key: value\` nor \`key:\``);
        }
        const key = m[1];
    ...
    switch (presence.kind) {
        ...
        case "admitted-under-another-id":
            ...
            if (presence.joinedStem === id) {
                return (`plans/tickets/${id}.md exists and declares the identifier ${presence.declaredId}, ` +
