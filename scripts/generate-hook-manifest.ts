...
export function deciderEntries(root: string = ROOT): readonly string[] {
 ...
 */
export function deriveManifest(root: string = ROOT): Readonly<Record<string, Record<string, string>>> {
  const out: Record<string, Record<string, string>> = {};
  for (const entry of deciderEntries(root)) {
    const files = new Set<string>([entry, ...jsImportClosure(root, entry)]);
...
export function renderManifestRegion(
  manifest: Readonly<Record<string, Record<string, string>>>,
): string {
  const lines: string[] = [];
...
export function normalizeManifestRegion(src: string): string {
  const a = src.indexOf(MANIFEST_OPEN);
  const b = src.indexOf(MANIFEST_CLOSE);
  if (a === -1 || b === -1) return src;
  return src.slice(0, a) + "<MANIFEST REGION>" + src.slice(b + MANIFEST_CLOSE.length);
}

export function rewriteHookEntry(root: string = ROOT): { changed: boolean; text: string } {
...
if (isEntry) {
  ...
  process.stdout.write(
    `Wrote hooks/hook-entry.ts manifest — ${Object.keys(m).length} decider(s), ${n} module hash(es)` +
      `${changed ? "" : " (unchanged)"}.\n`,
  );
  process.exit(0);
}
