...
cpSync(
  join(SCRIPT_ROOT, "scripts", "generate-skill-twins.js"),
  join(tmp, "scripts", "generate-skill-twins.js"),
);
cpSync(
...
const r = spawnSync(
  "node",
  [join(tmp, "scripts", "generate-skill-twins.js"), OVERWRITE_FLAG],
...
if (r.status !== 0) {
  ...
  die(
    "Skill-twin freshness check FAILED: the generator did not run cleanly — refusing to report the twins as fresh.",
  );
}
...
const listTwins = (root: string, what: string): string[] => {
  try {
    return listSkillAdapters(root);
  } catch (e) {
    die(
      `Skill-twin freshness check FAILED: cannot read the ${what} skill directory ${join(root, TWIN_DIR)} — ${e instanceof Error ? e.message : String(e)}\nRun \`${REGEN_CMD}\` and commit the result.`,
    );
  }
};
...
if (rebuiltNames.length === 0) {
  die(
    `Skill-twin freshness check FAILED: the generator ran cleanly but produced no twins in ${TWIN_DIR} — refusing to report an empty regeneration as fresh.`,
  );
}
...
for (const name of committedNames) {
  ...
  try {
    ...
    die(
      `Skill-twin freshness check FAILED: ${TWIN_DIR}/${name} could not be read — run \`${REGEN_CMD}\` and commit it.`,
    );
  }
...
if (differing.length > 0) {
  console.log(
    `STALE: ${differing.length} of ${committedNames.length} committed skill twin(s) differ from a fresh regeneration: ${differing.join(", ")}\nRun \`${REGEN_CMD}\` and commit the result.`,
  );
  process.exit(1);
}

console.log(
  `Skill twins fresh: ${committedNames.length} twin(s) compared in ${TWIN_DIR}, 0 byte difference(s), directory listings set-equal.`,
);
process.exit(0);
