...
export const BUILD_PARITY_REMEDY =
  "Remedy: run `npm run build` and commit the result, so the committed .js is a build of its .ts.";
 ...
 */
export function trackedBuildOutputs(root: string = ROOT): readonly string[] {
 ...
 */
export function movedBuildOutputs(root: string = ROOT): readonly string[] {
  return lines(git(root, ["diff", "--name-only", "--", "*.js"]));
}

export interface BuildParityResult {
  /** A NAMED cause when the build did not complete, or `null` when it did. Never an empty result. */
  readonly buildFailure: string | null;
  /** What the check examined and what it found — the measurement the PASS line carries. */
  readonly measured: Measured<string>;
}
 ...
 * ran.
 */
export function buildParity(root: string = ROOT): BuildParityResult {
  const tracked = trackedBuildOutputs(root);
  const nothingStated = (buildFailure: string): BuildParityResult => ({
    buildFailure,
  ...
  try {
    ...
    return nothingStated(
      "the compiler could not be located from this checkout " +
  ...
  if (build.error !== undefined || build.status !== 0) {
    ...
    const detail = [`${build.stdout ?? ""}${build.stderr ?? ""}`.trim(), build.error?.message ?? ""]
      ...
      .trim();
    return nothingStated(
      "the build did not complete, so this check states nothing about the build outputs" +
        (detail === "" ? "" : `:\n${detail}`),
    );
  }
  ...
  for (const rel of tracked) {
    visited += 1;
    if (moved.has(rel)) findings.push(rel);
  }
...
if (isEntry) {
  ...
  const fails = reportMeasured(
    ...
    (rel) =>
      `    ${rel} moved when the build ran — the committed build output is not a build of the ` +
