...
let citedCopied = 0;
{
  ...
  for (const line of registerText.split("\n")) {
    for (const rel of citedResidualPaths(line)) cited.add(rel);
  }
  for (const rel of cited) {
    const src = join(ROOT, rel);
    if (!existsSync(src)) continue; // the gate itself reports a missing one; the mirror does not hide it
    const dst = join(tmp, rel);
    mkdirSync(dirname(dst), { recursive: true });
  ...
  if (citedCopied === 0) {
    cleanupAndRefuse(
      `the register cites no existing file path, so the generator's residual-path gate would pass ` +
