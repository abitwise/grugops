...
if (GUARANTEES_DATA_SOURCES.length !== GUARANTEES_DATA_SOURCE_COUNT) {
    refuse(`the generator declares ${GUARANTEES_DATA_SOURCES.length} data source(s) where ` +
...
try {
    ...
    refuse(`the temp mirror directory could not be created under ${tmpdir()} ` +
        `(${e.message}) — refusing to report the guarantees page as fresh without ` +
        `regenerating it.`);
}
function cleanup() {
    rmSync(tmp, { recursive: true, force: true });
}
function cleanupAndRefuse(message) {
    cleanup();
    refuse(message);
}
...
try {
    ...
    cleanupAndRefuse(`the import closure of ${GUARANTEES_ENTRY_JS} could not be derived ` +
        `(${e.message}) — refusing to mirror a partial module graph.`);
}
...
for (const rel of GUARANTEES_DATA_SOURCES) {
    ...
    mkdirSync(dirname(dst), { recursive: true });
    cpSync(src, dst);
    if (rel.endsWith("factory.config.json"))
        configsCopied += 1;
}
if (configsCopied === 0) {
    cleanupAndRefuse(`no factory config was found at any of the declared candidates ` +
...
if (r.status !== 0) {
    ...
    cleanupAndRefuse("the generator did not run cleanly — refusing to report the guarantees page as fresh.");
}
...
try {
    ...
    cleanupAndRefuse(`${toPosix(OUT)} could not be read — run \`${REGEN_COMMAND}\` and commit it. A missing page is ` +
        `not a fresh one.`);
}
let rebuilt;
try {
    ...
    cleanupAndRefuse(`the mirrored regeneration wrote nothing to ${toPosix(OUT)} — refusing to report the ` +
        `guarantees page as fresh against a file that was never produced.`);
}
cleanup();
if (!committed.equals(rebuilt)) {
    console.log(`STALE: ${toPosix(OUT)} — the committed page differs from a fresh regeneration. Run ` +
