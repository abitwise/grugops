...
import { readRegister, readRegistry, isBlank, CLAIM_KINDS, REGISTER_PATH, REGISTRY_PATH, RESIDUAL_PATH, } from "./audit-model.js";
 ...
 */
export const REGISTRY_ARM_NON_MARKDOWN = [
    {
        file: ".claude-plugin/plugin.json",
        why: "the Claude Code plugin manifest — a public distribution surface whose `description` is " +
 ...
 */
export function registryArmFindings(i) {
    const findings = [];
    ...
    if (armFiles.length === 0) {
        findings.push(`equality four (the registry arm is EMPTY): ${REGISTRY_PATH} carries ZERO \`kind: safety\` ` +
    ...
    if (strays.length > 0) {
        findings.push(`equality four (safety claim NOT vouched for): ${strays.length} \`kind: safety\` claim(s) ` +
    ...
    if (undeclared.length > 0) {
        findings.push(`equality four (undeclared non-markdown safety claim): ${undeclared.length} \`kind: safety\` ` +
    ...
    for (const c of i.cardinality)
        declaredCounts.set(c.kind, c.count);
    ...
    if (uncovered.length > 0) {
        findings.push(`equality four (the declared kind map omits a legal kind): CLAIM_KIND_CARDINALITY names no ` +
    ...
    const disagreeing = kinds
        ...
        .map((k) => `${k} declares ${declaredCounts.get(k) ?? 0} but the registry carries ` +
        `${derivedCounts.get(k) ?? 0}`);
    if (disagreeing.length > 0) {
        findings.push(`equality four (kind cardinality): ${disagreeing.length} claim kind(s) disagree with the ` +
    ...
    if (declaredSum !== i.claims.length) {
        findings.push(`equality four (the declared kind map is SHORT): CLAIM_KIND_CARDINALITY sums to ` +
...
function checkResidualCitedPaths() {
    ...
    for (const raw of text.split("\n")) {
        ...
        for (const path of citedResidualPaths(line)) {
            cited += 1;
            if (!tracked.has(path)) {
                refusals.push(`residual row ${first} cites \`${path}\`, which is not a TRACKED file in this repository — ` +
    ...
    pass(`residual citations: ${cited} path claim(s) across ${scanned} published row(s), every one a ` +
...
function runAll() {
    ...
    if (missingOnDisk.length > 0) {
        fail(`${missingOnDisk.length} register row(s) name a file that is not on disk — ` +
    ...
    if (uncountedPaths.join("\n") !== expectedUncounted.join("\n")) {
        fail(`the register's uncounted rows are [${uncountedPaths.join(", ")}], expected exactly ` +
    ...
    try {
        ...
        fail(`${REGISTRY_PATH} could not be parsed, so the REGISTRY arm of the D-18 union could not be ` +
            `measured and NO verdict is reported over it — ${e.message}`);
    }
    ...
    const safetyClaimCount = registryClaims === null ? 0 : registryClaims.filter((c) => c.kind === "safety").length;
    const registryArmFiles = registryClaims === null
        ? []
