...
function readAdapter(targetRoot: string, rel: string): Adapter {
  ...
  if (!parsed.ok) {
    return fail(
      `the installed adapter ${rel} has frontmatter that is NOT in the canonical form [${parsed.code}] — ${parsed.reason}. An unreadable adapter is not the same fact as an adapter without a marker, so this precheck refuses rather than assuming.`,
    );
  }
...
function observeTarget(targetRoot: string): string[] {
  ...
  const coordinators = adapters.filter((a) =>
    admittedKeyHasValue(a.keys, "coordinator", "true"),
  );
