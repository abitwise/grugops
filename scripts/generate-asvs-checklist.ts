...
// ── A single ASVS requirement row from the vendored flat.json ─────────────────────────────────
...
const requirements = (data as { requirements?: unknown }).requirements as
  | AsvsRequirement[]
  | undefined;
...
const count = (lvl: string): number => requirements!.filter((r) => r.L === lvl).length;
const l1 = count("1");
