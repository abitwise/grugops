...
export function createLoop(options, io, deps) {
     ...
     */
    const watchErrorsByDir = new Map();
     ...
     * catch.
     */
    function noteWatchState(rel, source, message) {
     ...
     */
    function currentWatchErrors() {
        return [...watchErrorsByDir.keys()].sort().map((rel) => watchErrorsByDir.get(rel));
    }
    ...
    function emit(result) {
        ...
        const withWatch = noted.length === 0 ? result : { ...result, readErrors: [...result.readErrors, ...noted] };
 ...
 */
export function handleInterrupt(loop, exit) {
    loop.stop();
    exit(0);
}
