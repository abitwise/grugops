...
import { readRegistry, isBlank, SAFETY_FLOORS, DISPOSITIONS, MARKDOWN_SUFFIX, CLAIM_ANCHOR_RE, anchoredDocs, scanAnchoredDocument, anchoredBlockAt, } from "./audit-model.js";
// (Plan 30-09, D-18) THE DROPPED-CLAIM AUTHORITY. A dropped row's anchored text is not a string
// stored in the registry — it is GENERATED, and this gate compares against the generated bytes so
// the published prose and the mechanism cannot drift apart.
//
// WHY IMPORTING THIS IS NOT THE HAZARD THIS FILE'S HEADER WARNS ABOUT. The warning is about a gate
// importing a GATE, because a gate carries a refusal CHANNEL — an exit code — along with its value.
// scripts/generate-guarantees.ts is a GENERATOR: it THROWS, its writes sit behind an `isEntry`
// guard, and it exits nothing on import. It is the same throw-versus-report split as audit-model,
// and every refusal it raises is REPORTED below through this gate's own fail().
import { disclosureFor, dropConsistencyRefusals, guaranteesJoin, } from "./generate-guarantees.js";
...
function main() {
    ...
    // ── (Plan 30-09, D-18) THE LIVE JOIN, FOR THE DROPPED ROWS ────────────────────────────────
    //
    // A dropped row's anchored text is a function of the LIVE checkpoint matrix, so this gate needs
    // the join the render computes. It asks the ONE authority rather than resolving the matrix again:
    // a second resolution here would be a second opinion about which floors are lowered, and the two
    // published documents would then be able to disagree about one tree.
    //
    // A JOIN THAT CANNOT BE COMPUTED IS REPORTED, NEVER SWALLOWED. If it throws, `dropped` rows below
    // are refused by name rather than compared against their stored verbatim — comparing a dropped row
    // against the text it no longer carries would report a drift for a claim that was correctly
    // replaced, and comparing it against nothing would pass.
    let joinById = null;
    try {
        const joined = guaranteesJoin(ROOT);
        joinById = new Map(joined.map((r) => [r.claimId, r]));
        for (const refusal of dropConsistencyRefusals(joined)) {
            fail(`the registry's dropped rows and the live checkpoint matrix disagree — ${refusal}`);
        }
    }
    catch (e) {
        // Only reported as a failure when a dropped row actually needs it. A tree with no dropped row
        // must not go red because the guarantees join could not be built — that is the RENDER's
        // refusal to raise, and raising it twice would train a reader to ignore one of them.
        if (claims.some((c) => c.status === "dropped")) {
            fail(`the registry carries dropped row(s) and the guarantees join could not be built, so their ` +
                `generated disclosure text is unavailable and NO dropped row below was compared — ` +
                `${e.message}`);
        }
    }
    ...
    for (const doc of docs) {
        ...
        for (const claim of claims.filter((c) => c.file === doc).slice().sort((a, b) => (a.id < b.id ? -1 : 1))) {
            ...
            // ── (Plan 30-09, D-18) WHICH TEXT THIS ROW IS FROZEN AGAINST ─────────────────────────
            //
            // TWO COMPARISONS, WRITTEN AS TWO BRANCHES. They ask different questions: a standing row is
            // frozen against the sentence the registry RECORDS, and a dropped row is frozen against the
            // disclosure the mechanism GENERATES. Folding them into one loosened comparison would be a
            // single predicate answering two questions, which is how a carve-out comes to admit the case
            // it was never written about.
            const dropped = claim.status === "dropped";
            let expected = claim.verbatim;
            if (dropped) {
                const joined = joinById?.get(claim.id);
                if (joined === undefined) {
                    if (joinById !== null) {
                        fail(`${doc}: ${claim.id} is \`status: dropped\` but is not a \`kind: safety\` row, so it ` +
                            `rests on no floor and the guarantees join never produced it. A row that depends on ` +
                            `nothing cannot be dropped by a lowering: only a safety row has a floor to lower`);
                    }
                    continue; // the join failed to build; already reported once, above
                }
                try {
                    expected = disclosureFor(joined);
                }
                catch (e) {
                    fail(`${doc}: ${claim.id}'s disclosure text could not be generated — ${e.message}`);
                    continue;
                }
            }
            let block;
            try {
                block = anchoredBlockAt(scan, anchor, expected);
            }
            catch (e) {
            ...
            comparisons += 1;
            if (!block.matches) {
                // ── DELETION IS ITS OWN FAILURE, DISTINCT FROM DRIFT (plan 30-09, D-18) ────────────
                //
                // The two used to produce one message, and the difference matters: DELETION is the
                // prohibited act — D-18 forbids removing or striking through a claim — while DRIFT is a
                // stale regeneration or an un-companioned prose edit, which is ordinary and is fixed by
                // updating the registry row in the same commit. A reader sent to the drift remedy for a
                // deletion is being told to record what someone removed rather than to put it back.
                //
                // THE PREDICATE, AND WHAT BOUNDS IT. Deletion is decided on the FIRST line of the extent
                // alone: the claim begins on the line immediately below its anchor, so an anchor whose next
                // line is blank — or is the NEXT ANCHOR — stands over nothing. Both halves are asked of the
                // authorities this module already consults (`isBlank`, `CLAIM_ANCHOR_RE`); no third
                // spelling is introduced. LIVE MEASUREMENT taken before the rule was added: 0 of 47 registry
                // rows carry a verbatim whose first line is blank, so the rule has no false-positive subject
                // on this tree. STATED BOUND: a PARTIAL deletion that leaves the first line in place reads
                // as DRIFT, deliberately — a region with content still under it is not a region that stands
                // over nothing, and widening the predicate to guess at intent is the class this repository
                // has paid for eleven times.
                const deleted = !isBlank(block.verbatimFirstLine) &&
                    (isBlank(block.documentFirstLine) || CLAIM_ANCHOR_RE.test(block.documentFirstLine));
                if (deleted) {
                    fail(`${doc}: the anchored region for ${claim.id} at line ${block.start + 1} has been DELETED, ` +
                        `not changed — its anchor stands over ${JSON.stringify(block.documentFirstLine)}, which is blank ` +
                        `or is the next anchor.\n` +
                        `        expected (${block.verbatimBytes} byte(s)): ${JSON.stringify(expected)}\n` +
                        `        This is a DIFFERENT failure from a drifted region, and it has a different ` +
                        `remedy. D-18 forbids deleting, striking through or quietly rewording a registered ` +
                        `claim: the anchor and its row survive so the record shows what was claimed. If the ` +
                        `floor under this claim was lowered, set the row to \`status: dropped\` and write the ` +
                        `GENERATED disclosure at this anchor. Otherwise put the sentence back`);
                }
                else if (dropped) {
                    fail(`${doc}: the text at ${claim.id}'s anchor (line ${block.start + 1}) is not byte-identical to ` +
                        `the GENERATED disclosure for this dropped row.\n` +
                        `        generated (${block.verbatimBytes} byte(s)): ${JSON.stringify(expected)}\n` +
                        `        document  (${block.documentBytes} byte(s)): ${JSON.stringify(block.text)}\n` +
                        `        A dropped claim's replacement text is NOT a sentence anybody writes: it is ` +
                        `produced from the registry row and the live checkpoint matrix, so a hand-written ` +
                        `substitute — however plausible — is refused here. Paste the generated bytes above at ` +
                        `the anchor, exactly`);
                }
                else {
    ...
    for (const claim of claims) {
        ...
        if (claim.status === "true")
            continue;
        // (Plan 30-09, D-18) A `dropped` ROW IS EXEMPT FROM THE OBLIGATIONS BELOW, AND THE EXEMPTION IS
        // A STRONGER BAR RATHER THAN A WAIVER. Those obligations exist so a claim nobody decided about
        // does not stay STANDING in a shipped document. A dropped row's text has been mechanically
        // replaced by the generated disclosure — verified byte-for-byte a few lines up — so the thing
        // the obligation protects against cannot occur. Reaching `dropped` at all requires a floor
        // lowered in the live configuration AND agreement in both directions, which is refused above.
    ...
    // ── The id sequence ───────────────────────────────────────────────────────────────────────
    // ── (Plan 30-09, D-18) CONTIGUITY IS UNBROKEN BY A DROP — the SAME check, over ALL rows ────
    //
    // The id list below is built from EVERY parsed row, dropped ones included, because a dropped row
    // is not deleted: its id is still in the sequence and this check still holds it. That is the
    // structural half of D-18's no-deletion rule — a drop that removed a row would show up here as a
    // contiguity gap, and a drop that removed an anchor would show up above as a broken bijection.
    // Neither needed a new rule; both needed the existing ones to keep seeing dropped rows.
    ...
    if (FAILS === 0) {
        ...
        const droppedCount = claims.filter((c) => c.status === "dropped").length;
        pass(`${claims.length} registry row(s) parsed from ${headingShaped} claim-heading-shaped line(s), ` +
            ...
            `byte-identical, of which ${droppedCount} compared against GENERATED disclosure text for a ` +
            `\`dropped\` row (D-18: the anchor and the row survive a drop, so the bijection and the id ` +
            `contiguity below still hold over them); all ${SAFETY_FLOORS.length} safety floor(s) mapped`);
    }
    verdict();
}
function verdict() {
