...
if (build.status !== 0) {
  ...
  console.log(
    "Freshness check FAILED: the rebuild did not compile cleanly — refusing to report build outputs as fresh.",
  );
  cleanup();
  process.exit(1);
}

const committed = OUTPUT_DIRS.reduce<string[]>((acc, d) => collectJs(d, acc), []);
...
if (stale.length > 0) {
  ...
  console.log(
    `Freshness check FAILED: ${stale.length} stale build output(s) detected.`,
  );
  process.exit(1);
}

console.log(
  `All build outputs fresh: ${committed.length} committed .js file(s) match a fresh tsc rebuild.`,
);
process.exit(0);
