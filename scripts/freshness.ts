...
// ── git, invoked the way scripts/check-diff-disposition.ts invokes it: read-only, argument vector
//    never a shell string, and a non-zero exit turned into a named refusal rather than a throw. No
//    encoding is set, so stdout is a Buffer — a byte comparison decoded as text is not a byte
//    comparison. ────────────────────────────────────────────────────────────────────────────────
type GitOk = { ok: true; stdout: Buffer };
type GitErr = { ok: false; reason: string };

function git(args: string[]): GitOk | GitErr {
...
function refuse(reason: string): never {
  ...
  console.log(
    "BUILD-OUTPUT CHECK FAILED: the committed side could not be established from git, so this gate states nothing about the build outputs.",
  );
  process.exit(1);
}
...
if (!gitDir.ok) refuse(`this root is not inside a git repository. ${gitDir.reason}`);
...
if (!prefix.ok) refuse(`the repository root could not be located. ${prefix.reason}`);
const prefixText = prefix.stdout.toString("utf8").trim();
if (prefixText !== "") {
  refuse(
    `this root sits at \`${prefixText}\` inside a different git repository, so paths derived from git would not be the paths this gate walks.`,
  );
}

const headRev = git(["rev-parse", "HEAD"]);
if (!headRev.ok) refuse(`HEAD could not be resolved. ${headRev.reason}`);
...
if (!lsTree.ok) refuse(`the committed output set could not be derived. ${lsTree.reason}`);
const headSet = new Set<string>();
let largestCommitted = 0;
for (const record of lsTree.stdout.toString("utf8").split("\0")) {
  if (record === "") continue;
  const tab = record.indexOf("\t");
  if (tab < 0) refuse(`\`git ls-tree -l\` produced a record this gate cannot parse: ${record}`);
  const path = record.slice(tab + 1);
  if (!path.endsWith(".js")) continue;
  headSet.add(path);
  const size = Number(record.slice(0, tab).trim().split(/\s+/)[3]);
  if (!Number.isFinite(size)) refuse(`\`git ls-tree -l\` gave no readable size for ${path}.`);
  if (size > largestCommitted) largestCommitted = size;
}
if (largestCommitted >= GIT_MAX_BUFFER) {
  refuse(
    `the largest committed output is ${largestCommitted} byte(s), which is not below the ${GIT_MAX_BUFFER}-byte git read ceiling. Raise GIT_MAX_BUFFER.`,
  );
}
...
if (!status.ok) refuse(`the modified-source set could not be derived. ${status.reason}`);
...
function collectJs(dir: string, acc: string[]): string[] {
  ...
  for (const entry of readdirSync(abs, { withFileTypes: true })) {
    if (entry.name === "node_modules" || entry.name === ".tmp-build") continue;
...
const walkSet = new Set(OUTPUT_DIRS.reduce<string[]>((acc, d) => collectJs(d, acc), []));
...
try {
  ...
  console.log(
    "BUILD-OUTPUT CHECK FAILED: the compiler could not be located from this checkout " +
...
if (build.error !== undefined || build.status !== 0) {
  ...
  if (build.error !== undefined) {
    console.log(
      `BUILD-OUTPUT CHECK FAILED: the compiler could not be launched (${build.error.message}), ` +
        "so this gate states nothing about the build outputs.",
    );
  } else {
    console.log(
      "BUILD-OUTPUT CHECK FAILED: the rebuild did not compile cleanly, so this gate states nothing about the build outputs." +
...
const staleCommitted: string[] = [];
const staleWorking: string[] = [];
...
for (const rel of [...headSet].sort()) {
  ...
  if (useWorking) {
    ...
    if (!working.equals(rebuilt)) staleWorking.push(rel);
    continue;
  }
  ...
  if (!blob.stdout.equals(rebuilt)) staleCommitted.push(rel);
}
...
if (armHead + armWorking !== headSet.size) {
  refuse(
    `the arm counts do not account for the compared set: ${armHead} + ${armWorking} != ${headSet.size}.`,
  );
}
...
console.log(
  `Compared ${headSet.size} path(s) derived from \`git ls-tree -r HEAD\` — ${armHead} on the HEAD arm, ${armWorking} on the working-tree arm (uncommitted source); the arms sum to ${armHead + armWorking}.`,
);
console.log(
  `Set equality with the filesystem walk: ${headOnly.length} committed at HEAD and absent on disk, ${workingOnly.length} on disk and absent from HEAD.`,
);
console.log(
  `Largest compared output ${largestCommitted} byte(s); git read ceiling ${GIT_MAX_BUFFER} byte(s).`,
);

for (const rel of staleCommitted) {
  console.log(
    `STALE COMMITTED OUTPUT: ${toPosix(rel)} — the .js committed at HEAD is not a build of the .ts committed beside it. Run \`npm run build\` and commit the result.`,
  );
}
for (const rel of staleWorking) {
  console.log(
    `STALE WORKING OUTPUT: ${toPosix(rel)} — its source is modified or untracked, and the .js in the working tree is not a build of it. Run \`npm run build\` before committing.`,
  );
}
for (const rel of orphaned) {
  console.log(
    `ORPHANED COMMITTED OUTPUT: ${toPosix(rel)} — committed at HEAD with no counterpart in a rebuild. Delete it, or restore the .ts that emits it.`,
  );
}
for (const rel of headOnly) {
  console.log(
    `DELETED COMMITTED OUTPUT: ${toPosix(rel)} — committed at HEAD and absent from the working tree. Restore it, or commit the deletion.`,
  );
}
for (const rel of workingOnly) {
  console.log(
    `UNCOMMITTED BUILD OUTPUT: ${toPosix(rel)} — present on disk under a build-output directory and absent from HEAD. Commit it, or remove it.`,
  );
}

const findings =
  staleCommitted.length + staleWorking.length + orphaned.length + headOnly.length + workingOnly.length;
...
console.log(
  `${FRESH_LINE_PREFIX} ${headSet.size} committed .js file(s) match a rebuild of their sources.`,
);
process.exit(0);
