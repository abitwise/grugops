...
if (isEntry) {
    try {
        ...
        if (changed) {
            console.log("Hook-manifest freshness check FAILED: hooks/hook-entry.ts's manifest is not a derivation of " +
        ...
        console.log(`Hook-manifest freshness check FAILED: ${e.message}`);
        process.exit(1);
    }
}
