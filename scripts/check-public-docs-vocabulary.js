 ...
 */
export function corpusMember(rel, separator = sep) {
...
export function grepSubstringInsensitive(scan, needle) {
...
function guaranteesMembers() {
    if (!existsSync(abs(GUARANTEES_DOC))) {
        DERIVATION_REFUSALS.push(`${GUARANTEES_DOC} is a NAMED member of the public-docs scan set and does not exist at ` +
...
// ---------------------------------------------------------------------------
export const PUBLIC_DOCS_CORPUS_PARTS = [
...
export const PUBLIC_DOCS_SCAN_PARTS = PUBLIC_DOCS_CORPUS_PARTS.map((p) => ({
    name: p.name,
