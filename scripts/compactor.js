...
import { appendNote, admit, currentState, } from "./context-io.js";
...
const GUARDED_KEYS = ["id", "kind", "by", "at", "verified_by", "supersedes"];
function readNoteFields(text) {
    ...
    for (const line of m[1].split("\n")) {
        ...
        if (seen.has(key) && GUARDED_KEYS.includes(key))
            dupes.add(key);
        seen.add(key);
...
export function checkCarveOut(rawThread, promoted, _dial = DEFAULT_DIAL) {
    ...
    for (const id of rawFailedIds) {
        if (!promotedFailedIds.has(id)) {
            findings.push(`carve-out FAIL: failed-attempt "${id}" present in the raw thread was dropped from the ` +
    ...
    for (const [file, fields] of promoted) {
        if (fields.kind === "failed-attempt")
            continue; // FA survival is rule 1 (FA-token, not id)
        if (fields.id === "") {
    ...
    const rawRecords = rawDurable.map(({ fields }) => ({
        id: fields.id,
        kind: fields.kind,
        by: fields.by,
        at: fields.at,
    ...
    for (const { file, fields } of rawDurable) {
        ...
        if (!required)
            continue; // soft, non-verified, folded out raw-side — the one sanctioned removal
        if (fields.id === "") {
            findings.push(`carve-out FAIL: raw ${fields.kind} note "${file}" has no frozen "id" field — a durable raw ` +
