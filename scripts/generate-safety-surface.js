 ...
 * byte-identical.
 */
export function safetySurfaceUnion(root = DEFAULT_ROOT) {
    const byFile = new Map();
    const add = (file, reason) => {
        const existing = byFile.get(file);
        if (existing === undefined)
            byFile.set(file, [reason]);
        else
            existing.push(reason);
    };
    for (const row of readRegister(root).rows) {
        if (row.safetySurface === "yes") {
            add(row.file, `register row flagged \`safety_surface: yes\` (\`kind: ${row.kind}\`)`);
        }
    }
    ...
    return [...byFile.keys()]
        .sort()
        .map((file) => ({ file, reasons: byFile.get(file) }));
}
/**
 ...
 */
export function renderSafetySurface(root = DEFAULT_ROOT) {
    ...
    if (entries.length === 0) {
        throw new Error(`generate-safety-surface: no safety-surface file found in ${REGISTER_PATH} or ` +
