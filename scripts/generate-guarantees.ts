...
export function citedResidualPaths(rowText: string): readonly string[] {
  ...
  for (const m of rowText.matchAll(/`([^`\n]+)`/g)) {
    ...
    if (!/^(?:install|agent-factory|scripts|hooks|docs|\.claude-plugin)\//.test(t)) continue;
    out.push(t.replace(/\/$/, ""));
  }
  return [...new Set(out)];
}
...
export function residualPathRefusals(root: string = DEFAULT_ROOT): readonly string[] {
  ...
  for (const raw of text.split("\n")) {
    ...
    for (const path of citedResidualPaths(line)) {
      ...
      if (!existsSync(join(root, path))) {
        refusals.push(
          `residual row ${first} cites \`${path}\`, which is not in the tree — a published residual ` +
