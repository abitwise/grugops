...
interface NoteFields {
  ...
  // Non-recognized in-fence line shapes (CMP-02 round-5, IN-02 completion). The carve-out fails
  // closed on ANY entry (gate (a)) — an indented / `key : value` provenance line silently projects to
  // "" under the lenient parser and would let the byte-equal loop compare ""==="" on a line whose
  // bytes differ. Sourced from context-io.parseNote.malformedLines, not a local copy.
  malformedLines: string[];
  // The VERBATIM on-disk note bytes (readFileSync result), carried so the carve-out can run the
  // shared validate() (gate (b)) on the exact bytes — never a re-serialization from the parsed
  // scalars, which would silently normalize an indented/CRLF line away before validate() sees it.
  text: string;
}
...
export function checkCarveOut(
  ...
  for (const [file, fields] of rawNotes) {
    for (const line of fields.malformedLines) {
      findings.push(
        `carve-out FAIL: malformed frontmatter line "${line}" in raw thread note "${file}" — an ` +
  ...
  for (const [file, fields] of promotedNotes) {
    for (const line of fields.malformedLines) {
      findings.push(
        `carve-out FAIL: malformed frontmatter line "${line}" in promoted note "${file}" — an ` +
          `indented or "key : value" provenance line silently projects to empty under the lenient ` +
          `parser, so the carve-out cannot trust the byte-equal comparison of that line; fail closed ` +
          `(CMP-02, IN-02).`,
      );
  ...
  for (const [file, fields] of rawNotes) {
    for (const f of validate(fields.text)) {
      findings.push(
        `carve-out FAIL: raw thread note "${file}" is structurally invalid — ${f} (the carve-out ` +
  ...
  for (const [file, fields] of promotedNotes) {
    for (const f of validate(fields.text)) {
      findings.push(
        `carve-out FAIL: promoted note "${file}" is structurally invalid — ${f} (the carve-out runs ` +
  ...
  for (const [file, fields] of promotedNotes) {
    ...
    if (!seal.ok) {
      findings.push(
        `carve-out FAIL: promoted note "${file}" is unsealed (${seal.reason}) — the sanctioned ` +
