...
function fenceRun(line) {
    ...
    if (char !== "`" && char !== "~")
        return null;
    let len = 0;
    while (len < line.length && line[len] === char)
        len += 1;
    if (len < 3)
        return null;
    return { char, len };
 ...
 * ---------------------------------------------------------------------------------------------
 */
export function unfencedHeadingIndex(text, heading) {
...
export function unfencedHeadingNearMisses(text, heading) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (atxOrSetextHeadingText(lines, i, flags) === want)
            at.push(i);
    }
    return at;
}
 ...
 */
export function atxOrSetextHeadingText(lines, i, flags) {
    const line = lines[i];
    if (line === undefined)
        return null;
    ...
    if (under === undefined)
        return null;
    if (flags !== undefined && (flags[i] === true || flags[i + 1] === true))
        return null;
    if (/^[ \t]*$/.test(line) || /^ {0,3}#/.test(line))
        return null;
    if (!/^ {0,3}(?:=+|-+)[ \t]*$/.test(under))
        return null;
...
function renderedText(s) {
    return s
        .replace(NUMERIC_CHARACTER_REFERENCE, (_m, dec, hex) => {
 ...
 * ---------------------------------------------------------------------------------------------
 */
export function unfencedMatchIndices(text, re) {
    ...
    if (re.global || re.sticky) {
        throw new Error(`frontmatter: unfencedMatchIndices refuses the RegExp ${String(re)} — a \`g\`/\`y\` flagged ` +
...
// deletes on sight.
export const BLOCK_INDICATOR = /^[|>][0-9]*[+-]?[ \t]*(?:#.*)?$|^[|>][+-]?[0-9]*[ \t]*(?:#.*)?$/;
...
const blockBreakRun = (lineBreak, breaks) => breaks <= 1 ? lineBreak : "\n".repeat(lineBreak === "\n" ? breaks : breaks - 1);
...
function blockMapImplicitEntry(text) {
...
export function stripComment(s, entering, nodeStartAtOffsetZero, lineStartAtOffsetZero) {
    ...
    let mayBegin = entering.openQuote !== null
        ? false
    ...
    let atLineStructuralStart = entering.openQuote === null && lineStartAtOffsetZero;
    ...
    const separationFollows = (i) => i + 1 >= s.length || s[i + 1] === " " || s[i + 1] === "\t";
        ...
        if (indent > baseIndent) {
            ...
            if (cur.block) {
                ...
                if (inside) {
                    ...
                    const boundaryBreak = moreIndented || cur.blockPrevMoreIndented ? "\n" : cur.blockLineBreak;
