...
export interface CommandSegment {
  /** The first word of the segment — the tool being invoked. */
  readonly tool: string;
  /** EVERY subsequent word that is not a flag. See `commandSegments` for why it is every one. */
  readonly words: readonly string[];
  /** The segment's raw text, for the fail-closed tool-name scan. */
  readonly raw: string;
}
...
export function commandSegments(cmd: string, depth = 0): readonly CommandSegment[] | null {
  ...
  if (depth > 3) return null; // bounded recursion; deeper nesting is untokenizable, i.e. fail closed
  ...
  if (quote !== null) return null; // unbalanced quoting — untokenizable, fail closed
  segments.push({ tool: "", words: [], raw: cur });
 ...
 */
function expandNestedShells(
  segs: readonly CommandSegment[],
  depth: number,
): readonly CommandSegment[] | null {
  const out: CommandSegment[] = [];
  for (const seg of segs) {
    out.push(seg);
    if (!NESTED_SHELLS.has(seg.tool)) continue;
    ...
    if (nested === null) return null; // could not tokenize the nested command → fail closed
 ...
 * ---------------------------------------------------------------------------------------------
 */
function gitPushIsGoverned(words: readonly string[]): boolean {
...
function failClosedCheckpoints(text: string): readonly Checkpoint[] {
  ...
  for (const r of COMMAND_CHECKPOINT_RULES) {
    if (!new RegExp(`\\b${r.tool}\\b`).test(text)) continue;
    if (r.verbs.some((v) => new RegExp(`\\b${v}\\b`).test(text))) out.push(r.checkpoint);
  }
  return out;
}
...
export interface CommandMatch {
  ...
  /** A segment carried a substitution this model refuses to reason about. Fail closed. */
  ...
  for (const seg of segs) {
    ...
    for (const r of COMMAND_CHECKPOINT_RULES) {
      ...
      if (r.tool === "git") {
        ...
        if (gitPushIsGoverned(seg.words)) out.add(r.checkpoint);
        continue;
      }
      if (seg.words.some((w) => r.verbs.includes(w))) out.add(r.checkpoint);
    }
  }
