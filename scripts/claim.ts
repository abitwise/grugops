...
// A single trusted now-running row parsed from a claimed/<task>/claim.md record.
interface NowRunningRow {
  task: string;
  by: string;
  at: string;
}
...
export function renderNowRunning(queueRoot: string = DEFAULT_QUEUE_ROOT): void {
  const claimedDir = join(queueRoot, "claimed");
  const rows: NowRunningRow[] = [];
  ...
  if (existsSync(claimedDir)) {
    for (const task of readdirSync(claimedDir)) {
      ...
      const atLineCount = (claimText.match(/^at:/gm) ?? []).length;
      if (atLineCount > 1) continue; // tampered (multi-`at`) — never a trusted now-running row
      const atMatch = claimText.match(/^at:\s*(.+)$/m);
      if (!atMatch) continue; // no `at` field → cannot place it on the timeline; skip
...
if (isMain) {
  ...
  try {
    ...
    console.error(`claim: ${(e as Error).message}`);
    process.exit(1);
  }
}
