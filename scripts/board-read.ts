...
export function readVerifyReread(
  ...
  for (let attempt = 1; attempt <= retries; attempt += 1) {
    try {
      ...
      if (!(sizeAgrees && mtimeAgrees)) continue; // a real tear: the file moved under the read
...
type Settled<T> = {
  ...
  readonly errors: readonly ReadError[];
  /**
   * Every listed entry this source READ and could not admit (plan 32-15, CR-01).
   *
   * BESIDE `errors`, NOT INSTEAD OF IT. A refusal is already a `readErrors` entry and stays one —
   * that is the channel a human reading stderr and a consumer reading the document both have. This
   * field is the SAME refusal in the shape the JOIN can answer a question with: a per-identifier
   * set, so `joinSnapshot` can tell "no file carries this identifier" from "a file carries it and
   * the grammar refused it" without re-parsing an error message.
   *
   * EMPTY FOR EVERY SOURCE BUT `tickets`, because `tickets` is the only source an identifier on the
   * board is joined against. A source that never listed anything saw no entry, so the empty set is
   * the honest answer rather than a placeholder.
   */
  readonly unadmitted: readonly UnadmittedTicket[];
};
...
function settledFrom<T>(
  ...
  unadmitted: readonly UnadmittedTicket[] = [],
): Settled<T> {
  return {
    state: settled.state,
...
export function childPath(root: string, dir: string, name: string): ChildPath {
  ...
  if (name === "" || name === "." || name === ".." || name.includes("/") || name.includes("\\")) {
    return {
      ...
      target: dir,
      message:
        spelled`board-read: the directory entry \`${name}\` under ${dir} is not a plain path ` +
 ...
 */
export function readTicketsSource(
  root: string,
  readAt: string,
  ...
  for (const name of listing.names) {
    ...
    if (stem === "") {
      errors.push({
        ...
        code: "empty-stem",
        message:
          spelled`${name} leaves no file stem once the .md extension is removed, so this entry's ` +
    ...
    if (claimedBy !== undefined) {
      errors.push({
        ...
        code: "duplicate-id",
        message:
          spelled`${name} and ${claimedBy} both claim the identifier ${id}. Ticket identifiers ` +
...
function readQueueSource(
  ...
  if (listing.kind === "failed") {
    return settledFrom(
      settleSource("queue", queueRoot, listingFailure(claimedDir, listing), previous, readAt),
    );
  }
  ...
  for (const task of claimedNames) {
    ...
    if (!isSafeTaskName(task)) {
      errors.push({
        ...
        message:
          spelled`board-read: the claimed-stage entry \`${task}\` under ${claimedDir} is not a ` +
    ...
    if (!existsSync(claimMd)) {
      ...
      errors.push({
        ...
        message:
          spelled`${claimMd} does not exist, so the claimed task \`${task}\` carries no claim ` +
...
function readContextSource(
  ...
  if (listing.kind === "failed") {
    return settledFrom(
      settleSource("context", dir, listingFailure(dir, listing), previous, readAt),
    );
  }
  ...
  for (const name of listing.names) {
    if (!isSafeTaskName(name)) {
      ...
      errors.push({
        ...
        code: "unsafe-task-name",
        message:
          spelled`board-read: the context entry \`${name}\` under ${dir} is not a plain path ` +
