 ...
 */
export function parseCitation(cell) {
    const c = cell.trim();
    ...
    if (!/\(/.test(c))
        return { refused: "the cell carries no parenthetical, so no capture date and no citation" };
    if (!/\(captured \d{4}-\d{2}-\d{2}/.test(c))
        return { refused: "the parenthetical does not open with `captured` and an ISO date" };
    if (!/§/.test(c))
        return { refused: "the parenthetical names no summary section (no `§`)" };
...
function summaryHasSection(summaryText, section) {
    for (let level = 1; level <= 6; level += 1) {
        if (unfencedHeadingIndices(summaryText, `${"#".repeat(level)} ${section}`).length > 0)
            return true;
    }
    return false;
}
function checkCitations(m, manifestRel, parity, parityRows) {
    const summaryName = m.settings.get("capture summary");
    ...
    if (!existsSync(abs(summaryRel))) {
        throw new Error(`the manifest is discharged but the capture summary ${summaryRel} does not exist — every flipped ` +
    ...
    for (const row of parityRows) {
        ...
        for (let col = 1; col < data.length; col += 1) {
            ...
            if (!summaryHasSection(summaryText, parsed.citation.section)) {
                refusals += 1;
                fail(`flip row ${row.id}: ${where} cites section ${JSON.stringify(parsed.citation.section)}, which ` +
...
// ─────────────────────────────────────────────────────────────────────────────────────────────
function checkResidual(m, union) {
    const token = m.settings.get("residual token").toLowerCase();
    ...
    for (const rel of union) {
        ...
        for (let i = 0; i < lines.length; i += 1) {
            ...
            if (!t1 && !t2)
                continue;
            if (exempt.some((a) => line.includes(a)))
                continue;
            findings += 1;
            fail(`residual: ${rel}:${i + 1} still carries ${t1 ? `the cell token \`${token}\`` : "a GAP-D1 deferral sentence"} ` +
    ...
    if (findings === 0) {
        pass(`residual rule: no cell token or GAP-D1 deferral sentence survives in ${union.length} live-surface ` +
...
function checkLocators(m, manifestRel) {
    ...
    if (discharged && parity !== null)
        checkCitations(m, manifestRel, parity, parityRows);
}
