...
export function readCavemanFence(text) {
    ...
    const isRemoved = (i) => i === heading || (i >= open && i <= close);
    ...
    if (outsideLines + removedLines !== lines.length) {
        throw new Error(`readCavemanFence: line accounting does not close — ${outsideLines} retained + ${removedLines} removed != ${lines.length} document line(s) ` +
 ...
 */
export function countLexiconTokens(text) {
    let n = 0;
...
export function countBannedConstructions(text) {
    const out = {};
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        for (const sentence of lines[i].split(/(?<=[.!?])\s+/)) {
            ...
            for (const fragment of sentence.split(/ — | – | ; | : /)) {
                ...
                if (clause.split(" ").filter(Boolean).length < CLAUSE_MIN_WORDS)
                    continue;
