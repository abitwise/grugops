...
// The column a single ref belongs to (or null = the ticket key / unclassified, never a cell).
type Column = "Requirement" | "Code" | "Tests" | "UAT" | "Release";

function classify(ref: string): Column | null {
  const m = ID_SHAPE.exec(ref);
  const prefix = m ? m[1].toUpperCase() : "";
  if (prefix === "UAT") return "UAT";
  if (prefix === "REL") return "Release";
  if (prefix === "NFR" || prefix === "EPIC" || prefix === "FEAT") return "Requirement";
  if (isTestRef(ref)) return "Tests";
  if (isTicketRef(ref)) return null; // the row key, never a cell value
  if (isCodeRef(ref)) return "Code";
  ...
  if (m) return "Requirement";
  return null;
}

// ── A ticket's accumulated trace cells (deterministic ordered sets per column). ─────────────────
interface TicketRow {
  id: string;
  Requirement: string[];
  Code: string[];
...
function emptyRow(id: string): TicketRow {
...
function listTasks(contextRoot: string): string[] {
...
export function buildRows(contextRoot: string): TicketRow[] {
    ...
    for (const n of notes) {
      ...
      for (const ticketId of ticketIds) {
        ...
        for (const ref of n.refs) {
          if (ref === ticketId) continue; // the key itself is never a cell
          const col = classify(ref);
          if (col !== null) pushUnique(row[col], ref);
        }
      }
    }
  }
...
export function renderTraceability(
  ...
  plansRoot: string = DEFAULT_PLANS_ROOT,
): void {
...
if (isMain) {
  ...
  try {
    ...
    console.error(`trace-render: ${(e as Error).message}`);
    process.exit(1);
  }
}
