// generate-asvs-checklist.mjs — grugops security/NFR checklist generator (SEC-02).
//
...
// is created or required (D-03, TS pivot HELD → plain ESM .mjs). Invocation takes no arguments:
//
//   node scripts/generate-asvs-checklist.mjs    # exit 0 on success, 1 on any failure
//
// Read/write-only by construction (T-14-02, mirrors validate-agent-factory.mjs): SRC and OUT
...
if (requirements.length !== EXPECTED_ROWS) {
  fail(
    `${SRC}: expected ${EXPECTED_ROWS} requirements, found ${requirements.length} — refusing to write a partial checklist`,
  );
}

// ── Group by chapter, then numeric-sort V1..V17 (Number of the "V<n>" id) ────────────────────
...
lines.push("");
lines.push(
  `This is the full OWASP Application Security Verification Standard (ASVS) 5.0 requirement set —`,
);
lines.push(
  `${EXPECTED_ROWS} requirements across ${chapters.length} chapters, each tagged with its requirement ID and its`,
);
lines.push(
  `assurance level (L1, L2, or L3). It is generated from the pinned source above and is not`,
);
lines.push(
  `hand-transcribed; every row's text is copied verbatim from the standard.`,
);
lines.push("");
lines.push(
  `ASVS levels are cumulative: Level 2 includes all Level 1 requirements, and Level 3 includes`,
);
lines.push(
  `all Level 1 and Level 2 requirements. The \`security.asvs_level\` dial selects the active tier`,
);
lines.push(
  `at read and audit time — keep every requirement where \`L <= asvs_level\`. The active set is`,
);
lines.push(
  `${l1} requirements at L1, ${l2cum} at L2 (cumulative), and ${l3cum} at L3 (cumulative). This file is`,
);
lines.push(
  `not regenerated when the dial changes; the dial is a read-time filter over the full set.`,
);
lines.push("");
lines.push(
  `Apply this checklist whenever a change touches authentication, authorization, data handling,`,
);
lines.push(
  `cryptography, configuration, or another security concern. Reproduce each control exactly,`,
);
lines.push(
  `never skip one. Every pass must cite evidence; mark anything unverified \`UNKNOWN - verify\`.`,
);
...
console.log(
  `generate-asvs-checklist: wrote ${requirements.length} requirements across ${chapters.length} chapters to ${OUT}`,
);
process.exit(0);
