...
1. **Resolve A3** (the last open UAT-AUTO-04 item): redesign the A3-live case to assert on the on-disk handoff artifacts the orchestrator writes into the temp repo (`implementation-handoff.md` / `qe-handoff.md` / the verdict), rather than on `-p` stdout substrings — a full ticket-to-PR session does not reliably surface those literals in print output within budget. A gap-closure plan against UAT-AUTO-04. (DONE here: added `UAT_E2E_CALL_TIMEOUT_MS`, which resolved A1.)
