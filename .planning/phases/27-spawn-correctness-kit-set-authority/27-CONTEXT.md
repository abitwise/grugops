...
- **D-39 (delimiter posture — REFUSE AMBIGUITY BY NAME; supersedes nothing, but it is the fourth application of D-30's polarity):** `scripts/frontmatter.ts`'s delimiter region gets **one normalization point and one whitespace authority**, and the complement of the delimiter test stops being a silent success.
  ...
  3. **The refusal arm is DEFINED AS THE DISAGREEMENT BETWEEN THE MODULE'S OWN TWO GRAMMARS — it is not a new heuristic.** A head/close line is *near-delimiter* iff `line.trim() === "---"` (the Unicode-aware grammar already at line 686) and *is-delimiter* iff it matches the declared class (the ASCII grammar at 695/701). `near && !delimiter` → `ok:false`, naming the first code point present under `trim()`'s class but absent from the declared class. Nothing is enumerated; the refusal set is exactly the set on which the module currently contradicts itself.
