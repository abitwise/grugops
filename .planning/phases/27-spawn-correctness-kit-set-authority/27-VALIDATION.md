...
| T1–T2 | 27-16 | 5 | SPAWN-03 | — | Coordinator adapter resolves under `claude --agent`; `@grugops-orchestrator` header appears | **manual** (observable preconditions automated) | prerequisite, observable half only: `node scripts/coordinator-resolution-precheck.js` — *the runtime half has no automated equivalent; the runtime is the SUT* | `checkpoint:human-verify`; recorded in `27-SPAWN-03-RUNTIME-EVIDENCE.md` | ⬜ pending |
...
|----------|-------------|------------|-------------------|
| Coordinator adapter resolves under `claude --agent grugops-orchestrator`; the `@grugops-orchestrator` header appears in the session | SPAWN-03 | The Claude Code runtime is the system under test — no in-repo automated equivalent can observe real agent resolution | **Step 0 (prerequisite, automated):** run `node scripts/coordinator-resolution-precheck.js --keep-scratch-target`. It discharges every observable precondition — scratch install, coordinator marker, agent name, grant size, granted-name resolution, materialized kit path — exits non-zero naming any that fails, starts no session and spends no tokens, and prints the two remaining steps as human-only. **Steps 1–2 (human):** in the scratch target it names, run `claude --agent grugops-orchestrator`; confirm the session header names the agent, then ask for work that routes to one specialist and confirm a distinct role agent resolves and runs. **Recording surface:** `27-SPAWN-03-RUNTIME-EVIDENCE.md` in this phase directory — it ships with named empty slots marked unverified, and an empty slot means the check was not performed, never that it held. |
