...
- **D-03:** Generator obeys the tech-stack constraint: **POSIX-sh or stdlib Node only, no TypeScript** (TS pivot is HELD pending project ratification — see carried-forward locks). Match the existing `scripts/validate-agent-factory.mjs` style (Node 18+, `node:fs`/`node:path`, ESM, zero deps).
  - **SUPERSEDED by the Phase-15 TypeScript pivot (D-13, ratified 2026-06-13).** The "no TypeScript" rule no longer holds: Phase 15 ported the ASVS generator (and the rest of the tooling layer) to TypeScript at byte-reproducible parity — `scripts/generate-asvs-checklist.ts` compiled to a committed `scripts/generate-asvs-checklist.js`, Node 22+ floor. This decision is preserved for history; the ratified constraint now lives in CLAUDE.md + PROJECT.md.
...
- **New scripts = POSIX sh or stdlib Node, not TypeScript.** The TS pivot is HELD pending project-level ratification.
  - **SUPERSEDED by the Phase-15 TypeScript pivot (D-13, ratified 2026-06-13).** New tooling scripts are now TypeScript (tsc-compiled committed `.js`, freshness-checked; Node 22+ floor). This lock is preserved for history; the ratified constraint now lives in CLAUDE.md + PROJECT.md.
