...
fixed_count: 38
total_count: 309
last_updated: 2026-09-25T09:08:40.717Z
---
...
| 297 | 33 | unrun-verify | scripts/capture-live.ts | 998 | 33-R4-DIAGNOSIS section 1 and section 5 decision 1 (human): on round-4 run A the default session spawned the coordinator adapter as a subagent (A:80), which spawned role agents (A:313, A:711), nesting observed on Claude Code 2.1.281, and the runner every-spawn-is-a-member clause (scripts/capture-live.ts:998) failed CAP-03 side (a) on it, which set OUTCOME: fail. The D-02 (a) bar of two evidenced granted roles was met on path A (mapper, architect-design). Cause class SUITE with a platform/KIT rider. Whether D-02 (a) should forbid the coordinator-as-subagent route, or the clause relax to D-02, is the human decision; not decided here. Owner: the human. DISPOSITION 2026-09-25 (Phase 33 closed by human override D-33-R4-08, 33-CONTEXT.md): human decision D-33-R4-04 ALLOWS the coordinator-as-subagent route. D-02 (a) stands as written: at least two granted roles, each with own-session evidence, which path A met. The runner's stricter every-spawn-is-a-grant-member clause in capThreePredicate (scripts/capture-live.ts about :990-1000) is to be DROPPED FOR THE COORDINATOR SPAWN by a recorded decision in the next phase, not edited inside Phase 33 (D-20 forbids softening a predicate inside this phase). Work owed; the row stays open. The CLAUDE.md no-nesting contradiction is row 309. Owner: the next phase. | open |  | 2026-09-24T19:47:21.990Z |  |
| 298 | 33 | unrun-verify | scripts/capture-live.ts | 1868 | 33-R4-DIAGNOSIS section 3.1 and section 5 decision 2 (human): both round-4 sessions ran permissionMode auto inherited from the operator settings (A:12, B:12); the runner pins no permission mode (spawn args at scripts/capture-live.ts:1868), and role agents used the Write tool on 7 paths outside the target (A:569, A:611, A:919, A:960, B:508, B:857, B:920) though the grant names no Write; the checkout was not touched. Class HOST with a SUITE rider. Whether the capture should pin a permission mode is the human decision; whether --permission-mode default would deny these writes in -p mode is UNKNOWN - verify. Owner: the human. DISPOSITION 2026-09-25 (Phase 33 closed by human override D-33-R4-08, 33-CONTEXT.md): human decision D-33-R4-05: YES, the capture must pin --permission-mode default rather than inherit the operator's auto mode. The FIRST step in the next phase is a zero-token check of whether that mode actually denies an out-of-target Write in -p mode (UNKNOWN - verify); the pin lands only on that measurement. Work owed; the row stays open. Owner: the next phase. | open |  | 2026-09-24T19:47:22.161Z |  |
| 299 | 33 | deviation | scripts/capture-live.ts | 2724 | 33-R4-DIAGNOSIS section 3.3 and section 5 decision 4 (human): the per-run D-04 row does not key on the probe tool use; run B row reads yes on the deny of an ordinary find command (B:365, deny frame B:374) though path B never issued the probe. The outcome word is unaffected: denyFired is runs.some at scripts/capture-live.ts:2724 and run A probe deny (A:85, A:101) satisfies it. Whether the row should join the deny to the probe tool use through the tool result is the human decision. Owner: the human. DISPOSITION 2026-09-25 (Phase 33 closed by human override D-33-R4-08, 33-CONTEXT.md): human decision D-33-R4-07: YES, key the per-run D-04 row on the probe's own tool use, joining the deny through the tool result to the tool use whose command is the probe. A small runner fix in the next phase. Work owed; the row stays open. Owner: the next phase. | open |  | 2026-09-24T19:47:22.334Z |  |
...
| 301 | 33 | deviation | scripts/checkpoints.ts | 1864 | 33-close round-4 review (33-REVIEW.md, commit f20f1af5) CR-01, a LIVE PROD-DEPLOY GUARD BYPASS, accepted OPEN at the phase close by human override D-33-R4-08, not fixed: a whitespace-free, wholly-quoted nested shell body is now NAMED by plan 33-35's recursive projection (governedToolsNamedBy, :1371-1380), but the readable arm of matchCommandCheckpoints (:1864-1916) reads verb candidates only from the words AFTER the naming word, so a verb, remote and ref inside that same word are never read; the segment is not opaque either, because UNRESOLVABLE_SHELL_RE catches a braced expansion, a command substitution and a backtick but not a bare $IFS or a brace list. Two shell mechanisms split such a word into a full governed command in the nested shell: brace expansion of a comma list, and field splitting of an unquoted $IFS between double-quoted pieces. The reviewer measured ALLOW through the shipped wrapper with a scrubbed environment for a push to a protected ref, a force push, npm publish, kubectl apply and gh pr merge under the bash -c, sh -c, eval and pipe-to-shell wrappers, and a stub binary received the full governed argv. The verifier independently reproduced the brace-expansion form through BOTH hook entry points, hooks/guard.js and hooks/hook-entry.js guard.js, on HEAD f20f1af5 (33-VERIFICATION.md, aba4f1d3). Inherited, not created: the round-3 build also allowed it; plan 33-35 claimed the class closed. Not one of the ledgered residuals (RES-01..RES-03, LB-05..LB-08): nothing is computed at run time. Fix direction named by the review: refuse on the name when a word names a governed tool only through the nested re-projection and carries no whitespace, or classify such a quoted run opaque in classifyWords; add corpus rows for both mechanisms under all four families and a replay through hooks/hook-entry.js. Coupled with D-33-R4-06 (the guard redesign, row 269). PRIORITY: FIRST in the next phase, together with CR-02, ahead of every other carried item. Owner: the next phase (a new inserted phase is recommended to carry Phase 33 open items; Phase 34 is not re-scoped by this row). | open |  | 2026-09-25T09:08:13.549Z |  |
| 302 | 33 | deviation | scripts/checkpoints.ts | 1893 | 33-close round-4 review (33-REVIEW.md, commit f20f1af5) CR-02, a LIVE PROD-DEPLOY GUARD BYPASS, accepted OPEN at the phase close by human override D-33-R4-08, not fixed: positional-parameter nested bodies are read in the order their words are SPELLED, not the order the body USES them. Corpus rows R4-11 and R4-12 deny only for the one corpus ordering. A body that references its positional parameters in its own order, or a trailing unused argument, puts a non-protected word last, and gitPushIsGoverned (:1581-1597) reads the LAST candidate as the refspec, so the push to a protected ref is not governed. The reviewer measured ALLOW through hooks/hook-entry.js guard.js with a scrubbed environment while a stub git received push origin main; the literal patterns in hooks/guard.ts do not catch it because the tool and the verb are never adjacent in the text. The same mechanism lets a nested body take its ref from a positional parameter: in the model a double-quoted positional reference classifies as a canonical literal value, and that spelling is held today only by the literal pattern in hooks/guard.ts. Not RES-01: no variable is assigned and every value is literal on the line. The round-3 build also allowed it. Reviewed, NOT independently re-run by the verifier (33-VERIFICATION.md, aba4f1d3). Fix direction named by the review: a double-quoted run containing an unescaped dollar is an expansion, classify it opaque in classifyWords; for the top-level segment refuse on the name (the CR-01 fix); add permuted and trailing-argument variants of R4-11 and R4-12 as deny rows. Coupled with D-33-R4-06 (the guard redesign, row 269). PRIORITY: FIRST in the next phase, together with CR-01. Owner: the next phase (a new inserted phase is recommended to carry Phase 33 open items; Phase 34 is not re-scoped by this row). | open |  | 2026-09-25T09:08:13.644Z |  |
| 303 | 33 | deviation | scripts/context-io.ts | 1926 | 33-close round-4 review (33-REVIEW.md, commit f20f1af5) WR-01, accepted OPEN at the phase close (D-33-R4-08): the idempotent path of appendNote and admitAndAppend (:1926-1949, :6325-6358) appends a second GOV-02 ledger event for a note already on disk. Identical bytes fall through to admit() (or appendAuditLedger on the gated branch) under audit_retention retained, then writeNoteFile returns without writing, so a second ledger line is keyed by the same id; appendNote takes a caller precomputedId, so a re-run reaches it directly. promoteAdmitted guards the same case with ledgerRecordsId (D-19 (4)), so the sibling routes disagree about duplicates: the one-arm-fixed shape. Fix direction: return the id without admit() or the ledger append when the destination already holds the same text, or apply the ledgerRecordsId look; pin it with a test that re-appends the same precomputedId under retained and asserts one ledger line. Owner: the next phase (a new inserted phase is recommended to carry Phase 33 open items; Phase 34 is not re-scoped by this row). | open |  | 2026-09-25T09:08:13.731Z |  |
| 304 | 33 | deviation | scripts/context-io.ts | 1916 | 33-close round-4 review (33-REVIEW.md, commit f20f1af5) WR-02, accepted OPEN at the phase close (D-33-R4-08): the pre-ledger occupancy check plan 33-38 added is check-then-act, not true by construction as the comments at :1916-1929, :3464-3491 and :6310-6326 claim. decideNoteDestination reads the destination, the ledger is appended, and writeNoteFile decides again; an occupant created in between (a concurrent writer, and v2.0 runs parallel agents on Claude Code) still leaves the over-record 33-38 set out to remove. Fix direction: reword the claim to name the race window, or publish with an exclusive create and append the ledger only once the destination is decided; if the ledger-first ordering of 31-21 is kept, document the race as the accepted residual. Owner: the next phase (a new inserted phase is recommended to carry Phase 33 open items; Phase 34 is not re-scoped by this row). | open |  | 2026-09-25T09:08:13.824Z |  |
| 305 | 33 | deviation | scripts/capture-live.ts | 1112 | 33-close round-4 review (33-REVIEW.md, commit f20f1af5) WR-03 (NOT the round-3 WR-03 on row 280), accepted OPEN at the phase close (D-33-R4-08): the 33-39 parity docblock (WRITE_SHAPED_COMMAND_WORDS, :1112-1125, :1182-1186, :1285-1289) claims adding a word can only widen the parity input and never produce a false pass, and that removing reads cannot make a false pass reachable. compareLivePaths compares the A count to the B count for EQUALITY, so changing the counted set in either direction can turn an unequal pair equal. 33-39 now drops real write routes the old union counted: a written non-script file later run by an interpreter; Windows script extensions (.ps1, .cmd, .bat); versioned interpreter names (WRITE_WORD_RE needs a boundary right after the word); writers outside the list (sponge, curl -o, wget -O). A route one path took and the other did not can be invisible to both counts, and parity then reads pass. Fix direction: state the real property (an approximation; over-count and under-count can both mask a difference), extend SCRIPT_EXTENSIONS with .ps1, .psm1, .cmd and .bat, and admit a version suffix after an interpreter name. Owner: the next phase (a new inserted phase is recommended to carry Phase 33 open items; Phase 34 is not re-scoped by this row). | open |  | 2026-09-25T09:08:13.917Z |  |
| 306 | 33 | deviation | hooks/guard.test.ts | 2382 | 33-close round-4 review (33-REVIEW.md, commit f20f1af5) WR-04, accepted OPEN at the phase close (D-33-R4-08): the allow-control assertions in the spawn harness pass on a crash or a timeout. runAll (:2382-2407) resolves with whatever stdout arrived; after a SIGKILL on timeout, or a child that throws before writing, it resolves with an empty string, and the allow controls (:2536-2538, :2593-2594) assert only that no permissionDecision appears, which an empty stdout satisfies. So a guard that crashes on those inputs still passes the allows half; the exit status is never recorded. Deny assertions are unaffected. Fix direction: resolve with stdout, exit code and signal, and for allow controls also assert code 0 and signal null. Owner: the next phase (a new inserted phase is recommended to carry Phase 33 open items; Phase 34 is not re-scoped by this row). | open |  | 2026-09-25T09:08:14.007Z |  |
| 307 | 33 | deviation | scripts/context-io.ts | 1926 | 33-close round-4 review (33-REVIEW.md, commit f20f1af5) IN-01, accepted OPEN at the phase close (D-33-R4-08): the pre-check appendNote now runs (decideNoteDestination, :1926) raises errors spelled context-io.writeNoteFile (messages at :1501-1566) before admit() and outside appendNote's own wrapping, so the message names a function that was not called; admitAndAppend does wrap them, so the siblings are inconsistent. Fix direction: wrap the pre-check in appendNote's own refusal framing, or drop the function prefix from the shared messages. Owner: the next phase (a new inserted phase is recommended to carry Phase 33 open items; Phase 34 is not re-scoped by this row). | open |  | 2026-09-25T09:08:14.096Z |  |
| 308 | 33 | deviation | scripts/checkpoints.ts | 1880 | 33-close round-4 review (33-REVIEW.md, commit f20f1af5) IN-02, accepted OPEN at the phase close (D-33-R4-08): under xargs an adjacent benign word decides (:1880-1891), and several benign words LAUNCH other commands whose arguments then come from stdin (npm, yarn and pnpm exec; git submodule foreach; git bisect run; git rebase -x). Reachable today only when the stdin text is invisible on the line, which is the ledgered formatter-assembles-and-pipes class; the docblock at :1596-1631 does not name it. Fix direction: remove launcher-shaped words from the adjacency exemption under a stdin feed, or add the class to the failClosedCheckpoints residual list. Handle with the CR-01/CR-02 guard work (D-33-R4-06). Owner: the next phase (a new inserted phase is recommended to carry Phase 33 open items; Phase 34 is not re-scoped by this row). | open |  | 2026-09-25T09:08:14.184Z |  |
...
[
  ...
  },
  {
    ...
    "file": "hooks/guard.ts",
    "line": null,
    "description": "33-DIAGNOSIS section 2 accepted open for round 3 (human decision ledger-and-hold at plan 33-21 Task 2, 2026-09-21): the guard (hooks/guard.ts + scripts/checkpoints.ts) classifies a 2>&1 redirection and an ordinary $var expansion as substitutions it will not reason about and refuses any such segment carrying git or npm anywhere in it, on tool name alone — fifteen over-matched denies across the two round-1 transcripts (A:454 … B:1750). Reproduced offline against the committed guard on stdin with no GRUGOPS_ variable set: git log --oneline -5 -> allow; git log --oneline -5 2>&1 -> deny (matched by the command model, the git push sentence printed for a git log); echo \"npm run lint\" -> allow; echo \"npm run $s\" -> deny (carries a shell substitution the guard will not reason about); matchCommandCheckpoints reports untokenizable: true for both. Fails closed — an availability defect, not a safety hole; no offline test covers the class (275 guard tests green). Correction named by the diagnosis: the tokenizer's redirection-vs-substitution classification, and the deny text. No further direction stated beyond accepted-open. Owner: round 3 DISPOSITION 2026-09-25 (Phase 33 closed by human override D-33-R4-08, 33-CONTEXT.md): this fixed row's still-open half is row 269, disposed by human decision D-33-R4-06 (guard redesign in the next phase). Status unchanged.",
    "status": "fixed",
    "reason": "",
  ...
  },
  {
    ...
    "file": "scripts/capture-live.ts",
    "line": 998,
    "description": "33-R4-DIAGNOSIS section 1 and section 5 decision 1 (human): on round-4 run A the default session spawned the coordinator adapter as a subagent (A:80), which spawned role agents (A:313, A:711), nesting observed on Claude Code 2.1.281, and the runner every-spawn-is-a-member clause (scripts/capture-live.ts:998) failed CAP-03 side (a) on it, which set OUTCOME: fail. The D-02 (a) bar of two evidenced granted roles was met on path A (mapper, architect-design). Cause class SUITE with a platform/KIT rider. Whether D-02 (a) should forbid the coordinator-as-subagent route, or the clause relax to D-02, is the human decision; not decided here. Owner: the human. DISPOSITION 2026-09-25 (Phase 33 closed by human override D-33-R4-08, 33-CONTEXT.md): human decision D-33-R4-04 ALLOWS the coordinator-as-subagent route. D-02 (a) stands as written: at least two granted roles, each with own-session evidence, which path A met. The runner's stricter every-spawn-is-a-grant-member clause in capThreePredicate (scripts/capture-live.ts about :990-1000) is to be DROPPED FOR THE COORDINATOR SPAWN by a recorded decision in the next phase, not edited inside Phase 33 (D-20 forbids softening a predicate inside this phase). Work owed; the row stays open. The CLAUDE.md no-nesting contradiction is row 309. Owner: the next phase.",
    "status": "open",
    "reason": "",
  ...
  },
  {
    ...
    "file": "scripts/capture-live.ts",
    "line": 1868,
    "description": "33-R4-DIAGNOSIS section 3.1 and section 5 decision 2 (human): both round-4 sessions ran permissionMode auto inherited from the operator settings (A:12, B:12); the runner pins no permission mode (spawn args at scripts/capture-live.ts:1868), and role agents used the Write tool on 7 paths outside the target (A:569, A:611, A:919, A:960, B:508, B:857, B:920) though the grant names no Write; the checkout was not touched. Class HOST with a SUITE rider. Whether the capture should pin a permission mode is the human decision; whether --permission-mode default would deny these writes in -p mode is UNKNOWN - verify. Owner: the human. DISPOSITION 2026-09-25 (Phase 33 closed by human override D-33-R4-08, 33-CONTEXT.md): human decision D-33-R4-05: YES, the capture must pin --permission-mode default rather than inherit the operator's auto mode. The FIRST step in the next phase is a zero-token check of whether that mode actually denies an out-of-target Write in -p mode (UNKNOWN - verify); the pin lands only on that measurement. Work owed; the row stays open. Owner: the next phase.",
    "status": "open",
    "reason": "",
  ...
  },
  {
    ...
    "file": "scripts/capture-live.ts",
    "line": 2724,
    "description": "33-R4-DIAGNOSIS section 3.3 and section 5 decision 4 (human): the per-run D-04 row does not key on the probe tool use; run B row reads yes on the deny of an ordinary find command (B:365, deny frame B:374) though path B never issued the probe. The outcome word is unaffected: denyFired is runs.some at scripts/capture-live.ts:2724 and run A probe deny (A:85, A:101) satisfies it. Whether the row should join the deny to the probe tool use through the tool result is the human decision. Owner: the human. DISPOSITION 2026-09-25 (Phase 33 closed by human override D-33-R4-08, 33-CONTEXT.md): human decision D-33-R4-07: YES, key the per-run D-04 row on the probe's own tool use, joining the deny through the tool result to the tool use whose command is the probe. A small runner fix in the next phase. Work owed; the row stays open. Owner: the next phase.",
    "status": "open",
    "reason": "",
  ...
  },
  {
    ...
    "file": "scripts/checkpoints.ts",
    "line": 1864,
    "description": "33-close round-4 review (33-REVIEW.md, commit f20f1af5) CR-01, a LIVE PROD-DEPLOY GUARD BYPASS, accepted OPEN at the phase close by human override D-33-R4-08, not fixed: a whitespace-free, wholly-quoted nested shell body is now NAMED by plan 33-35's recursive projection (governedToolsNamedBy, :1371-1380), but the readable arm of matchCommandCheckpoints (:1864-1916) reads verb candidates only from the words AFTER the naming word, so a verb, remote and ref inside that same word are never read; the segment is not opaque either, because UNRESOLVABLE_SHELL_RE catches a braced expansion, a command substitution and a backtick but not a bare $IFS or a brace list. Two shell mechanisms split such a word into a full governed command in the nested shell: brace expansion of a comma list, and field splitting of an unquoted $IFS between double-quoted pieces. The reviewer measured ALLOW through the shipped wrapper with a scrubbed environment for a push to a protected ref, a force push, npm publish, kubectl apply and gh pr merge under the bash -c, sh -c, eval and pipe-to-shell wrappers, and a stub binary received the full governed argv. The verifier independently reproduced the brace-expansion form through BOTH hook entry points, hooks/guard.js and hooks/hook-entry.js guard.js, on HEAD f20f1af5 (33-VERIFICATION.md, aba4f1d3). Inherited, not created: the round-3 build also allowed it; plan 33-35 claimed the class closed. Not one of the ledgered residuals (RES-01..RES-03, LB-05..LB-08): nothing is computed at run time. Fix direction named by the review: refuse on the name when a word names a governed tool only through the nested re-projection and carries no whitespace, or classify such a quoted run opaque in classifyWords; add corpus rows for both mechanisms under all four families and a replay through hooks/hook-entry.js. Coupled with D-33-R4-06 (the guard redesign, row 269). PRIORITY: FIRST in the next phase, together with CR-02, ahead of every other carried item. Owner: the next phase (a new inserted phase is recommended to carry Phase 33 open items; Phase 34 is not re-scoped by this row).",
    "status": "open",
    "reason": "",
    "recorded_at": "2026-09-25T09:08:13.549Z",
    "resolved_at": null,
    "milestone": "v2.1"
  },
  {
    ...
    "file": "scripts/checkpoints.ts",
    "line": 1893,
    "description": "33-close round-4 review (33-REVIEW.md, commit f20f1af5) CR-02, a LIVE PROD-DEPLOY GUARD BYPASS, accepted OPEN at the phase close by human override D-33-R4-08, not fixed: positional-parameter nested bodies are read in the order their words are SPELLED, not the order the body USES them. Corpus rows R4-11 and R4-12 deny only for the one corpus ordering. A body that references its positional parameters in its own order, or a trailing unused argument, puts a non-protected word last, and gitPushIsGoverned (:1581-1597) reads the LAST candidate as the refspec, so the push to a protected ref is not governed. The reviewer measured ALLOW through hooks/hook-entry.js guard.js with a scrubbed environment while a stub git received push origin main; the literal patterns in hooks/guard.ts do not catch it because the tool and the verb are never adjacent in the text. The same mechanism lets a nested body take its ref from a positional parameter: in the model a double-quoted positional reference classifies as a canonical literal value, and that spelling is held today only by the literal pattern in hooks/guard.ts. Not RES-01: no variable is assigned and every value is literal on the line. The round-3 build also allowed it. Reviewed, NOT independently re-run by the verifier (33-VERIFICATION.md, aba4f1d3). Fix direction named by the review: a double-quoted run containing an unescaped dollar is an expansion, classify it opaque in classifyWords; for the top-level segment refuse on the name (the CR-01 fix); add permuted and trailing-argument variants of R4-11 and R4-12 as deny rows. Coupled with D-33-R4-06 (the guard redesign, row 269). PRIORITY: FIRST in the next phase, together with CR-01. Owner: the next phase (a new inserted phase is recommended to carry Phase 33 open items; Phase 34 is not re-scoped by this row).",
    "status": "open",
    "reason": "",
    "recorded_at": "2026-09-25T09:08:13.644Z",
    "resolved_at": null,
    "milestone": "v2.1"
  },
  {
    ...
    "file": "scripts/context-io.ts",
    "line": 1926,
    "description": "33-close round-4 review (33-REVIEW.md, commit f20f1af5) WR-01, accepted OPEN at the phase close (D-33-R4-08): the idempotent path of appendNote and admitAndAppend (:1926-1949, :6325-6358) appends a second GOV-02 ledger event for a note already on disk. Identical bytes fall through to admit() (or appendAuditLedger on the gated branch) under audit_retention retained, then writeNoteFile returns without writing, so a second ledger line is keyed by the same id; appendNote takes a caller precomputedId, so a re-run reaches it directly. promoteAdmitted guards the same case with ledgerRecordsId (D-19 (4)), so the sibling routes disagree about duplicates: the one-arm-fixed shape. Fix direction: return the id without admit() or the ledger append when the destination already holds the same text, or apply the ledgerRecordsId look; pin it with a test that re-appends the same precomputedId under retained and asserts one ledger line. Owner: the next phase (a new inserted phase is recommended to carry Phase 33 open items; Phase 34 is not re-scoped by this row).",
    "status": "open",
    "reason": "",
    "recorded_at": "2026-09-25T09:08:13.731Z",
    "resolved_at": null,
    "milestone": "v2.1"
  },
  {
    ...
    "file": "scripts/context-io.ts",
    "line": 1916,
    "description": "33-close round-4 review (33-REVIEW.md, commit f20f1af5) WR-02, accepted OPEN at the phase close (D-33-R4-08): the pre-ledger occupancy check plan 33-38 added is check-then-act, not true by construction as the comments at :1916-1929, :3464-3491 and :6310-6326 claim. decideNoteDestination reads the destination, the ledger is appended, and writeNoteFile decides again; an occupant created in between (a concurrent writer, and v2.0 runs parallel agents on Claude Code) still leaves the over-record 33-38 set out to remove. Fix direction: reword the claim to name the race window, or publish with an exclusive create and append the ledger only once the destination is decided; if the ledger-first ordering of 31-21 is kept, document the race as the accepted residual. Owner: the next phase (a new inserted phase is recommended to carry Phase 33 open items; Phase 34 is not re-scoped by this row).",
    "status": "open",
    "reason": "",
    "recorded_at": "2026-09-25T09:08:13.824Z",
    "resolved_at": null,
    "milestone": "v2.1"
  },
  {
    ...
    "file": "scripts/capture-live.ts",
    "line": 1112,
    "description": "33-close round-4 review (33-REVIEW.md, commit f20f1af5) WR-03 (NOT the round-3 WR-03 on row 280), accepted OPEN at the phase close (D-33-R4-08): the 33-39 parity docblock (WRITE_SHAPED_COMMAND_WORDS, :1112-1125, :1182-1186, :1285-1289) claims adding a word can only widen the parity input and never produce a false pass, and that removing reads cannot make a false pass reachable. compareLivePaths compares the A count to the B count for EQUALITY, so changing the counted set in either direction can turn an unequal pair equal. 33-39 now drops real write routes the old union counted: a written non-script file later run by an interpreter; Windows script extensions (.ps1, .cmd, .bat); versioned interpreter names (WRITE_WORD_RE needs a boundary right after the word); writers outside the list (sponge, curl -o, wget -O). A route one path took and the other did not can be invisible to both counts, and parity then reads pass. Fix direction: state the real property (an approximation; over-count and under-count can both mask a difference), extend SCRIPT_EXTENSIONS with .ps1, .psm1, .cmd and .bat, and admit a version suffix after an interpreter name. Owner: the next phase (a new inserted phase is recommended to carry Phase 33 open items; Phase 34 is not re-scoped by this row).",
    "status": "open",
    "reason": "",
    "recorded_at": "2026-09-25T09:08:13.917Z",
    "resolved_at": null,
    "milestone": "v2.1"
  },
  {
    ...
    "kind": "deviation",
    "phase": "33",
    "file": "hooks/guard.test.ts",
    "line": 2382,
    "description": "33-close round-4 review (33-REVIEW.md, commit f20f1af5) WR-04, accepted OPEN at the phase close (D-33-R4-08): the allow-control assertions in the spawn harness pass on a crash or a timeout. runAll (:2382-2407) resolves with whatever stdout arrived; after a SIGKILL on timeout, or a child that throws before writing, it resolves with an empty string, and the allow controls (:2536-2538, :2593-2594) assert only that no permissionDecision appears, which an empty stdout satisfies. So a guard that crashes on those inputs still passes the allows half; the exit status is never recorded. Deny assertions are unaffected. Fix direction: resolve with stdout, exit code and signal, and for allow controls also assert code 0 and signal null. Owner: the next phase (a new inserted phase is recommended to carry Phase 33 open items; Phase 34 is not re-scoped by this row).",
    "status": "open",
    "reason": "",
    "recorded_at": "2026-09-25T09:08:14.007Z",
    "resolved_at": null,
    "milestone": "v2.1"
  },
  {
    ...
    "file": "scripts/context-io.ts",
    "line": 1926,
    "description": "33-close round-4 review (33-REVIEW.md, commit f20f1af5) IN-01, accepted OPEN at the phase close (D-33-R4-08): the pre-check appendNote now runs (decideNoteDestination, :1926) raises errors spelled context-io.writeNoteFile (messages at :1501-1566) before admit() and outside appendNote's own wrapping, so the message names a function that was not called; admitAndAppend does wrap them, so the siblings are inconsistent. Fix direction: wrap the pre-check in appendNote's own refusal framing, or drop the function prefix from the shared messages. Owner: the next phase (a new inserted phase is recommended to carry Phase 33 open items; Phase 34 is not re-scoped by this row).",
    "status": "open",
    "reason": "",
    "recorded_at": "2026-09-25T09:08:14.096Z",
    "resolved_at": null,
    "milestone": "v2.1"
  },
  {
    ...
    "file": "scripts/checkpoints.ts",
    "line": 1880,
    "description": "33-close round-4 review (33-REVIEW.md, commit f20f1af5) IN-02, accepted OPEN at the phase close (D-33-R4-08): under xargs an adjacent benign word decides (:1880-1891), and several benign words LAUNCH other commands whose arguments then come from stdin (npm, yarn and pnpm exec; git submodule foreach; git bisect run; git rebase -x). Reachable today only when the stdin text is invisible on the line, which is the ledgered formatter-assembles-and-pipes class; the docblock at :1596-1631 does not name it. Fix direction: remove launcher-shaped words from the adjacency exemption under a stdin feed, or add the class to the failClosedCheckpoints residual list. Handle with the CR-01/CR-02 guard work (D-33-R4-06). Owner: the next phase (a new inserted phase is recommended to carry Phase 33 open items; Phase 34 is not re-scoped by this row).",
    "status": "open",
    "reason": "",
    "recorded_at": "2026-09-25T09:08:14.184Z",
    "resolved_at": null,
    "milestone": "v2.1"
  },
  {
