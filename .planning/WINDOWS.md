...
fixed_count: 4
total_count: 98
last_updated: 2026-08-21T18:56:07.668Z
---
...
| 86 | 29.1 | deviation | scripts/model-tiers.ts |  | 29.1-21: the unknown-stem override REFUSAL reverses a documented design decision — the module argued the skip was correct for a caller resolving a mirror narrower than the corpus its overrides were read for. Measured that no such caller exists (both production sites hand readModelsConfig and resolveModels the SAME stems; the whole suite and all three live gates are green with the refusal), but the reversal is a contract change, not a bug fix. Direction: FAIL-CLOSED — a future caller that legitimately reads a wider corpus than it resolves is now refused by name rather than silently mis-resolved, which is loud and recoverable, but it IS refused. AMENDED BY PLAN 29.1-25 — THIS ROW IS NO LONGER THE ONLY RECORD OF THE REVERSAL OUTSIDE THE SOURCE FILE. Round-4 finding WR-03 was RULED `record-decision-only` by Olger Oeselg on 2026-08-21, and the reversal is now filed as numbered decision **D-29.1-18** in `.planning/phases/29.1-per-role-model-assignment/29.1-CONTEXT.md`, section "Amendments after research", which supersedes D-06's resolver-side corollary only — D-06's text stands unedited and `readModelsConfig`'s unknown-key refusal is unchanged. The ruling, verbatim: "WR-03 ruled `record-decision-only` by Olger Oeselg on 2026-08-21. The reversal of D-06's unknown-key disposition inside `resolveModels` requires a numbered decision in `29.1-CONTEXT.md`'s amendments section. `CHANGELOG.md` is NOT to be touched for this reversal: the refusal branch is unreachable from production (both call sites hand `readModelsConfig` and `resolveModels` the same `stems` array), and the changelog's stated audience is humans reading about observable changes. A `### Changed` entry is owed if and when a reachable behaviour change ships." THE CHANGELOG HALF IS OWED ON FIRST REACHABLE CHANGE, NOT CLOSED: a consumer who reads only `CHANGELOG.md` still does not meet this reversal, and that gap is a recorded consequence of the ruling rather than an oversight. The row stays `open` because the FAIL-CLOSED direction it discloses is unchanged by the ruling; what the ruling closed is the question of where the reversal is recorded, not the residual itself.| open |  | 2026-08-20T20:44:28.465Z |  |
