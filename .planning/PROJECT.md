...
---
*Last updated: 2026-06-03 — Phase 3 (Roles & AGENTS.md Substrate) complete: all 16 role prompts now exist as real files — the Orchestrator routing contract authored first (13-arrow matrix, 15-item classification, WIP+DoR gate, SPLIT_REQUIRED, verbatim clear-voice hard limits), 11 core + 5 enterprise roles on the fixed 9-section §5 skeleton (caveman prompts byte-exact, real `agent-factory/handoffs/` citations, no path-drift), plus the minimal root `AGENTS.md` (§17.1 shape, 5064 B < 32 KiB, 13 `UNKNOWN - verify` slots, Karpathy's 12 rules single-sourced). A runnable Wave-0 structural harness (`check-structure.sh`) encodes the acceptance criteria and reports ALL CHECKS PASSED. Verified 5/5 success criteria; ROLE-01/02/03, AGENTS-01/02 satisfied. Advisory code review (1 critical + 6 warning) recorded as Phase-4 refinements (board-flow ownership gap, routing-matrix workflow rows). Requirements remain hypotheses pending the Phase 6 dogfood.*
