...
---
*Last updated: 2026-06-03 — Phase 4 (Workflows, Cadence & Backpressure) complete: all 14 lifecycle/ceremony/enterprise workflows now exist as real files in `agent-factory/workflows/` (00..13) on the fixed 10-section v2 template. The §14 CI/CD backpressure loop is single-sourced in `05-pr-quality-gate.md` — prefetch → branch → gate (install/lint/typecheck/unit/build/e2e) → bounded self-fix → terminal result, never auto-merges (GATE-01); `04-ticket-to-pr.md` references it without restating it (D-26). Dual cadence is resolved by config with no filename suffix — `08`/`10` scrum-only, `07`/`09`/`11` both (FLOW-05, BOARD-03); `09-daily-sweep.md` is the BOARD-02 board↔ticket reconciliation engine. The load-bearing SAFE-01 deploy gate `12-release.md` requires named-human approval and never deploys prod itself, staying dispatch-neutral (mechanical PreToolUse hook deferred to Phase 5 / SAFE-02); `13-incident.md` is the blameless postmortem path (FLOW-04). The Wave-0 structural harness `check-structure.sh` (invariants V-01..V-13) reports ALL CHECKS PASSED. Verified 13/13 must-haves; FLOW-01..05, BOARD-02/03, GATE-01, SAFE-01 satisfied. Advisory code review (0 critical, 3 warning) recorded as pre-dogfood refinements (board-flow ownership gaps WR-01/02, UAT input conditionality WR-03). Requirements remain hypotheses pending the Phase 6 dogfood.*
