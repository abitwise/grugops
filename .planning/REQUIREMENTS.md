...
- [ ] **UAT-AUTO-04**: Run for real, the harness honestly resolves the A1/A2/A3 + B3 items in their UAT files (`05-HUMAN-UAT.md`, `06-HUMAN-UAT.md`, `11-HUMAN-UAT.md` scenario 3) — status set from real runs, never faked; B1/B2 remain human-only _(2026-06-16: B3 + A1/D-31 + A2/SAFE-02 resolved from real runs; **A3/DOG-02 human-waived** — carried to the next-milestone decentralization that removes handoffs, making the dual-path handoff-parity test moot. Kept `[ ]` because A3 is genuinely unverified, not because work is pending.)_
