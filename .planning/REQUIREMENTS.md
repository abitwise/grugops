...
- **Version:** v2.1. Phases continue from 27. ~~Artifact SemVer stays 0.1.0 (D-28).~~ Reversed 2026-09-18 by human decision at the v2.1 release: artifact SemVer is `2.1.0`, aligned with the milestone tag.
...
**Amendment 2026-08-20 — MODEL-06 narrowed to what is true and provable.** As originally written, MODEL-06's first clause asserted per-subagent model selection to be a capability of Claude Code and of no other host CLI, and concluded that the other four are unaffected. The conclusion is true; the capability claim rested on a premise this phase's gap-closure research refuted. The superseded wording is recorded verbatim in this file's git history and quoted in `.planning/phases/29.1-per-role-model-assignment/29.1-VERIFICATION.md`; it is described rather than reproduced here so that a `grep` for the refuted claim over the live planning surfaces returns nothing. `.planning/phases/29.1-per-role-model-assignment/29.1-RESEARCH.md` §"Host-CLI per-agent model support (gap-closure research, MODEL-06 / WR-06)" establishes, HIGH against primary vendor documentation in each case, that **all four** non-Claude host CLIs carry a per-agent `model` field — Codex CLI in `.codex/agents/*.toml`, Gemini CLI and OpenCode in agent-file frontmatter, GitHub Copilot CLI as a `.agent.md` property — and that three of the four document an inherit-by-default semantic essentially identical to the one this kit relies on. The requirement's *conclusion* survives unchanged, but for a different reason: grugops emits no agent definition for any of those four, which is a fact about this kit, provable by `grep`, needing no vendor citation. The requirement is therefore narrowed to that true form rather than deleted, and the superseded wording is described here rather than silently replaced. MODEL-06 stays unchecked; verification decides its status.
...
- [x] **UATX-02**: Browser MCP tooling is used to *author* specs, with `@playwright/mcp` pinned (pre-1.0) and documented for all five host CLIs.
- [x] **UATX-03**: Claude in Chrome is available as an optional, clearly-labelled `verified_by: <named human>` lane, and is structurally barred from producing a `§14-gate` stamp.
...
- [x] **UATX-05**: An absent or unusable browser produces a **loud skip** leaving the UAT `pending` (reusing the existing Tier-2 convention verbatim), never a silent pass.
...
- [x] **DASH-01**: `scripts/board-model.ts` is the single authority for every board sub-grammar — the column-heading parser is **extracted from and deleted in** `validate-agent-factory.ts` (porting the WR-03 prefix-match hardening verbatim), and it becomes the first authority for the ticket-row and WIP-number grammars.
...
- [x] **DASH-05**: Torn reads and the Windows `atomicWrite` ENOENT window are handled by read-verify-reread with a last-good snapshot and a visible stale badge; a partial parse or an ENOENT is never rendered as an empty board.
...
- [x] **DASH-07**: Non-TTY, `--json`, and `--once` modes work for CI and piping, and the renderer degrades visibly rather than rendering a confident wrong board.
- [x] **DASH-08**: The snapshot shape is stable enough that a future web renderer consumes it unchanged; the dashboard adds **zero** runtime dependencies and no listening socket this milestone.
...
| UATX-01 | Phase 31 | Gaps Found — accepted open by override (D-44) |
| UATX-02 | Phase 31 | Complete |
| UATX-03 | Phase 31 | Complete |
