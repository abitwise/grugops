...
- [ ] **CMP-02**: A load-bearing-field carve-out — `verified_by`, `failed-attempt`, `supersedes`, and `by`/`at` provenance are compaction-exempt; a RED test fails if any is dropped. _(22-05 EXECUTED (round-4 oracle unification) — both named bypasses CLOSED with proof: failed-attempts now flow through the SAME id-keyed byte-equal pass as durable notes (CR-01), the raw side is id-collision-guarded mirroring the promoted side (CR-03), kind∈NOTE_KINDS validated (WR-03), unparseable .md fails closed (WR-02), FA identity keyed on the frozen id not the body token (WR-01), and the read path adopts the single exported context-io.parseNote (IN-02). RED→GREEN demonstrated against the COMMITTED scripts/compactor.js: CR-03 + CR-01 exit 0 pre-fix (commit ec252b0) → exit 1 post-fix naming the colliding id / altered field. freshness exits 0; full non-e2e regression green. Checkbox stays unchecked pending the formal round-4 re-verification — do NOT mark complete on a green suite alone; adversarially reproduce against the committed .js. See 22-05-SUMMARY.md.)_
