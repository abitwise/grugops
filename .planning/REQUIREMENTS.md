...
- [x] **INSTALL-05**: `install.sh --check` (and `install.mjs`) is a doctor that verifies every referenced path resolves (kit at the kit root, state in the repo), names the first failure with its referencing file, and uses clear exit codes (0 pass / nonzero fail; WARN→0; `--strict` gates WARN)
