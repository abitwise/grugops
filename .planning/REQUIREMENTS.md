...
- [x] **CMP-02**: A load-bearing-field carve-out — `verified_by`, `failed-attempt`, `supersedes`, and `by`/`at` provenance are compaction-exempt; a RED test fails if any is dropped. _(CLOSED at round 8 (22-08), verified 2026-06-19 — 22-VERIFICATION passed 8/8. Took 8 rounds and 7 distinct bypasses of the same silent-absorb class; each round's heuristic boundary test proved a strict subset of `parseNote`'s grammar. Final closure is STRUCTURAL, not another heuristic: one exported `isRecognizedFrontmatterLine` grammar shared by `parseNote` + `splitNotes` so the two cannot drift, boundaries keyed on an id-bearing frontmatter run, and every region either recovered or refused — never silently absorbed. RED→GREEN proven against the COMMITTED `scripts/compactor.js` (22-08-RED-baseline.txt exit 0 "carve-out intact" → 22-08-GREEN-proof.txt exit 1 naming the dropped id). Two fail-SAFE deferrals remain, both carried as v2.0 tech debt: the WR-03 usability false-positive (a faithful note whose body holds a column-0 `id:`-looking line is loudly REFUSED rather than admitted) and the `---\n--- \n…` byte-round-trip adjacency (397/30000 fuzz occurrences, all fail-closed, never silent-absorb). Lesson recorded: a green suite is not proof of a safety invariant.)_
...
- [x] **MIGR-02**: Delete all 17 handoff templates + the `plans/handoffs/` seed; update `validate-agent-factory.ts` + `generate-catalog.ts` in the *same* grep-to-zero change.
...
- [x] **MIGR-04**: `install.ts --migrate` renames user `plans/handoffs/` state to a timestamped backup (never delete-first); rollback is `git revert`.
