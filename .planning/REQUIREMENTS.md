...
- [ ] **CMP-02**: A load-bearing-field carve-out — `verified_by`, `failed-attempt`, `supersedes`, and `by`/`at` provenance are compaction-exempt; a RED test fails if any is dropped. _(ROUND-4 RE-VERIFICATION 2026-06-18 = **gaps_found** — STILL bypassable. 22-05 (oracle unification) DID close the two round-3 gaps with RED→GREEN proof against the committed .js — CR-01-r4 (FA folded into the id-keyed byte-equal pass) + CR-03 (raw-side id-collision guard) — plus WR-01/WR-02/WR-03/IN-02-partial; freshness exits 0; 280 non-e2e tests green. BUT a NEW 4th bypass remains: WHITESPACE/PARSER-PROJECTION DRIFT. The oracle adopted the shared parser (parseNote) but NOT the shared validator — checkCarveOut never calls validate(). parseNote's kv regex matches a key only at column 0, so an indented or `key : value` (space-before-colon) load-bearing line silently projects to "" with no parse error, and the byte-equal loop compares the projection, not the file bytes. Reproduced by deep code review (22-REVIEW.md CR-01/CR-02/CR-03) AND by the orchestrator against the committed scripts/compactor.js with a column-0 CONTROL: a §14-gate-verified finding (folded out by a soft note's supersedes) is dropped at exit 0 when its raw `verified_by` is indented one space — and correctly refused (exit 1) when at column 0. Fix (round 5): run the shared validate() over every raw+promoted note AND/OR fail closed on any unrecognized in-fence line shape; compare on bytes not the lenient projection; RED-first line-shape sweep (indent / space-before-colon / trailing-ws / CRLF × field × kind). See 22-VERIFICATION.md + 22-REVIEW.md. Green suite ≠ proof. Next: `/gsd-plan-phase 22 --gaps`.)_
