...
- [ ] 25-08-PLAN.md — GOV-01/SC1 gap closure (round 5): make the admit-SHAPE detector STRUCTURAL inside the ONE `liveAdmitSegments` authority (mirroring the round-4 command-word invert, extended from the command word to the argument run) — a recognized-launcher segment (node|npx|tsx|nodejs) whose script-or-verb position carries a shell-expansion trigger (glob `*?[`, `$`-parameter `$NAME`/`${...}`/`$N`, command-substitution `$(...)`/backtick via the existing synthetic DYNAMIC_COMMAND_WORD token in argument position, unquoted word-split `$`), OR whose argv is fed cross-segment by an stdin→argv feeder (xargs), is statically UNRESOLVABLE → fail-closed GATE; a literal `admit` verb behind any script spelling is ADMIT → gate per dial; a fully-static non-admit launcher is NONE → ALLOW (bounded over-block: a dynamic `node $SCRIPT build` is gated only while `human_admission != off`). Closes class A glob script-ref (`node scripts/context-i*.js admit`), B arg-position command-substitution (`node $(echo … admit …)`), C parameter-expansion/word-split (`node "$S" admit`, `node <ctx> "$V" …`, `$A`), D cross-segment stdin→argv (`… | xargs node`), the nested wrapper+expansion compound (the round-4 catch-all consults the same detector), the D-01 glob self-set (`commandHasLiveAdmitShape` made structural-consistent), and the dial-`all` routine-via-expansion — from ONE structural detector. The trigger set is the complete BOUNDED POSIX expansion alphabet (a grammar property, not a spelling list); does NOT widen `COMMAND_MODIFIERS`, keeps the single authority (second walk stays deleted, `liveTokens` byte-identical); `scripts/context-io.ts` NOT edited (the un-forgeable hook re-closes the dial-`all` case; the weak self-settable D-05 admit() tier adds no un-forgeable defense). + anti-whack-a-mole STRUCTURAL-SHAPE property fuzz (launcher × expansion-trigger × trigger-position × dial × admit-ordering, with a held-out combination) + both-direction RED→GREEN proof vs the committed `.js` (blob 756ce508…) + independent both-angle red-team checkpoint (Wave 8). Preserves the round-4 a/b/c + round-3 Class A/B/E/F + round-2 GAP-B/C/D + SC2 + the GOV-02 ledger
