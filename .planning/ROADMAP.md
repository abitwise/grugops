...
- [ ] **Phase 25: Governance-on-a-Dial** — `context.human_admission` + `context.audit_retention` enterprise tiers over the decentralized substrate; the un-dialable safety floor unchanged (all 3 plans executed; verification GAPS_FOUND 2026-06-24 — two independent opus red-teams reproduced real GOV-01 bypasses vs the committed admission-guard.js: matcher false-negatives [subshell / `\`-continuation / `npx tsx`] + hook fail-open on garbage/corrupt-config dial; SC2 verified, SC1+SC3 fail; formal gap-closure planned via `/gsd-plan-phase 25 --gaps`)
