...
- [ ] **Phase 25: Governance-on-a-Dial** — `context.human_admission` + `context.audit_retention` enterprise tiers over the decentralized substrate; the un-dialable safety floor unchanged (3 base plans + gap-closure rounds 1-3 [25-04/05/06] executed; round-3 UNIFY [25-06] CLOSED Classes A/B/E/F + preserved SC2/GOV-02/GAP-B/C/D, but the blocking 25-06-04 independent both-angle red-team found a NEW command-RESOLUTION class [leading-run modifier-operand not consumed / unlisted wrapper / leading redirection — `timeout 5 node …admit` slips] → STILL gaps_found 2026-06-25 [round 4, 8th green-suite-insufficient catch]; SC2 verified, SC1 fail; fix = invert the default to fail-closed on an unresolved command word behind a live admit shape; next `/gsd-plan-phase 25 --gaps`)
...
**Plans**: 7 plans (7 waves; 25-04 + 25-05 + 25-06 + 25-07 are gap-closure rounds for the SC1+SC3 GOV-01 bypasses)
Plans:
**Wave 1**
