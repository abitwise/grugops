...
Phase: 25 (governance-on-a-dial) — round-5 gap-closure plan 25-08 PLANNED + design-verified; READY TO EXECUTE (`/gsd-execute-phase 25`)
Plan: 25-08 round-5 gap-closure (8 plans total) — created + revised through THREE independent opus adversarial plan-checks (5→3→0 blockers) + a disclosure pass; committed a5e9ee6. Round-4 25-07 remains tasks 01-03 committed with checkpoint 25-07-04 = BYPASS_FOUND (SC1 still forgeable until 25-08 executes and ITS blocking red-team confirms closure).
Status: 25-08 makes admit-SHAPE DETECTION structural — the fix the round-4 red-team's converged root cause demanded. Design (verified by 3 opus checks): `tokenIsFinalLiteral` is an ALLOWLIST of provably-inert chars `^[A-Za-z0-9/._:=,-]*$` (a token outside it can't be proven its final literal form → unresolvable → fail-closed gate; structurally catches glob/brace/extglob/tilde/`$`/backtick/quote-removal/word-split without enumerating them); position-free RULE 1 scans the whole post-launcher argv for the first final-literal `context-io` token (closes the npx value-flag regression); RULE 2 direct/forwarding split + a new `JS_RUNNERS={bun,bunx,deno,ts-node}` capability set (closes `bun $S $V`; distinct from COMMAND_MODIFIERS, not a widening); a bash-grounded fuzz oracle (spawns bash, breaks the self-referential circularity). THREE disclosed accepted limits (no fabrication): the bounded `node $SCRIPT build` + BROADER forwarding-runner over-block (`npx vitest run $FILE` etc. gate under active governance — opt-in/avoidable, asserted as intended); the `qjs $S $V` unknown-runtime residual (T-25-40); and the PRE-EXISTING `node /tmp/<renamed-context-io>.js admit` name-resolution residual (T-25-41, out of hook-tier scope). SC1 wording SCOPED to command-string shape-hiding, NOT claimed as full un-forgeability. Preserved: round-1..4 closures, four floor invariants, single liveAdmitSegments authority, COMMAND_MODIFIERS/LAUNCHERS unwidened, guard.ts + context-io.ts byte-frozen, freshness 0. The blocking independent both-angle red-team is the final task 25-08-04. Decision-coverage gate: parser FALSE-BLOCK overridden (could-not-parse on D-03/D-09's inner-colon bold labels; both decisions ARE covered — D-03 in 25-02, D-09 in 25-01/25-03; same quirk overridden on 25-07).
Last activity: 2026-06-26 — round-5 plan 25-08 planned + design-verified through 3 opus adversarial plan-checks (caught 5 design bypasses pre-code: brace/non-node-launcher/over-block-contradiction, then npx-flag-regression/extglob) + disclosure pass; ready for /gsd-execute-phase 25
