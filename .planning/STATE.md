...
last_updated: "2026-09-05T11:10:00.420Z"
state_head: ace883bba4b2392e66bc3298d7bfec76e57c24b5
progress:
  total_phases: 9
  completed_phases: 3
  total_plans: 176
...
| Phase 29 P35 | 50m | 3 tasks | 12 files |
| Phase 29 P36 | 35m | 3 tasks | 3 files |
| Phase 29 P37 | 40m | 3 tasks | 3 files |
| Phase 29 P38 | 50m | 3 tasks | 6 files |
| Phase 29 P39 | 30m | 3 tasks | 2 files |
| Phase 29 P40 | 34min | 3 tasks | 9 files |
| Phase 29 P41 | 20m | 3 tasks | 3 files |
| Phase 29 P42 | 35m | 3 tasks | 4 files |
| Phase 29 P43 | 22min | 3 tasks | 7 files |
| Phase 29 P44 | 47 min | 3 tasks | 5 files |
| Phase 29 P45 | 15min | 3 tasks | 7 files |
| Phase 29 P46 | 7min | 3 tasks | 5 files |
| Phase 29 P47 | 33min | 3 tasks | 1 files |
| Phase 29 P48 | 15m | 2 tasks | 2 files |
| Phase 29 P49 | ~50 minutes | 3 tasks | 5 files |
| Phase 29 P50 | 35m | 3 tasks | 3 files |
...
| Phase 29 P55 | ~95 minutes | 3 tasks | 2 files |
| Phase 29 P56 | 38 | 2 tasks | 4 files |
| Phase 29 P57 | 15m | 2 tasks | 4 files |
...
| Phase 29.1 P09 | 48m | 3 tasks | 6 files |
| Phase 29.1 P10 | 21min | 2 tasks | 5 files |
| Phase 29.1 P11 | 30min | 3 tasks | 9 files |
| Phase 29.1 P12 | 35 min | 2 tasks | 3 files |
| Phase 29.1 P13 | 12 min | 2 tasks | 3 files |
| Phase 29.1 P14 | 24 min | 2 tasks | 3 files |
| Phase 29.1 P15 | 35 min | 3 tasks | 5 files |
| Phase 29.1 P16 | 14 min | 3 tasks | 3 files |
| Phase 29.1 P17 | 22 min | 3 tasks | 7 files |
| Phase 29.1 P18 | 22 min | 3 tasks | 3 files |
| Phase 29.1 P19 | 23 min | 3 tasks | 4 files |
...
| Phase 29.1 P24 | 41 min | 3 tasks | 5 files |
| Phase 29.1 P25 | 79 min | 3 tasks | 2 files |
| Phase 29.2 P01 | 24 min | 2 tasks | 6 files |
| Phase 29.2 P02 | 30 min | 3 tasks | 3 files |
| Phase 29.2 P03 | 40 min | 2 tasks | 11 files |
...
| Phase 30 P03 | 88 min | 3 tasks | 11 files |
...
- [Phase ?]: Plan 29-35: the two replacement sectionBody bodies were KEPT SEPARATE rather than promoted to a shared helper — both generators are top-level script code that writes files on import, so neither may import the other and a wrapper would need a NEW module; a third exported name in frontmatter.ts would give one question a second name inside its own authority.
- [Phase ?]: Plan 29-35: D-50 IN-05's module-level import test narrowed to a SYMBOL-level ALLOW-LIST (section-locator family only) rather than a deny-list of frontmatter symbol names — a deny-list over a thirty-export authority is a hand-maintained set literal, this repository's second named systemic failure class.
- [Phase ?]: Plan 29-35: V-29-35-01 (private parseFrontmatter in generate-catalog.ts beside the exported one) measured at 0 key-set differences over 36 governed documents and recorded by name in audit §9.3c — OUT OF SCOPE by user decision, neither fixed nor absorbed.
- [Phase ?]: REACH.I5 names NO locator's answer: the review's and the plan's own suggested at>0 conjunct excluded 360 cells headLastUnfenced really violates I5 on. Floor 1800 -> 720, not 360.
- [Phase ?]: fail( excluded from the -1 contract exit set (it does not leave the scope) and disclosed as blind spot 6 instead.
- [Phase ?]: 29-37: the WR-02 replacement SHIPS rather than being deleted — input falsifiers exist (a canonical claim heading after a bare CR / U+2028 / U+2029, which `^`/`$` under `m` see as a line start and split("\\n") does not), so the plan's delete-if-unfalsifiable rule does not apply. Three inputs reach the refusal through readRegistry with the shipped recogniser.
- [Phase ?]: 29-37: the [B1] blast-radius pin in check-foundation-guards.test.ts moved from a LINE NUMBER to the site's CODE — a positional literal that drifts when nothing about the property moved is set-literal drift inside a guard.
- [Phase ?]: 29-37: the tripwire's two paren counters now diverge by one (+12/+11, disagreements 14 -> 15) and it was REPORTED not smoothed — the causing line is named and deliberately not reworded, because rewording deletes a true instance of the measurement's own error from the measurement of that error.
- [Phase ?]: 29-38: only WP-04's SECOND sentence is pinned — the first names no heading spelling, so no drift of it is visible to the gate and pinning it would assert an agreement the module cannot check
- [Phase ?]: 29-38: the rule-constant block MOVED above FORM_REMEDY rather than the constants being declared apart — a const cannot be read before its declaration, and two declaration sites 160 lines apart is how a second spelling of a pinned sentence gets written
- [Phase ?]: 29-38: main was RED at 2ca3ac3 — 29-37-SUMMARY.md's own frontmatter failed the D-49 control. A SUMMARY is written AFTER the regression run it reports, so a defect in its own frontmatter cannot be caught by that plan's evidence
- [Phase ?]: 29-40: the private parseFrontmatter in generate-catalog.ts is DELETED, not widened — one authority per predicate (D-24, G-29-1 / V-29-35-01 closed)
- [Phase ?]: 29-40: an unreadable frontmatter, a key declared twice and a key absent are THREE findings, never one sentence — a parse artifact is not a verdict
- [Phase ?]: 29-40: the frontmatter-parser owner tripwire is NAME-scoped and says so at its declaration; a duplicate under another name is a different predicate and out of its reach
- [Phase ?]: 29-40: a retired pattern's SPELLING goes with the pattern — the D-50 IN-05 classifier reads comments as code, so prose quoting a deleted regex re-creates the finding
- [Phase ?]: 29-40: an empty-valued cadence and an absent cadence both reach UNKNOWN - verify; the two grammars AGREE there, so the case asserts agreement rather than a fabricated disagreement
- [Phase ?]: 29-41: the comprehension prohibition is DECIDED by a rule (user decision (c)) — a bare term plus a co-occurring benefit verb on the same line, reusing the existing requiresOnSameLine mechanism; enumeration (option b) refused and recorded in the admission log with its measurement
- [Phase ?]: 29-41: a SECOND bare term (understand) was admitted because one member of the measured family carries no occurrence of comprehension and is outside that rule BY CONSTRUCTION — 29-UAT.md's measured_probe claim that all five bypasses red does NOT reproduce, and the discrepancy is reported rather than absorbed
- [Phase ?]: 29-41: BANNED_CLAIM_EXEMPT_SUPPRESSED 10 -> 12 DERIVED from the gate's own refusal text, moved in the same commit as the rule, both entrants (writing-profile.md:256 via marker benefit, :288 via marker improve) named — the pin is a function of which markers are admitted
- [Phase ?]: Plan 29-42: the two reds plan 29-41 handed over are cleared by DERIVATION, not re-pinning — the conditional-member count relaxed 1 to 3 with its reason at the assertion, and findingCount's stale 2 replaced by countBannedClaimOccurrences.
- [Phase ?]: Plan 29-42: FIVE surfaces encoded the one-conditional-member singular, not the three the plan named — a describe title and an admission-log reason string were undiscovered, plus the selector itself.
- [Phase ?]: Plan 29-42: four residuals opened by the adversarial pass are measured, counted, directional and LEFT OPEN — V-29-42-01 is fail-OPEN at 1983 hard wraps and 0 live instances; the other three are fail-closed at 0 live.
- [Phase ?]: 29-43: a shared derivation exports one function per QUESTION, named for the question — publicDocsCorpus() vs publicDocsScan(). A consumer taking the wrong one is the CR-01 defect, and that is why both exist.
- [Phase ?]: 29-43: .claude/ ADMITTED (24 files), not excluded as transitively covered — the argument was tested against generate-role-adapters.ts and REFUTED (adapter body prose is string literals in specialistBody()/coordinatorBody(), and scripts/ is excluded).
- [Phase ?]: 29-43: the banned-claim corpus is now 115 documents. Plan 29-44 must measure its admission cost over 115, not 82 or 83.
- [Phase ?]: WR-06 discharged by DELETING THE MECHANISM, not by shipping the refusal: D-53 removed the field from the type, so a runtime guard for that shape could never fire and counting it in a PASS line is AP-1.
- [Phase ?]: A tripwire is justified by the route the compiler does NOT cover — the field re-added to the INTERFACE, demonstrated at tsc exit 0 — never by the route it does.
- [Phase ?]: check-nul-bytes.ts widened to the whole forbidden control-byte class over an unchanged 1598-path scanned set; its own RED proof falsified the header claim that git's -text verdict is NUL-based, and the cross-check arms were re-anchored on two different predicates.
- [Phase ?]: 29-46: a stale number is DELETED from prose, never replaced by a fresher number in the same place — the range-free regex is the contract
- [Phase ?]: 29-46: where a count is useful, prose NAMES the constant that holds it (ROLE_COUNT/WORKFLOW_COUNT in scripts/kit-model.ts) rather than repeating its value
- [Phase ?]: 29-46: a comment that outlives its COUNT is the same defect as one that outlives its CONSTRUCT — found a fifth stale statement the round-5 review did not list
- [Phase ?]: 29-46: an audit body's identity is proven by sha256, not by a claim the diff was confined to a heading
- [Phase ?]: Round 6's adversarial pass produced the phase's NINTH false harness result and caught it: a plant that never landed rendered as a fail-open bypass of the sole carve-out. Assert the harness premise on BOTH sides — the gate ran AND the plant landed.
- [Phase ?]: V-29-42-01/-02/-04 CLOSED BY CONSTRUCTION with closing measurements; -02 and -04 closed AS DESCRIBED while their false-positive surface MIGRATED into V-29-44-01 — recorded rather than collapsed.
- [Phase ?]: 29-48: LANG-07 corrected to [x]/Complete in REQUIREMENTS.md under 29-VERIFICATION-round6.md's named authority; LANG-04 left byte-unchanged, its verdict reserved for round 7's verifier
- [Phase ?]: 29-48: a plan that corrects a requirement state declares requirements-completed: [] — the automated mark-complete marker is the mechanism that produced the inversion, and a second mechanism writing the same fact is a second authority
- [Phase ?]: 29-48: an EMPTY intermediate in a verification harness renders as a MAXIMAL finding — both of this plan's own harness defects published as alarming false findings about the file until a floor and four premise assertions converted them to aborts
- [Phase ?]: 29-49: the false residual block at BANNED_CLAIM_EXEMPT_REGION was DELETED, not corrected — its construct is gone, and a corrected record still leaves a reader reasoning about it at the worst address in the file
- [Phase ?]: 29-49: the exempt-read guard (WR-06's class) was fixed here rather than escalated, because the cached-read substitution cannot be sound without a failure path to set its success flag false on
- [Phase ?]: 29-49: the two derivation-refusal channels are reported in separate loops with separate prefixes and never concatenated — a refusal raised here and one raised in the corpus derivation have different remedies
- [Phase ?]: 29-49: no count is pinned on the refusal channel and the comment says so plainly — a refusal count is an event count, not a set cardinality
- [Phase ?]: 29-50: the production-dead NUL-only predicate was DELETED rather than wired up — wiring it in would have meant a second read of every hit-bearing file to answer what the wide scan's bytes array already answers
- [Phase ?]: 29-50: the bytes field's declaration POINTS at the module's measured table rather than restating it; the contradicted worked example was deleted, not corrected, because a corrected example is a second declaration in the place the first one rotted
- [Phase ?]: 29-50: the gitlink arm's fixture is an ordinary directory (identical EISDIR, deterministic under any user); the unwitnessed submodule path is escalated as V-29-50-02 rather than implied away
- [Phase ?]: 29-51: the anchored-block authority lives in audit-model.ts (a library), not in either gate — a gate import carries a refusal channel, a copy is a second grammar
- [Phase ?]: 29-51: the blank-verbatim refusal has ONE authority on the registry path (parseClaimBlock); the authority's own throw is a contract guard with zero live reachability, stated in source
- [Phase ?]: 29-51: the registry line-field disagreements (19 of 41) were MEASURED and NOT corrected — repairing a trace surface while measuring it destroys the measurement
- [Phase ?]: D-54 — the sole banned-claim carve-out stays POSITIONAL and its exempted BYTES are FROZEN against docs/audit/28-claim-registry.md; the exempt set is DERIVED with a two-sided cardinality assertion, never listed
- [Phase ?]: The per-group composition pin ships as a SECONDARY measure with its non-closure limitation stated in the source — it narrows the substitution class and does NOT close CR-01
...
- [Phase ?]: 29-55: the round-7 residual register derives its V- marker set by grep over the tree rather than from the predecessor's roll-up — which found 35 markers against round 6's 18, and nine that no residual register has ever rolled up
- [Phase ?]: 29-55: no number in a residual register is transcribed — 23 published numbers were re-taken on the final tree and the six disagreeing rows are published rather than reconciled; carried-count 0
- [Phase ?]: 29-55: a stale count in CI is replaced by the NAME of the constant that holds it, and a HISTORICAL enumeration of the stale values is the same construct wearing a past tense
- [Phase ?]: 29-55: LANG-04 remains unmarked — round 7 records evidence (both bypasses red by name, V-29-47-04 still fail-open with a live occupied instance at CHANGELOG.md:67) and reserves the verdict for verification
- [Phase 29]: 29-56 (D-55): guard_banned_claims's published header and module docblock narrowed to the decided predicate — one physical line, a derived document set, 22 pinned literals, outside the registry-anchored blocks of one named region; both numbers interpolated from the module's own derivations, never typed
- [Phase 29]: 29-56: the claim-site set was DERIVED (13 files over a 780-file denominator with named exclusions), not accepted from a plan's list; every derived site carries a disposition row in docs/audit/29-round8-residuals.md §1
- [Phase 29]: 29-56: two findings recorded and LEFT with reason — scripts/compactor.ts:7 restates the disproven caveman-as-token-economy claim (outside the gate's corpus by construction), and .github/workflows/ci.yml:221-224 still asserts absence of a CLASS of claim; both out of D-58's scope fence, both recommended to the follow-up
- [Phase ?]: 29-57: the round-7 review's reach figure (16 of 22 literals reachable) is CORRECTED to 11, derived from BANNED_CLAIM_LITERALS itself — multi-word and wrap-reachable are different quantities, and the five-member difference is the comprehension group whose pinned bare terms survive every split of its own longer phrasings
- [Phase ?]: 29-57: the round-7 review's house-style wrap figure (822/2458/60) is NOT carried because its predicate is unrecorded; a definition is stated and the figure re-derived at HEAD (757/2612/73), both printed and the disagreement named rather than reconciled
- [Phase ?]: 29-57: D-56 applied — the hard-wrap matcher is deliberately NOT fixed; V-29-57-01 carries the axis with its FAIL-OPEN direction, derived reach 11/22, derived live count 0, reproduction re-run at HEAD, and the named remedy with the reason it is declined
- [Phase ?]: D-55 applied at the third address: the kit's honesty floor states the predicate the gate decides, not three prohibitions over the shipped kit and the public documents
- [Phase ?]: The plan's premise that round 7 gave the enumeration axis a V- id was asserted and found FALSE; V-29-58-01 was opened in the register rather than minted in shipped prose
- [Phase ?]: V-29-58-01's live count is UNKNOWN - verify BY CONSTRUCTION, deliberately not 0 — the complement of a 22-member enumeration is not a searchable set
- [Phase ?]: Six of nine watching gates derive to NOTHING OWED; the only companion obligation is C-28-042's registry row, same commit
- [Phase ?]: D-57 implemented: the build-output freshness gate reads its committed side from git (git show HEAD:<path>, set derived from git ls-tree -r -l HEAD), so no earlier build step can repair its subject
...
- [Phase 29.1]: 29.1-09: The union case an arm pair asks for may not EXIST as an input — the two degraded branches in guard_model_assignment are exclusive by construction, so the pair was pinned structurally (the guard makes no warn() call at all) rather than by composing inputs that cannot compose.
- [Phase 29.1]: 29.1-09: Before asserting a phrase is ABSENT from a verdict, check the verdict is PRINTED on that path. Promoting the degraded branches would have deleted the run summary from every degraded run, making the WR-08 assertion pass vacuously; the summary is now rendered on BOTH outcomes.
- [Phase 29.1]: 29.1-09: IN-04 resolved by DERIVING, and the measurement decided it — SPAWN_GRANT_SCAN_PARTS enumerates four families, nonAgentSurfaces named two, and the missing one (packaging) contains slash-command.template.md, the exact surface the review named as its promote trigger. The trigger had already fired.
- [Phase 29.1]: 29.1-10: the model dial's Claude-Code-only reach is stated as a fact about what grugops EMITS (per-agent adapters at .claude/agents/ alone), not as a capability claim about the other four host CLIs — all four of which DO carry a per-agent model field, each cited
- [Phase 29.1]: 29.1-10: the retired wording is quoted NOWHERE on a live surface — not in the oracle, not in the two dated amendment notes. Annotate-never-rewrite is satisfied by describing it and pointing at git history and 29.1-VERIFICATION.md, so the refuted claim greps clean off the shipped tree
- [Phase 29.1]: 29.1-11: WR-05's must-have was NARROWED, not met as written — check-kit-refs Assertion 1 (D-08.1) forbids agent-factory/config/ refs in kit prose, so the documentation obligation is split by audience: the config reference names both locations and holds the precedence rule; the packaging authority names the user's location plus the location COUNT and asserts the kit-internal path's absence positively
- [Phase 29.1]: 29.1-11: the model-dial surface roster's denominator is DERIVED from a run-time scan of every shipped markdown document, with the marker chosen by recorded measurement and the audit register partitioned out by identity against REGISTRY_PATH — never trimmed to fit
- [Phase 29.1]: CHECK_ROOT is NORMALISED rather than REFUSED (29.1-12, R2-CR-01) — A relative override is a documented invocation shape (check-kit-refs.ts:11-12 attaches no absoluteness contract), so refusing it would close a working invocation to fix a RENDERING defect. The disclosed cost: resolve() is taken against the process working directory, so one override string names two roots from two directories — a property the caller already owns.
- [Phase 29.1]: The boundary alternation is ONE module-scope declaration read once (29.1-12) — Two hand-kept copies of a boundary rule is how a second, disagreeing grammar arrives — this project's most-repeated defect class. ROOT_PATH_BOUNDARY occurs exactly twice in the file: one declaration, one reference.
- [Phase 29.1]: MEASURED CORRECTION: reverting the normalisation alone does NOT red the byte-identity case (29.1-12) — Each half of the R2-CR-01 fix independently prevents the CHECK_ROOT=. corruption, so (o-rel) only reds when BOTH are absent. New case (o-rel-word) — a mirror named after a word the refusal sentence contains — makes the normalisation half genuinely load-bearing rather than assumed. A mutation that fails to red is a harness result, not a code result.
- [Phase 29.1]: 29.1-13: the stray-pin arm's excluded members are published as a COUNT plus file identity plus enumerated refusal code — never as a second copy of guard_wr05's reason sentence. — One fact, one sentence. guard_wr05 already names an unadmittable scan file with its code and fails the gate closed; a second sentence for the same fact is duplication this gate's conventions refuse. What the stray-pin arm owes the reader is the honest DENOMINATOR, not a duplicate finding.
- [Phase 29.1]: 29.1-13: case (m) is narrowed rather than replaced — it pins the advisory HELPER, and the new sibling (m-channel) pins the advisory CHANNEL, both over one hoisted slicer. — Two separately attributable properties over one region of text. A second slicer would be a second authority on where the function begins and ends; a replacement case would have lost the helper property. Proven by a mutation that reds (m-channel) while (m) stays green.
- [Phase 29.1]: The two ubuntu-block readers do NOT share a helper: copying it would create two authorities over one predicate, and promoting it into a production module only tests consume would add a shipped surface for a test. Each file bounds its own slice and (r-class) is the single authority over whether the CLASS is bounded. — This repository's one-authority-per-predicate rule cuts both ways here; the resolution is to make the CLASS the asserted thing rather than the implementation, with the right-bound marker required in one canonical source spelling so the two slicers cannot drift into equivalent-but-differently-written.
- [Phase 29.1]: The CI adapter-cleanliness assertion is TWO lines — one that PRINTS the porcelain status of .claude/agents/ and one that ASSERTS it is empty — because a bare emptiness assertion fails with no output and is a gate the failing operator cannot act on. — Measured on this tree with a planted untracked adapter: the replaced git diff --exit-code printed nothing and exited 0, and the pair prints the entry and exits 1. (r-commands) pins the printing line before the asserting line, so the position is part of the guarantee.
- [Phase 29.1]: Floor 0b runs AFTER Floor 0 in resolveModels. Placement MEASURED, not chosen: both orders were built and the whole 100-case oracle run under each, and no existing case moved either way. The order is decided by the one input that distinguishes them — a caller with both a bad preset and a bad overrides value receives the PRESET finding, because the preset decides the base map the overrides are applied on top of. (29.1-15, R2-WR-01)
- [Phase 29.1]: The deleted symbol roleCorpusCardinalityRefusal is NOT NAMED in any shipped source, including in the comment that records its deletion. A shipped comment naming a symbol that does not exist sends a reader to a fact that is not there — the defect class the deletion closes. Its argument survives in tieredCorpusRefusals' docstring; its NAME lives in the 29.1-15 SUMMARY, the commit message and git history. (29.1-15, R2-IN-02)
- [Phase 29.1]: R2-IN-04 was closed by moving the CODE to the MESSAGE. readAssignmentPayload now checks each announced alias against isModelAlias rather than only against typeof. The rejected alternative — rewording the declared shape to a bare string list — is recorded beside the check: the announced aliases are what a gate compares against the resolved alias set, so a reader admitting a value no resolution can produce hands its consumer a set difference to explain rather than a payload to refuse. (29.1-15)
- [Phase 29.1]: 29.1-16: R2-WR-04 closed on BOTH halves — the packaging authority's `model` bullet stops restating the precedence rule (ordinal AND absence clause deleted) and stops claiming not to restate it (the self-assessing sentence deleted, not reworded, under D-59/D-16). — A document cannot decide a totality about its own prose; either half of the review's either/or alone leaves a defect standing.
- [Phase 29.1]: 29.1-16: the plan's basename spelling identifies NEITHER configuration location (`factory.config.json` is a substring of `.grugops/factory.config.json`), so the derived spelling set is PARTITIONED into identifying and ambiguous with all three cardinalities asserted; the plan's basename-only acceptance criterion is recorded NOT satisfied as written, with the measurement that disproves it. — Asserting the basename absent contradicts the same case's requirement that the first candidate be present; exclusion is by derivation over the constant, not by a hand edit a later rename would not move.
- [Phase 29.1]: R2-WR-05 / D6 closed as the USER RULED: agent-factory/config joined check-kit-refs SCAN, with an exemption that is a PREDICATE over hits (a self-reference to an existing sibling), both cardinalities asserted two-sided in the gate (2 hits, 1 file), a vacuity floor on the denominator, and the exemption published in the Assertion 1 pass line. — install/install.ts:1065 copyKit recursively copies the whole agent-factory tree to every installed user, so the config field reference is shipped kit prose. Deleting its two kit-internal mentions was never available: model-dial-consistency.test.ts asserts the reference names every declared configuration location, which IS the WR-05 closure.
- [Phase 29.1]: The acceptance criterion grep -c const SCAN scripts/check-kit-refs.test.ts returns 0 is recorded NOT SATISFIED as written: measured 1, the extractor locator string that must quote what it locates. Splitting the literal to make a grep return zero was rejected (the move 29.1-11 deviation 3 rejected). Honest anchored substitutes recorded in file and SUMMARY. — A locator string will always answer a grep for the thing it locates. Reporting the measured number plus an anchored substitute predicate is honest; hiding the string is the failure class this repository spends rounds closing.
- [Phase 29.1]: Round 3 reconciliation: all twelve round-2 findings closed, each by exactly one plan, item list DERIVED by grep from both round-2 documents. The count is reported at BOTH granularities: fifteen entries (12 findings + 3 human_verification entries) but SIXTEEN items, because the third entry names two. D6 closed here; D5, MODEL-07/boundary and MODEL-07/precision remain CARRIED for human adjudication. — A single asserted count over a set assembled from two documents is granularity-dependent; reporting only the plan number would have repeated the backstop-marker double-count this project already paid for.
- [Phase 29.1]: R3-CR-02 closed by CANONICAL FORM — a captured basename is a self-reference only when it is a member of the depth-1 regular files agent-factory/config actually carries, derived through check-kit-refs' own walk(). One rule refuses the parent traversal, the current directory, a subdirectory name and a fabricated name, and closes the depth residual round 3 filed as Info. The review's proposed deny-list of '.' and '..' would have left the subdirectory shape open.
- [Phase 29.1]: The Assertion 1 exemption counts MENTIONS, accumulated from the same CONFIG_NAMED_PATH extraction the predicate decides on. All three units — 1 file, 2 lines, 3 mentions — are asserted two-sided and published in the verdict, so a gate can no longer print a number in a unit it does not count.
- [Phase 29.1]: check-kit-refs publishes its derived exemption sibling set (cardinality and member names) so the harness asserts the disclosed denominator instead of re-implementing the derivation rule test-side, following the reportedMarkerSites precedent already in that test file.
- [Phase 29.1]: 29.1-19: the at + UBUNTU_BLOCK_STEP_NAME.length slice base survives the two-reader reconciliation; the bare at base returned the region plus its own heading as a command entry (21 against 20)
- [Phase 29.1]: 29.1-19: the third tautology site (model-dial-consistency.test.ts) was closed although the plan's files_modified omitted it, because the must_haves require three deletions and only two existed in the declared files; disclosed as WINDOWS row 82
- [Phase 29.1]: 29.1-19: the (o-one-consumer) census widening is idiom-shaped and file-scoped rather than a taint tracer; the narrower reach is disclosed as WINDOWS row 81 with direction FAIL-OPEN
- [Phase 29.1]: 29.1-19: R3-IN-01's stated cause (distance) was corrected in the comment to the measured cause (indirection) — the census window reads FORWARD, so no window size fixes a declaration sitting behind the call
- [Phase 29.1]: 29.1-20: the packaging authority's cross-document section citation is compared through ONE declared canonical form (citableForm) with an injectivity floor, not two normalisations — The heading wraps the field name in inline code delimiters the citing sentence omits, so byte equality would be RED on a correct tree; the form is applied by one function to both sides and the cited surface's 8 headings are asserted to produce 8 distinct forms
- [Phase 29.1]: 29.1-20: agent-factory/packaging/subagent.frontmatter.md was NOT edited — the canonical form absorbed the delimiter difference — The plan permitted a spelling change only if the measurement required it; it did not, so the prohibition against touching the document bound and agent-factory/ is byte-unchanged
- [Phase 29.1]: 29.1-20: the synthetic bound input is TRUNCATED at the region's own end before the bullet is appended — Measured: appended to the whole document, dropping the reader's bullet arm left the case GREEN; truncating removes every other candidate bound so the bullet arm becomes load-bearing
...
- [Phase 29.2]: D-29.2-R1 IMPLEMENTED: install.ts reads the generator's announced resolution by writing a fixed ES-module probe into the temp mirror and spawning it, so the announcement grammar stays in scripts/model-tiers.ts and install/ imports nothing from scripts/. — D-04 (report the resolution) and D-18/D-28 (install/ imports nothing from scripts/) point in opposite directions. Copying either announcement prefix into install.ts would create a hand-synced cross-boundary literal whose drift direction is silent: the reader stops finding the line exactly when the emitter stops announcing. The probe imports the MIRRORED ./model-tiers.js and prints one JSON line, so the grammar is consulted rather than copied and the installer's module graph is untouched. Measured: grep -c for both prefixes in install.ts prints 0.
- [Phase 29.2]: The install-time render mirror carries a package.json declaring {"type":"module"}, and every spawn uses process.execPath rather than a bare launcher name. — Implicit-ESM detection for a bare .js only became Node's default at 22.12; on 22.0-22.11 the mirrored generator dies with "Cannot use import statement outside a module", and R-5 (no fallback byte source) would turn that into an install laying down zero adapters on a supported Node. process.execPath avoids a PATH/PATHEXT lookup and is correct on Windows (T-29.2-01). Both are pinned by cases over install.ts AND the committed install.js.
- [Phase 29.2]: install/install.test.ts's makeSyntheticSrc() is now RENDERABLE and its adapter set is DERIVED from the real one; the two symlink plants REPLACE an existing member instead of adding an eighteenth. USER-APPROVED (Option A) at the plan's blocking-human checkpoint. — R-5 makes an unrenderable source install zero adapters, so fourteen cases driving the old synthetic fixture would have become assertions about the fail-closed branch rather than about the derivation they were written for. The fixture is now coupled to the real roles corpus where it was deliberately decoupled before; that cost is stated in the fixture's own comment. Every reshaped case still asserts the same invariant, with per-case accounting in 29.2-01-SUMMARY.md, and the reshape was proven behaviour-neutral by running it against the PRE-CHANGE committed install.js: 77 passed, 1 skipped, zero failures.
- [Phase 29.2]: 29.2-02 R-2: the target banner recogniser is whole-line equality against a KIT_BANNER constant in install/install.ts, pinned byte-equal to the generator's PROVENANCE source text AND to the seventeen committed adapters' emitted bytes, with an exactly-one-per-adapter count floor for the silent direction.
- [Phase 29.2]: 29.2-02: materializeAdapter is build-then-compare-then-write over the FINAL bytes, and the --check doctor computes its expected side through the same pure transformAdapter, so the skip decision and the staleness verdict are one comparison with one answer.
- [Phase 29.2]: 29.2-02: the --check doctor does NOT restate renderAdaptersInMirror's generator-twins pre-check; it branches on the helper's own failure result and supplies the severity plus the sentence that makes an absent verdict legible as one, keeping one authority per predicate.
- [Phase 29.2]: Refuse a hazardous adapter destination rather than unlink it: --migrate is a declared one-time conversion, the ordinary re-run is not
- [Phase 29.2]: The write-path guard precedes D-11's skip-if-identical arm, so 'skipped (identical copy present)' can never describe bytes outside the target
- [Phase 29.2]: adapterDestHazard bounds D-13 rather than withdrawing it: a regular file inside the target is still kit-owned and still rewritten
- [Phase 29.2]: A hazardous destination is its own doctor class, not folded into stale or absent; it stays in the D-10 WARN tier promoted by --strict
- [Phase 29.2]: Containment compares path segments (resolvedTarget + sep), never a bare startsWith; both sides realpath-resolved
- [Phase 29.2]: 29.2-05 WR-03: a render-mirror cleanup failure reports through report("cleanup", ...), never verify(). verify() increments VERIFY_FINDINGS which drives exit 3, and exit 3 in this installer means an install CLASS was refused (D-03); housekeeping must not be given that meaning.
- [Phase 29.2]: 29.2-05 WR-05: the temp-mirror disclaimer has ONE declaration and three interpolating consumers (the doctor NO VERDICT arm, the render refusal, the success-path relay). The generator's relayed line is never rewritten — that would be a second grammar over the same sentence.
- [Phase 29.2]: 29.2-05 IN-01: held as a CORRECTED COMMENT rather than a behaviour change. Making the banner slot anchor-preserving would move rendered bytes in every target, so the transform asymmetry is stated with its reachability bound instead.
- [Phase 29.2]: 29.2-05 WR-02: the two-location precedence sentence STAYS and gains an audience bound. It is true of the resolver and of a grugops checkout; deleting it would trade one false impression for another. The kit-internal path is not spelled a fourth time — check-kit-refs declares that exemption at exactly 3 mentions and the argued answer was not to add one.
...
- [Phase 30]: The surviving governance reader IS the discriminated fail-closed one, renamed — the fail-open value reader was deleted outright, not kept as a wrapper — A convenience wrapper over the surviving reader would be a second authority wearing a smaller name, which is the exact defect AUTO-06 exists to close. The rename is the mechanism: there is nowhere for a second verdict to live. GovernanceConfigWithCheckpoints collapsed into GovernanceConfig for the same reason.
- [Phase 30]: admit()'s unreadable-config refusal is NOT scoped to high-severity roles, and degrades to UNKNOWN - verify rather than throwing — Scoping it to HIGH_SEVERITY_ROLES would have produced a smaller diff and a green suite while leaving the fail-open standing for every routine admission, which is most of them. An unknown dial is unknown for everybody. Measured RED first: the corrupt-config fixture returned findings = [] and did not throw.
- [Phase 30]: D-13 names model-tiers.ts as the one deliberate non-governance reader; measurement shows 8 config-resolving sites and 3 dial readers, so only the narrower governance-subset claim was asserted — compactor.ts reads context.compaction from the same file, and three gate/validator scripts read the shipped kit config. Writing the instructed comment verbatim would have violated the no-fabrication floor. The site count (8) and the governance subset (1) are both derived and pinned; D-13 is annotated in deferred-items V-30-03-01 rather than rewritten.
...
- LANG-04 is marked Complete in .planning/REQUIREMENTS.md (set by 29-45's docs commit d5360dc via requirements mark-complete) against the round-5 verifier's explicit 'stays Gaps Found'; LANG-07 is Gaps Found against its 'can move to Complete'. Exact inverse. Recorded as V-29-47-05; the verifier owns the correction.
...
Last session: 2026-09-05T11:09:01.912Z
