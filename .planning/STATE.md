...
|------|------|--------|--------------|
| 27-29 (tracer) | 1 | **CR-01** — `unquote()` inverted to an escape ALLOWLIST per **D-30**; every un-allowlisted backslash sequence refuses by name, pinned by an exhaustive alphabet property test | KIT-03, SPAWN-04 |
| 27-30 | 2 | **IN-02** — leading `%`-directive refused by name (**D-34**), closing the second silent-SUCCESS arm in the same module; **IN-01** — the scoping gate proven load-bearing in both directions | KIT-03, SPAWN-04 |
| 27-31 | 2 | **WR-01** — `MAX_WALK_ENTRIES` work bound kept explicitly separate from the per-path ancestor stack (**D-35**); **WR-04** — cycle arm named on both sides, reported in the installer, thrown in the authority (**D-36**, AMENDS D-29) | KIT-01, KIT-02 |
| 27-32 | 3 | **WR-02** — `SOURCE_MARKERS` naming the runtime artifact, asserted over the real repo (**D-37**); **WR-03** — comment amended to D-28's rationale plus the cross-site equality case (**D-38**); **IN-03** — README reversal clause | KIT-02 |

**Two decisions an executor must not quietly undo.** **D-36 amends D-29's `kit-model` half:** a currently-*passing* case asserting a cycle yields the real member set must become a named throw — 27-31 flags this so the throw is not weakened to keep the old assertion green. **D-30 rejects the review's own proposed fix:** `27-REVIEW-GAPS-3.md` proposes a `NUMERIC_ESCAPE` regex; that was declined as the fourth enumerate-the-bad patch on the same fail-open, and implementing it instead of the allowlist is a defect, not a shortcut.
